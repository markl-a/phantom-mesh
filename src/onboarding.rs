...
pub struct OnboardConfig {
    ...
    /// Remote directory to deploy into (default: ~/clawtex-worker).
...
impl WorkerOnboarder {
    ...
    pub fn generate_worker_config(worker_name: &str, hub_url: &str) -> String {
        format!(
            r#"# Clawtex Worker Configuration
...
"#,
    ...
    async fn step_ssh_check(&self, user: &str, host: &str, port: u16) -> OnboardStep {
        let start = Instant::now();
        let result = Self::run_ssh_command(user, host, port, "echo clawtex-ssh-ok").await;

        match result {
            Ok(output) if output.contains("clawtex-ssh-ok") => OnboardStep {
    ...
    async fn step_deploy(&self, config: &OnboardConfig) -> OnboardStep {
        ...
        if let Some(ref binary_path) = config.binary_path {
            ...
            let remote_path = format!("{}/clawtex-worker", remote_dir);
            ...
            match Self::run_shell_command(&scp_cmd).await {
                Ok(_) => {
                    // Make executable
                    let chmod_cmd = format!("chmod +x {}/clawtex-worker", remote_dir);
            ...
            let write_cmd = format!(
                "cat > {}/worker.py << 'CLAWTEX_WORKER_EOF'\n{}\nCLAWTEX_WORKER_EOF",
    ...
    async fn step_deploy_config(&self, config: &OnboardConfig) -> OnboardStep {
        ...
        let write_cmd = format!(
            "cat > {}/worker.toml << 'CLAWTEX_CONFIG_EOF'\n{}\nCLAWTEX_CONFIG_EOF",
    ...
    async fn step_start_worker(&self, config: &OnboardConfig) -> OnboardStep {
        ...
        let start_cmd = if config.binary_path.is_some() {
            ...
            format!(
                "cd {} && nohup ./clawtex-worker worker --hub {} --name {} --port {} --device-type {} > worker.log 2>&1 &",
                remote_dir,
                config.hub_url,
                config.worker_name,
                config.worker_port,
...
"""Clawtex lightweight Python worker - auto-generated by onboarding system."""
...
mod tests {
    ...
    fn test_config_pc() -> OnboardConfig {
        OnboardConfig {
            ...
            remote_dir: "~/clawtex-worker".to_string(),
            remote_python: None,
        }
    }
    ...
    fn test_onboard_config_defaults() {
        ...
        assert_eq!(config.remote_dir, "~/clawtex-worker");
    ...
    fn test_generate_mobile_link() {
        ...
        assert!(link.starts_with("clawtex://join?"));
    ...
    async fn test_onboard_mobile_worker() {
        ...
        assert!(result.mobile_link.unwrap().starts_with("clawtex://join?"));
