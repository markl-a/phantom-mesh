//! Web Dashboard — unified dashboard module.
//!
//! Contains both the legacy v1 server-side rendered Kanban board (`render()`)
//! and the v2 single-page JSON API dashboard (`dashboard_routes()`).
//!
//! Routes:
//!   - GET /dashboard?token=...      (v1 — server-rendered Kanban)
//!   - GET /dashboard/v2             (v2 — SPA with JS fetch)
...
// ── Legacy v1 server-rendered Kanban dashboard ──────────────────────────────

/// Render the full v1 dashboard HTML page (server-side rendered Kanban board).
///
/// This is the original dashboard that displays tasks in Pending/Running/Done/Failed
/// columns. Served at `GET /dashboard?token=...`.
pub fn render(
    tasks: &[Task],
    tools: &[String],
    agents: &[String],
...
<title>Phantom Mesh Dashboard</title>
<style>
...
async function loadCluster(){
  try{
    ...
    tbody.innerHTML=nodes.map(n=>`<tr>
      ...
      <td>${n.cpu_load!=null?escapeHtml((n.cpu_load*100).toFixed(1))+'%':'-'}</td>
      <td>${escapeHtml(fmtTime(n.last_seen))}</td>
    </tr>`).join('');
  }catch{}
}
...
async function loadCosts(){
  try{
    ...
    tbody.innerHTML=records.slice(0,50).map(c=>`<tr>
      ...
      <td style="color:#8b949e;font-size:11px">${escapeHtml(c.model)}</td>
      <td>${escapeHtml(c.total_tokens.toLocaleString())}</td>
      <td>$${escapeHtml((c.estimated_cost_usd).toFixed(6))}</td>
    </tr>`).join('');
  }catch{}
}
...
mod tests {
    ...
    fn test_dashboard_html_contains_title() {
        assert!(
            DASHBOARD_HTML.contains("Phantom Mesh Dashboard"),
    ...
    // ── Legacy v1 Kanban dashboard tests ────────────────────────────────────────

    fn make_task(id: &str, title: &str, status: crate::task_queue::TaskStatus, result: Option<&str>) -> crate::task_queue::Task {
        crate::task_queue::Task {
            ...
            strategy_used: None,
            feedback_score: None,
            priority: crate::task_queue::TaskPriority::Normal,
    ...
    fn test_v1_render_empty_tasks() {
        ...
        let body = html.0;
        assert!(body.contains("Phantom Mesh Dashboard"));
        assert!(body.contains("0 tasks"));
        assert!(body.contains("No tasks"));
    }

    #[test]
    fn test_v1_render_with_tasks_in_all_columns() {
        let tasks = vec![
    ...
    #[test]
    fn test_v1_render_ollama_connected_green_dot() {
    ...
    #[test]
    fn test_v1_render_ollama_disconnected_red_dot() {
    ...
    #[test]
    fn test_v1_render_uptime_hours() {
    ...
    #[test]
    fn test_v1_render_uptime_minutes_only() {
    ...
    #[test]
    fn test_v1_render_refresh_link_with_token() {
    ...
    #[test]
    fn test_v1_format_uptime_zero() {
        assert_eq!(format_uptime(0), "0m");
    }

    #[test]
    fn test_v1_format_uptime_hours_and_minutes() {
        assert_eq!(format_uptime(3661), "1h 1m");
    }

    #[test]
    fn test_v1_format_uptime_exact_hour() {
        assert_eq!(format_uptime(7200), "2h 0m");
    }

    #[test]
    fn test_v1_html_escape_special_chars() {
    ...
    #[test]
    fn test_v1_html_escape_no_change() {
        assert_eq!(html_escape("hello world"), "hello world");
    }

    #[test]
    fn test_v1_render_cards_empty() {
        let cards = render_cards(&[]);
        assert!(cards.contains("No tasks"));
    }

    #[test]
    fn test_v1_render_cards_with_task() {
    ...
    #[test]
    fn test_v1_render_cards_html_escapes_title() {
    ...
    #[test]
    fn test_v1_render_cards_long_result_truncated() {
