...
BINARY="$PROJECT_DIR/target/release/clawtex-core"

echo "=== Clawtex Cluster Start ==="
...
if [ ! -f "$BINARY" ] || [ "$PROJECT_DIR/src/main.rs" -nt "$BINARY" ]; then
    echo ""
    echo "[1/3] Building clawtex-core (release)..."
    cd "$PROJECT_DIR"
    cargo build --release
else
    echo ""
...
cd "$PROJECT_DIR"
nohup "$BINARY" daemon --host "$HUB_HOST" --port "$HUB_PORT" > /tmp/clawtex-hub.log 2>&1 &
HUB_PID=$!
echo "Hub PID: $HUB_PID"
echo "$HUB_PID" > /tmp/clawtex-hub.pid
...
for i in $(seq 1 15); do
    ...
    sleep 1
    if [ "$i" -eq 15 ]; then
        echo "WARNING: Hub may not be ready yet. Check /tmp/clawtex-hub.log"
    fi
done
