//! Example: experimental-hermes-memory (H3, FTS5 SQLite backend).
//!
...
//!
//! Expected last line: `experimental-hermes-memory OK`. Exit code 0.

use phantom_mesh::hermes::{escape_fts5_query, HermesMemory, NewMemory};

#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    // Use a per-process temp file so concurrent runs don't collide.
    let pid = std::process::id();
    let db_path = std::env::temp_dir().join(format!("hermes_memory_example_{pid}.db"));
    let _ = std::fs::remove_file(&db_path); // fresh start
    let mem = HermesMemory::open_at(db_path.clone())?;
    ...
    println!("experimental-hermes-memory OK");
    Ok(())
}
