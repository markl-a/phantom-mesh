...
//!     --example openclaw_telegram_dispatch `
...
fn main() {
    eprintln!(
        "openclaw_telegram_dispatch example requires --features experimental-openclaw-telegram"
    );
    std::process::exit(2);
}
...
async fn main() -> anyhow::Result<()> {
    ...
    use phantom_mesh::openclaw::telegram::{
        run_round_trip, OpenclawTelegramBot, OpenclawTelegramConfig,
    };
    use phantom_mesh::openclaw::telegram_agent_dispatcher::PhantomAgentDispatcher;
    ...
    let bot = Arc::new(OpenclawTelegramBot::new(cfg, dispatcher));

    tracing::info!(
        agent = %agent_name,
        "openclaw_telegram_dispatch: starting long-poll loop. Ctrl-C to quit."
    );
    run_round_trip(bot)
        .await
