//! Example: experimental-hermes-tools (H5).
//!
//! Loads the full Hermes catalog, asserts there are 30 tools with
//! unique names + well-formed schemas, and exercises 3 tools:
//!   (1) hermes_calculator: "2 + 3 * 4" -> 14
//!   (2) hermes_text_stats: word/line/char counts
//!   (3) hermes_uuid_gen:   produces a v4 UUID
//!
//! Run:
...
//!
//! Expected last line: `experimental-hermes-tools OK`. Exit code 0.
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let calc = cat
        .iter()
        .find(|t| t.name() == "hermes_calculator")
        .unwrap();
    let r = calc.call(&json!({"expression": "2 + 3 * 4"})).await?;
    assert_eq!(r["result"], 14.0);
    println!("[2] hermes_calculator: '2 + 3 * 4' = {}", r["result"]);

    // (b) Text stats
    let stats = cat
        .iter()
        .find(|t| t.name() == "hermes_text_stats")
        .unwrap();
    let r = stats
        .call(&json!({"text": "one two\nthree four five"}))
        .await?;
    println!(
        "[3] hermes_text_stats: words={} lines={} chars={}",
        r["words"], r["lines"], r["chars"]
    );

    // (c) UUID gen
    let uuid = cat.iter().find(|t| t.name() == "hermes_uuid_gen").unwrap();
    let r = uuid.call(&json!({})).await?;
    let s = r["uuid"].as_str().unwrap();
    assert_eq!(s.len(), 36, "v4 uuid must be 36 chars including hyphens");
    println!("[4] hermes_uuid_gen: {s}");

    println!("experimental-hermes-tools OK");
    Ok(())
}
