//! bench_hermes_tool_catalog_lookup
//!
...
#[cfg(feature = "experimental-hermes-tools")]
fn find_tool(cat: &[Box<dyn HermesTool>], name: &str) -> Option<usize> {
...
fn bench_lookup(c: &mut Criterion) {
    ...
    let mut g = c.benchmark_group("hermes_tool_lookup");

    // Best case: first tool in the Vec.
    g.bench_function("first", |b| {
        b.iter(|| {
            let r = find_tool(black_box(&cat), black_box("hermes_calculator"));
            black_box(r)
        });
    });
    ...
    g.bench_function("last", |b| {
        b.iter(|| {
            let r = find_tool(black_box(&cat), black_box("hermes_uuid_gen"));
            black_box(r)
        });
    });
