//! bench_skill_tool_dispatch
//!
//! Goal: measure the *end-to-end* dispatch cost of "find tool by name,
//! then call it with a small input". Chosen tool is `skill_calculator`
//! because:
...
//!
//! Contrast with `bench_skill_tool_catalog_lookup`: that one only times
...
#[cfg(feature = "experimental-tools")]
fn find_tool<'a>(
    cat: &'a [Box<dyn SkillTool>],
    name: &str,
) -> Option<&'a (dyn SkillTool + 'static)> {
...
fn bench_dispatch(c: &mut Criterion) {
    ...
    let warm = rt.block_on(async {
        let t = find_tool(&cat, "skill_calculator").expect("calculator present");
    ...
    let mut g = c.benchmark_group("skill_tool_dispatch");

    g.bench_function("calculator_simple_expr", |b| {
        b.to_async(&rt).iter(|| async {
            let t = find_tool(black_box(&cat), black_box("skill_calculator"))
