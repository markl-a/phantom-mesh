...
phantom-mesh 是一套用 Rust 寫成的跨平台 AI agent（智能代理）mesh（網狀網路）。一個
...
設定檔位於 `~/.phantom-mesh/agents.toml`（完整參考請見 `agents.toml.example`）。
...
`~/.phantom-mesh/conversations/<session_id>.jsonl` 之下。該 store（儲存器）對 `JobStore`
...
- `bin/phantom.rs` 中的 `phantom init` 子命令是一個 TODO stub（待辦樁）— 必須先把 `scaffold.rs` 接上才會運作。
...
- 工具變更時，提交前先透過 REPL（`cargo run --bin phantom-mesh`）手動操作該工具。
- Provider 變更時，使用一次性提示（one-shot prompt）：在設定好目標 provider 的情況下執行 `cargo run --bin phantom-mesh "hello"`。
