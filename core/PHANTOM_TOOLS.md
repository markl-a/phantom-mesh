...
本文件是 phantom-mesh 核心內部執行的每個代理（agent）所有可用工具的權威參考。撰寫方式力求讓另一個 AI 代理能準確理解每個工具的功能、可接受的參數，以及如何有效地串接（chain）多個工具。
...
| `git_push` | git | 將提交推送（push）到遠端（需要 PHANTOM_AUTO_APPROVE=1） |
...
| `rename_file` | fs | 重新命名／移動檔案（需要 PHANTOM_AUTO_APPROVE=1） |
...
執行一個 shell 命令並回傳 stdout、stderr 與結束碼。支援複合命令（`&&`、`||`、`;`）。某些具破壞性的樣式需要 `PHANTOM_AUTO_APPROVE=1`。
...
- 需要 `PHANTOM_AUTO_APPROVE=1` 的樣式：`rm `、`sudo `、`kill `、`pkill `、`git reset --hard`、`git clean `、`chmod `、`chown `、`DROP TABLE`、`curl `、`wget `、`nc `，以及把 `mv`／`cp` 指向絕對路徑。
...
將一組鍵值字串對持久化儲存到 `~/.phantom-mesh/memory.json`。支援帶命名空間的鍵。
...
將提交推送到遠端儲存庫。**需要 `PHANTOM_AUTO_APPROVE=1`。**
...
重設工作樹。Hard 重設**需要 `PHANTOM_AUTO_APPROVE=1`**。
...
重新命名或移動檔案。**需要 `PHANTOM_AUTO_APPROVE=1`。**
...
| `session` | string | 否 | `"default"` | 工作階段名稱（任務按工作階段儲存於 `~/.phantom-mesh/tasks/<session>.json`）。 |
...
- 當你必須以 `shell` 執行破壞性命令時，在環境中設定 `PHANTOM_AUTO_APPROVE=1`，否則該命令會回傳 `APPROVAL_REQUIRED`。
