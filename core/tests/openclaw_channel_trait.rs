//! Integration test for the OpenClaw `Channel` trait.
//!
//! Compiled only when `--features experimental-openclaw` is enabled. The whole
...
// ── Integration test: drive the existing TelegramBot through openclaw::Channel ─
//
...
// server. When O1's `openclaw::telegram::Bot` lands, this adapter can be
...
mod telegram_through_trait {
    ...
    /// the openclaw `Channel` trait. Replace with `openclaw::telegram::Bot`
    ...
    async fn telegram_adapter_sends_through_trait() {
        ...
        adapter
            .send_message(42, "hello from openclaw trait test")
            .await
