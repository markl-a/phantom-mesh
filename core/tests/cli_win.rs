...
// is platform-agnostic in `core/src/bin/phantom.rs`; only the OS-isms differ:
//
...
//     `~/.phantom-mesh/identity.key` is present, and only runs on a key-less
...
/// WIN D22 — `phantom <sub> --help` must print usage and exit 0 WITHOUT
/// launching the subcommand (mirrors `help_flag_never_executes_subcommand_linux`).
/// The footgun guard in `phantom.rs` (the `serve`/`mcp`/`onboarding`/… intercept
...
/// hang, init would write PHANTOM.md into the cwd, etc. Each invocation runs in
/// an isolated cwd (the one side-effect we CAN redirect on Windows) and must
/// terminate within a hard timeout, emit "usage", and leave no PHANTOM.md.
#[tokio::test(flavor = "current_thread")]
...
/// BEFORE any home access in `phantom.rs`, so HOME isolation is only defensive.
#[test]
...
/// WIN D24 — a PRESENT-but-corrupt `identity.key` must make `phantom note`
...
/// unit tests).
///
/// Windows wrinkle: `phantom note` resolves its store + identity.key via
...
/// SKIP entirely when a real `~/.phantom-mesh/identity.key` is present (never
...
fn corrupt_identity_key_refuses_plaintext_note_windows() {
    ...
    let Some(home) = dirs::home_dir() else {
        eprintln!("no home dir resolved — skip (cannot locate ~/.phantom-mesh on this host)");
        return;
    };
    ...
    // SAFETY — no PHANTOM_HOME isolation on Windows yet (dirs::home_dir() ignores
    ...
    if key_path.exists() {
        eprintln!(
            "real ~/.phantom-mesh/identity.key present — skip (refusing to touch the operator's \
             private key; no PHANTOM_HOME isolation on Windows yet)"
        );
        return;
    }
    std::fs::create_dir_all(&phantom_dir).expect("create .phantom-mesh");
