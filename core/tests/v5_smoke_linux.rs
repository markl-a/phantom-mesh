...
//! NO-NETWORK / NO-LLM / NO-daemon set of checks that the built `spectyn`
...
//!
//! Uses `env!("CARGO_BIN_EXE_spectyn")` so `cargo test` builds + locates the
...
fn spectyn() -> Command {
    Command::new(env!("CARGO_BIN_EXE_spectyn"))
}
...
fn v5_version_has_linux_provenance() {
    let out = spectyn().arg("--version").output().expect("spawn --version");
    ...
    assert!(s.contains("spectyn"), "missing product name: {s}");
    assert!(s.contains("linux"), "missing linux target triple: {s}");
    // provenance form: `spectyn <ver> (<hash>, linux-…, built …)`
...
/// V5/LIN — `spectyn doctor` runs to completion (exit 0; it's a best-effort
...
/// V5/LIN — headless contract: `spectyn exec` with no prompt arg and a
