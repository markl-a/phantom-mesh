...
//
// scripts/install.sh honours SPECTYN_INSTALL_DRY_RUN=1: it prints a "would
...
// asserts (a) exit 0, (b) the preview wording is present, and (c) no spectyn
...
// Recursively check that no regular file named "spectyn" exists under `dir`.
fn has_spectyn_binary(dir: &std::path::Path) -> bool {
    ...
    for entry in rd.flatten() {
        let p = entry.path();
        if p.is_dir() {
            if has_spectyn_binary(&p) {
                return true;
            }
        } else if p.file_name().and_then(|n| n.to_str()) == Some("spectyn") {
            return true;
        }
    }
    false
...
fn install_sh_dry_run_previews_without_writing() {
    ...
    assert!(
        !has_spectyn_binary(&home),
        "dry-run must not write a spectyn binary, but one appeared under {home:?}"
    );
