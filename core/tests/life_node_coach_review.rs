//! Integration smoke for E003 `spectyn coach review`.
//!
//! Seeds a fake `~/.spectyn-mesh/events/` with one event's meta.json +
...
//!
//! Skips if the `spectyn` binary isn't built — `cargo test --test ...`
//! does NOT build the BIN target, so dev boxes need `cargo build --bin
//! spectyn` first; CI builds in advance.
...
fn spectyn_binary() -> Option<PathBuf> {
    ...
    let p = PathBuf::from(env!("CARGO_BIN_EXE_spectyn"));
    if p.exists() {
        Some(p)
    } else {
        None
...
fn coach_review_outputs_markdown_referencing_seeded_event() {
    let Some(bin) = spectyn_binary() else {
        eprintln!(
            "SKIPPED: coach_review_outputs_markdown_referencing_seeded_event — spectyn \
             binary not found (build with `cargo build --bin spectyn`)"
        );
        return;
    };
    ...
    // Run `spectyn coach review --date 2026-05-22` with HOME redirected.
...
fn coach_review_save_flag_writes_reviews_dir() {
    let Some(bin) = spectyn_binary() else {
        eprintln!("SKIPPED: coach_review_save_flag_writes_reviews_dir — spectyn binary not found");
        return;
    };
    let tmp = tempfile::tempdir().expect("tempdir");
    let home = tmp.path();
    ...
    fs::create_dir_all(home.join(".spectyn-mesh/events")).unwrap();
    ...
    let saved = home.join(".spectyn-mesh/reviews/2026-05-22.md");
...
fn coach_review_save_encrypts_when_identity_key_present() {
    let Some(bin) = spectyn_binary() else {
        eprintln!(
            "SKIPPED: coach_review_save_encrypts_when_identity_key_present — spectyn \
             binary not found"
        );
        return;
    };
    ...
    fs::write(home.join(".spectyn-mesh/identity.key"), &identity_bytes).unwrap();
    ...
    assert!(
        spectyn_mesh::life_node::crypto::looks_like_age(&bytes),
