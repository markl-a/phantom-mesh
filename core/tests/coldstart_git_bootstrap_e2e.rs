//! E2E proof for `phantom memory bootstrap` (MAIN-REPO P1.4 cold-start).
//!
//! This is the anti-fake-green proof the feature actually ships: it drives the
//! REAL `phantom` CLI binary (`env!("CARGO_BIN_EXE_phantom")` — Cargo builds it
//! before running this test) end to end:
//!
//!   temp git repo (known commits) → `phantom memory bootstrap --repo <tmprepo>`
//!     → `phantom recall <kw>` returns the matching commit
//!     → re-run bootstrap is idempotent (no duplication)
//!
//! It is NOT a library-only unit test: every step goes through the compiled
//! `phantom` binary over a subprocess, with `PHANTOM_HOME` pointed at a throwaway
//! data root so the REAL `~/.phantom-mesh` store is never touched. The store the
//! bootstrap writes (`<PHANTOM_HOME>/events.sqlite` + `<PHANTOM_HOME>/events/`) is
//! the SAME store `phantom recall` reads — that round-trip is the whole point.

use std::path::Path;
use std::process::Command;

/// The compiled `phantom` binary under test. Cargo sets `CARGO_BIN_EXE_phantom`
...
/// Run `phantom <args>` with `PHANTOM_HOME` pointed at `data_root`. Returns
/// (success, stdout, stderr).
fn phantom(data_root: &Path, args: &[&str]) -> (bool, String, String) {
    let out = Command::new(phantom_bin())
        .args(args)
        .env("PHANTOM_HOME", data_root)
        .output()
        .unwrap_or_else(|e| panic!("failed to spawn phantom {args:?}: {e}"));
    (
        out.status.success(),
...
/// Parse `phantom recall <q> --json` stdout into the hit array. The `--json`
...
fn recall_json(data_root: &Path, query: &str) -> Vec<serde_json::Value> {
    ...
    let (ok, stdout, stderr) =
        phantom(data_root, &["recall", query, "--mode", "keyword", "--json"]);
    assert!(ok, "phantom recall {query} failed: stderr=\n{stderr}");
...
fn bootstrap_git_commits_then_recall_returns_them_and_is_idempotent() {
    ...
    assert!(
        Path::new(bin).exists(),
        "phantom binary missing at {bin} — `cargo test` should have built it",
    );

    let tmp = tempfile::tempdir().expect("tempdir");
    let repo = tmp.path().join("repo");
    // PHANTOM_HOME is the data root used VERBATIM (cli_config::phantom_data_dir),
    // so this isolates BOTH events.sqlite and events/ — the real ~/.phantom-mesh
    // is never written.
    ...
    let (ok, stdout, stderr) =
        phantom(&data_root, &["memory", "bootstrap", "--repo", repo_str]);
