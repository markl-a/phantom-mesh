...
async fn one_real_satellite_task_end_to_end() {
    let q = spectyn_mesh::fleet::queue::FleetQueue::open_in_memory().unwrap();
    let cfg = spectyn_mesh::fleet::FleetConfig::load().unwrap();
    let n = spectyn_mesh::fleet::ingest_all(&cfg, &q).await.unwrap();
    println!("ingested {n} tasks from fleet.toml");
    let n2 = spectyn_mesh::fleet::ingest_all(&cfg, &q).await.unwrap();
    assert_eq!(
        n2, 0,
