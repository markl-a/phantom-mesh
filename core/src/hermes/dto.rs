//! F400 — Flat JSON DTOs for the Hermes skill RPC endpoints.
//!
...
//!
//! Gated behind `experimental-hermes-memory` so the default build doesn't
...
use crate::hermes::memory::MemoryRow;
...
// `phantom hermes status`; this `SkillSummary` is a *per-row* shape used by
...
// (likely `hermes::dto::SkillSummary` → `SkillListEntry`) once SPEC-25 §9.7
...
///                        life of the DB; pass back to `/api/hermes/skills/:id`.
/// * `created_at`       — unix seconds (UTC) when the row was written.
/// * `source`           — `hermes_skills` (seed) | `hermes_auto_register`
...
//   • `hermes::dto::SkillSummary`  — THIS one: full skill record for the
//     `/hermes/skills` HTTP paginated list (9 fields incl. body Markdown).
//   • `skill_wire::SkillSummary`   — dashboard overview (4 fields: counts +
//     top-3 names) for `phantom hermes status` card.
...
mod tests {
    ...
    fn summary_from_row_parses_skill_row() {
        ...
        assert_eq!(s.source, "hermes_skills");
