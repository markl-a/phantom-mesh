//! `hermes_regex_extract` — extract regex matches from text.
//!
//! Concept ported from hermes-agent's regex tool. Uses the existing
//! `regex` dep (no extra Cargo cost).
...
#[async_trait]
impl HermesTool for RegexExtract {
    fn name(&self) -> &'static str {
        "hermes_regex_extract"
    }
