//! Hermes runtime integration façade.
//!
//! Wires the four weekend-shipped Hermes submodules (curator, skill, memory,
...
//!
//! Gated behind the umbrella feature `experimental-hermes`. The four
...
use crate::hermes::curator::{NoopSkillExtractor, SkillExtractor};
use crate::hermes::memory::HermesMemory;
use crate::hermes::skill::{parse_str as parse_skill, serialize as serialize_skill, SkillDocument};
use crate::hermes::tools::{catalog, HermesTool};

/// Façade that owns the four Hermes subsystems and the wiring between them.
///
/// Construct once per process via [`HermesRuntime::new`]; clone (cheap — all
/// owned state is `Arc`-backed inside `HermesMemory`) into worker tasks as
/// needed. The runtime is `Send + Sync` by virtue of its members.
pub struct HermesRuntime {
    memory: HermesMemory,
    #[allow(dead_code)]
    tools: Vec<Box<dyn HermesTool>>,
    curator: Option<crate::hermes::curator::Curator>,
...
impl HermesRuntime {
    ...
    pub async fn new(db_path: PathBuf) -> Result<Self> {
        use crate::hermes::memory::NewMemory;

        let memory = HermesMemory::open_at(db_path)?;
        let tools = catalog();
        ...
        for tool in tools.iter() {
            ...
            let probe = crate::hermes::memory::escape_fts5_query(tool.name());
        ...
        const SAMPLE_SKILL_MD: &str = include_str!("../../../docs/hermes-skills/sample-skill.md");
        ...
        let probe = crate::hermes::memory::escape_fts5_query(&skill.frontmatter.name);
    ...
    pub fn set_curator(&mut self, curator: crate::hermes::curator::Curator) {
    ...
    pub async fn judge_and_record(
        ...
        self.memory
            .insert(crate::hermes::memory::NewMemory {
                kind: "judge_verdict",
                source: "hermes_curator",
                text: &text,
                tags: &tags,
            })
            .await?;
    ...
    ) -> Result<Vec<crate::hermes::memory::MemoryRow>> {
        let trimmed = sha_or_session_id.trim();
        if trimmed.is_empty() {
            return Ok(Vec::new());
        }
        let escaped = crate::hermes::memory::escape_fts5_query(trimmed);
    ...
    pub async fn find_tool_by_intent(&self, query: &str) -> Result<Vec<String>> {
        ...
        let escaped = trimmed
            .split_whitespace()
            .map(crate::hermes::memory::escape_fts5_query)
            .collect::<Vec<_>>()
            .join(" OR ");
        let rows = self.memory.search(&escaped, 25).await?;
        Ok(rows
    ...
    #[cfg(test)]
    pub(crate) fn memory_for_test(&self) -> &HermesMemory {
        &self.memory
    }
    ...
    /// [`HermesRuntime::find_tool_by_intent`]). Each returned string is
...
/// Per-row character cap applied by [`HermesRuntime::recall_context_for`] so
...
mod tests {
    ...
    use crate::hermes::curator::{Curator, DEFAULT_JUDGE_MODEL};
    ...
    async fn new_opens_db_at_given_path() {
        ...
        let rt = HermesRuntime::new(db_path.clone()).await.expect("new ok");
    ...
    async fn new_seeds_memory_with_each_tool_in_catalog() {
        let td = tempfile::tempdir().unwrap();
        let rt = HermesRuntime::new(td.path().join("h.db")).await.unwrap();
        // Every tool in the catalog should appear as one memory row
        // with kind=tool. Catalog size tracks crate::hermes::tools::catalog()
        ...
        let calc = rt
            .memory_for_test()
            .search(
                &crate::hermes::memory::escape_fts5_query("hermes_calculator"),
                5,
            )
            .await
            .unwrap();
        assert!(!calc.is_empty(), "calculator tool not stored");
        assert!(calc[0].text.contains("hermes_calculator"));
    }
    ...
    async fn reopening_same_db_does_not_duplicate_skill_rows() {
        ...
        let _ = HermesRuntime::new(p.clone()).await.unwrap();
        let r2 = HermesRuntime::new(p).await.unwrap();
    ...
    async fn judge_and_record_writes_verdict_into_checkpoint_and_memory() {
        ...
        let td = tempfile::tempdir().unwrap();
        let mut rt = HermesRuntime::new(td.path().join("h.db")).await.unwrap();
        ...
        let hits = rt
            .memory_for_test()
            .search(&crate::hermes::memory::escape_fts5_query(&c.session_id), 10)
            .await
            .unwrap();
    ...
    async fn end_to_end_session_loop_persists_then_recalls() {
        ...
        // === Session 1 ===
        let mut rt1 = HermesRuntime::new(db.clone()).await.unwrap();
        ...
        let rt2 = HermesRuntime::new(db).await.unwrap();
    ...
    mod auto_register {
        use super::*;
        use crate::hermes::curator::SkillExtractor;
        use crate::hermes::skill::{SkillDocument, SkillFrontmatter};
        ...
        async fn make_runtime_with_score(
            score: i64,
        ) -> (HermesRuntime, wiremock::MockServer, tempfile::TempDir) {
            let server = MockServer::start().await;
            Mock::given(method("POST"))
                .and(path("/v1/messages"))
                .respond_with(
    ...
    async fn seed_needle_rows(rt: &HermesRuntime, n: usize) {
        for i in 0..n {
    ...
    async fn reopening_same_db_does_not_duplicate_tool_rows() {
        let td = tempfile::tempdir().unwrap();
        let p = td.path().join("h.db");
        let _r1 = HermesRuntime::new(p.clone()).await.unwrap();
        drop(_r1);
        let r2 = HermesRuntime::new(p).await.unwrap();
