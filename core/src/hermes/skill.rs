//! Hermes Skill Document parser.
//!
//! A Skill Document is a Markdown file beginning with a `---`-fenced YAML
//! frontmatter block (matching `docs/hermes-skill-schema.json`) followed by
...
//!
//! Gated behind the `experimental-hermes-curator` cargo feature — the default
...
/// Structured frontmatter — mirrors `docs/hermes-skill-schema.json`.
///
...
";
    ...
    fn round_trip_sample_skill_from_docs() {
        // Reads docs/hermes-skills/sample-skill.md from the repo root.
        // CARGO_MANIFEST_DIR points to core/, so we go one level up.
        let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()
            .expect("workspace root")
            .join("docs/hermes-skills/sample-skill.md");
    ...
    fn sample_skill_validates_against_json_schema() {
        ...
        let schema_path = workspace.join("docs/hermes-skill-schema.json");
        ...
        let sample_path = workspace.join("docs/hermes-skills/sample-skill.md");
        ...
        if let Some(msgs) = msgs {
            panic!(
                "sample-skill.md frontmatter does NOT validate against docs/hermes-skill-schema.json:\n{}",
                msgs.join("\n")
            );
        }
    }
