//! B9 / T90 — per-channel token-bucket rate limiter for OpenClaw's
...
//! # Feature gating
//!
//! Compiled only with `--features experimental-openclaw`. Default
...
impl PerChannelLimiter {
    ...
    /// Build a limiter pre-populated with the OpenClaw production defaults
    /// (Telegram / Slack / WhatsApp). `default_rate` still applies to any
    /// channel name not listed here.
    pub fn with_openclaw_defaults(default_rate: u32) -> Self {
...
impl Default for PerChannelLimiter {
    ...
    /// should prefer [`PerChannelLimiter::with_openclaw_defaults`] or call
...
mod tests {
    ...
    #[test]
    fn limiter_with_openclaw_defaults_seeds_known_channels() {
        let lim = PerChannelLimiter::with_openclaw_defaults(50);
