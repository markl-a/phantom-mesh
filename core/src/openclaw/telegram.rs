...
//! [`Dispatcher`] (the agent / runtime / Hermes loop) and the textual
...
//!
//! See `docs/superpowers/BIG-GOAL.md` §P3 and
//! `docs/superpowers/specs/_current/E004-openclaw-remote-control.md` for
//! the locked framing, and
//! `docs/superpowers/specs/2026-05-15-weekend-multi-agent-push-design.md`
//! §4 row [O1] for this module's original track.
//!
//! Build with:
//!   cargo build --features experimental-openclaw-telegram
//!
//! ## Architecture
//!
//! `OpenclawTelegramBot` wraps a `teloxide::Bot` plus an injected
...
//! openclaw_telegram_echo.rs` or, eventually, `main.rs`) constructs
...
///
/// Named `OpenclawDispatcher` (not just `Dispatcher`) so it cannot
...
/// Configuration for the OpenClaw Telegram bot.
///
...
impl std::fmt::Debug for OpenclawTelegramConfig {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("OpenclawTelegramConfig")
...
pub struct OpenclawTelegramBot {
    config: OpenclawTelegramConfig,
    dispatcher: Arc<dyn OpenclawDispatcher>,
}

impl OpenclawTelegramBot {
    pub fn new(config: OpenclawTelegramConfig, dispatcher: Arc<dyn OpenclawDispatcher>) -> Self {
        Self { config, dispatcher }
    }

    pub fn config(&self) -> &OpenclawTelegramConfig {
        &self.config
    }
    ...
    pub async fn handle_text(&self, user_id: i64, chat_id: i64, text: String) -> Option<String> {
        if !self.config.is_user_allowed(user_id) {
            tracing::warn!(
                user_id,
                "openclaw-tg: rejecting message from non-allowlisted user"
            );
            return None;
        }
        match self.dispatcher.dispatch_with_chat(chat_id, text).await {
            ...
            Err(e) => {
                ...
                tracing::error!(error = %e, "openclaw-tg: dispatcher returned error");
...
pub async fn run_round_trip(bot: Arc<OpenclawTelegramBot>) -> Result<(), String> {
    ...
    match tg.get_me().await {
        Ok(me) => {
            tracing::info!(
                ...
                "openclaw-tg: bot identity verified"
            );
        }
        Err(e) => {
            ...
            if let Some(reply) = bot_ref.handle_text(user_id, chat_id.0, text).await {
                ...
                for chunk in chunk_message(&reply, 4000) {
                    if let Err(e) = tg.send_message(chat_id, chunk).await {
                        tracing::warn!(error = %e, "openclaw-tg: send_message failed");
                    }
                }
            }
            Ok(())
...
mod tests {
    ...
    async fn handle_text_translates_dispatcher_error_to_generic_reply() {
        ...
        #[async_trait]
        impl OpenclawDispatcher for FailingDispatcher {
...
/// Helpers used by `phantom serve --openclaw-telegram` to read the bot token
/// from the operator's environment and build an [`OpenclawTelegramConfig`].
///
...
pub mod cli {
    use super::OpenclawTelegramConfig;
    ...
    /// Build an [`OpenclawTelegramConfig`] from operator-controlled inputs.
    ///
    ...
    pub fn resolve_config<F>(
        ...
        mut lookup: F,
    ) -> Result<OpenclawTelegramConfig, CliError>
    where
