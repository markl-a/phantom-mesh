//! OpenClaw bot persona — TOML schema + per-channel binding.
//!
...
//!   non-empty". This matches Hermes-style tool catalog semantics so the V3
...
//! # Feature gating
//!
//! Compiled only with `--features experimental-openclaw`. Default `cargo
...
///
/// Semantics (matches Hermes tool catalog gating, see V3 prep notes in module
/// docs):
///
...
name = "  ""#;
    ...
    fn shipped_example_files_parse() {
        // CARGO_MANIFEST_DIR points at .../core, so docs/openclaw-personas
        // sits one level up.
        let root = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()
            .expect("core has parent");
        for name in ["example.toml", "assistant-friendly.toml"] {
            let path = root.join("docs").join("openclaw-personas").join(name);
