...
//!     (`openclaw::webhook_auth::validate_telegram_secret_token`, B3/T84).
//!   * Slack    → HMAC-SHA256 over `v0:<ts>:<body>` plus a ±5-min replay
//!     window (`openclaw::slack::verify_slack_signature`, B5/T86).
...
//! ## Feature gating
//!
//! Compiled under any of the OpenClaw sub-features so umbrella +
//! channel-only builds all see the trait. Default `cargo build` (no
//! flags) still compiles zero new code (mirrors the
//! `experimental-openclaw` gating convention used by every other module
//! in this directory).
...
mod tests {
    ...
    fn slack_signed_headers(secret: &str, ts: u64, body: &[u8]) -> HeaderMap {
        use crate::openclaw::slack::compute_slack_signature;
