//! OpenClaw — **Remote Control for your cluster** (BIG-GOAL pillar P3).
//!
...
//! the epic that this module serves.
//!
...
//! behind `experimental-openclaw-telegram`. When the feature is enabled the
//! `openclaw::telegram` submodule pulls in `teloxide` and exposes a
//! round-trip bot loop plus a `cli` helper used by `phantom serve
//! --openclaw-telegram` (B2/T83).
...
#[cfg(feature = "experimental-openclaw-whatsapp")]
pub mod whatsapp;

#[cfg(feature = "experimental-openclaw-slack")]
pub mod slack;

// T54 — bot persona TOML schema + per-channel binding (v0.6.0 V3 prep).
// Compiled whenever any openclaw channel subfeature is on (the umbrella
...
// `docs/openclaw-personas/example.toml` for the canonical schema.
#[cfg(any(
...
// Gated under the umbrella `experimental-openclaw` flag so default `cargo
...
#[cfg(feature = "experimental-openclaw")]
pub mod dispatcher;
...
// without channel-specific branching. Compiled under any of the OpenClaw
