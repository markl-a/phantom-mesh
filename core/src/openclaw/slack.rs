...
//! both live elsewhere; see [`crate::openclaw`] module docs for the
...
//! ## Feature gating
//!
//! Built only with `--features experimental-openclaw-slack`. Default `cargo
...
// IDs. The `experimental_openclaw_example` binary still imports `SlackStub`
...
// Slack API). Keep the type alive so the umbrella `experimental-openclaw`
...
impl SlackStub {
    ...
    /// persona via [`SlackStub::dispatcher`] — see `openclaw::dispatch`
