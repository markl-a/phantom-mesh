...
//!
//! Track **B7 / T88** (2026-05-16). Promotes OpenClaw from "one bot per
...
//! # Back-compat
//!
//! The single-bot constructor [`OpenclawDispatcher::single`] builds a
...
//! a single line. `OpenclawTelegramBot::handle_text(user_id, text)` from
...
//! # Concurrency
//!
//! [`OpenclawDispatcher`] is `Send + Sync` and cheap to clone — the
...
#[derive(Clone)]
pub struct OpenclawDispatcher {
    channels: Arc<RwLock<HashMap<String, Arc<dyn Channel>>>>,
}

impl OpenclawDispatcher {
...
impl Default for OpenclawDispatcher {
    fn default() -> Self {
        Self::new()
    }
}
...
#[cfg(test)]
mod multi_bot {
    //! Multi-bot routing tests. Path: `openclaw::dispatcher::multi_bot`.
    //!
    //! Run: cargo test --features experimental-openclaw \
    //!         openclaw::dispatcher::multi_bot
    ...
    async fn two_bots_receive_only_their_own_messages() {
        ...
        let dispatcher = OpenclawDispatcher::new();
        dispatcher
    ...
    async fn single_bot_constructor_round_trips() {
        ...
        let dispatcher = OpenclawDispatcher::single("telegram:only", bot);
    ...
    async fn allowlist_empty_permits_anyone_via_dispatcher() {
        ...
        let dispatcher = OpenclawDispatcher::single("telegram:open", bot.clone());
    ...
    /// `phantom openclaw list` command for stable diff-able output.
    #[tokio::test]
