...
//!
//! Usage:
//!   `phantom-mesh install-worker --hub http://100.x.x.x:7878 --name my-worker --port 7879`
...
fn default_install_dir() -> String {
    "/opt/phantom-mesh-worker".to_string()
}
...
pub fn generate_config_file(config: &InstallerConfig) -> String {
    ...
    out.push_str("# Phantom Mesh Worker Configuration\n");
    out.push_str("# Generated by phantom-mesh installer\n\n");
    out.push_str("[worker]\n");
...
pub fn generate_systemd_unit(config: &InstallerConfig) -> String {
    let binary = config
        .binary_path
        .clone()
        .unwrap_or_else(|| format!("{}/phantom-mesh", config.install_dir));
    ...
    unit.push_str(&format!(
        "Environment=PHANTOM_MESH_CONFIG={}/worker.toml\n",
        config.install_dir
    ));

    if let Some(ref token) = config.auth_token {
        unit.push_str(&format!("Environment=PHANTOM_MESH_AUTH_TOKEN={}\n", token));
    }
...
pub fn generate_windows_service(config: &InstallerConfig) -> String {
    let binary = config
        .binary_path
        .clone()
        .unwrap_or_else(|| format!("{}\\phantom-mesh.exe", config.install_dir));

    let mut script = String::new();
    script.push_str("# Phantom Mesh Worker — Windows Service Install Script\n");
    ...
    script.push_str(&format!(
        "$ServiceName = \"Phantom MeshWorker_{}\"\n",
        config.worker_name
    ));
    script.push_str(&format!(
        "$DisplayName = \"Phantom Mesh Worker ({})\"\n",
        config.worker_name
    ));
    ...
    script.push_str("sc.exe description $ServiceName \"Phantom Mesh distributed worker node\"\n");
...
pub fn generate_launchd_plist(config: &InstallerConfig) -> String {
    ...
    let label = format!("com.phantom-mesh.worker.{}", config.worker_name);
    ...
    plist.push_str(&format!(
        "    <string>{}/phantom-mesh-worker.log</string>\n",
        config.install_dir
    ));

    plist.push_str("    <key>StandardErrorPath</key>\n");
    plist.push_str(&format!(
        "    <string>{}/phantom-mesh-worker.err</string>\n",
        config.install_dir
    ));
    ...
    if let Some(ref token) = config.auth_token {
        ...
        plist.push_str("    <dict>\n");
        plist.push_str("        <key>PHANTOM_MESH_AUTH_TOKEN</key>\n");
        plist.push_str(&format!("        <string>{}</string>\n", token));
        plist.push_str("    </dict>\n");
    }
...
pub fn generate_install_summary(config: &InstallerConfig) -> InstallSummary {
    ...
    let (service_file_path, instructions) = match platform.as_str() {
        "linux" => {
            let svc_path = format!(
                "/etc/systemd/system/phantom-mesh-worker-{}.service",
                config.worker_name
            );
            let instructions = vec![
                ...
                format!(
                    "4. Run: sudo systemctl enable --now phantom-mesh-worker-{}",
                    config.worker_name
                ),
            ];
        ...
        "macos" => {
            let plist_path = format!(
                "~/Library/LaunchAgents/com.phantom-mesh.worker.{}.plist",
                config.worker_name
            );
            let instructions = vec![
                format!("1. Copy config to {}/worker.toml", config.install_dir),
...
mod tests {
    ...
    fn test_config() -> InstallerConfig {
        InstallerConfig {
            ...
            auth_token: Some("phantom_mesh-hub-2026".to_string()),
            ...
            install_dir: "/opt/phantom-mesh-worker".to_string(),
            binary_path: None,
        }
    }
    ...
    fn test_config_defaults() {
        ...
        assert_eq!(config.install_dir, "/opt/phantom-mesh-worker");
    ...
    fn test_config_full_deserialize() {
        let json = r#"{
            ...
            "device_type": "light",
            "install_dir": "/home/user/phantom_mesh",
            "binary_path": "/usr/local/bin/phantom-mesh"
        }"#;
        let config: InstallerConfig = serde_json::from_str(json).unwrap();
        ...
        assert_eq!(config.binary_path.unwrap(), "/usr/local/bin/phantom-mesh");
    }
    ...
    fn test_generate_config_file_basic() {
        ...
        assert!(toml.contains("auth_token = \"phantom_mesh-hub-2026\""));
    ...
    fn test_generate_systemd_unit() {
        ...
        assert!(unit.contains("PHANTOM_MESH_AUTH_TOKEN=phantom_mesh-hub-2026"));
    }

    #[test]
    fn test_generate_systemd_unit_custom_binary() {
        let mut config = test_config();
        config.binary_path = Some("/usr/local/bin/phantom_mesh".to_string());
        let unit = generate_systemd_unit(&config);
        assert!(unit.contains("ExecStart=/usr/local/bin/phantom_mesh worker"));
    }
    ...
    fn test_generate_systemd_unit_no_token() {
        ...
        config.auth_token = None;
        let unit = generate_systemd_unit(&config);
        assert!(!unit.contains("PHANTOM_MESH_AUTH_TOKEN"));
    }
    ...
    fn test_generate_windows_service_custom_binary() {
        let mut config = test_config();
        config.binary_path = Some("C:\\phantom_mesh\\phantom-mesh.exe".to_string());
        let script = generate_windows_service(&config);
        assert!(script.contains("C:\\phantom_mesh\\phantom-mesh.exe"));
    }
    ...
    fn test_generate_launchd_plist() {
        ...
        assert!(plist.contains(&format!(
            "<string>com.phantom-mesh.worker.{}</string>",
            config.worker_name
        )));
        ...
        assert!(plist.contains("<key>PHANTOM_MESH_AUTH_TOKEN</key>"));
    }
    ...
    fn test_generate_launchd_plist_no_token() {
        ...
        config.auth_token = None;
        let plist = generate_launchd_plist(&config);
        assert!(!plist.contains("PHANTOM_MESH_AUTH_TOKEN"));
