...
//!
//! Usage:
//!   `spectyn-mesh install-worker --hub http://100.x.x.x:7878 --name my-worker --port 7879`
...
pub struct InstallerConfig {
    ...
    pub hub_url: String,
    /// Unique worker name (e.g., "node-b", "mac-worker").
...
fn default_install_dir() -> String {
    "/opt/spectyn-mesh-worker".to_string()
}
...
pub fn validate_config(config: &InstallerConfig) -> Vec<ValidationIssue> {
        ...
        .all(|c| c.is_alphanumeric() || c == '-' || c == '_')
    {
        issues.push(ValidationIssue {
            field: "worker_name".to_string(),
            message: "Worker name must be alphanumeric (hyphens and underscores allowed)"
                .to_string(),
            severity: IssueSeverity::Error,
        });
    }
...
pub fn generate_config_file(config: &InstallerConfig) -> String {
    ...
    out.push_str("# Spectyn Mesh Worker Configuration\n");
    out.push_str("# Generated by spectyn-mesh installer\n\n");
    out.push_str("[worker]\n");
...
pub fn generate_systemd_unit(config: &InstallerConfig) -> String {
    let binary = config
        .binary_path
        .clone()
        .unwrap_or_else(|| format!("{}/spectyn-mesh", config.install_dir));

    let mut unit = String::new();
    unit.push_str("[Unit]\n");
    unit.push_str(&format!(
        "Description=Spectyn Mesh Worker ({})\n",
        config.worker_name
    ));
    ...
    unit.push_str(&format!(
        "Environment=SPECTYN_MESH_CONFIG={}/worker.toml\n",
        config.install_dir
    ));

    if let Some(ref token) = config.auth_token {
        unit.push_str(&format!("Environment=SPECTYN_MESH_AUTH_TOKEN={}\n", token));
    }
...
pub fn generate_windows_service(config: &InstallerConfig) -> String {
    let binary = config
        .binary_path
        .clone()
        .unwrap_or_else(|| format!("{}\\spectyn-mesh.exe", config.install_dir));

    let mut script = String::new();
    script.push_str("# Spectyn Mesh Worker — Windows Service Install Script\n");
    ...
    script.push_str(&format!(
        "$ServiceName = \"Spectyn MeshWorker_{}\"\n",
        config.worker_name
    ));
    script.push_str(&format!(
        "$DisplayName = \"Spectyn Mesh Worker ({})\"\n",
        config.worker_name
    ));
    ...
    script.push_str("sc.exe description $ServiceName \"Spectyn Mesh distributed worker node\"\n");
    script.push_str("sc.exe start $ServiceName\n\n");

    script.push_str("Write-Host \"Service $ServiceName installed and started.\"\n");
    script.push_str("Write-Host \"To check status: sc.exe query $ServiceName\"\n");
    script.push_str("Write-Host \"To stop: sc.exe stop $ServiceName\"\n");
    script.push_str("Write-Host \"To remove: sc.exe delete $ServiceName\"\n");
...
pub fn generate_launchd_plist(config: &InstallerConfig) -> String {
    ...
    let label = format!("com.spectyn-mesh.worker.{}", config.worker_name);
    ...
    plist.push_str(&format!(
        "    <string>{}/spectyn-mesh-worker.log</string>\n",
        config.install_dir
    ));

    plist.push_str("    <key>StandardErrorPath</key>\n");
    plist.push_str(&format!(
        "    <string>{}/spectyn-mesh-worker.err</string>\n",
        config.install_dir
    ));
    ...
    if let Some(ref token) = config.auth_token {
        ...
        plist.push_str("    <dict>\n");
        plist.push_str("        <key>SPECTYN_MESH_AUTH_TOKEN</key>\n");
        plist.push_str(&format!("        <string>{}</string>\n", token));
        plist.push_str("    </dict>\n");
    }
...
pub fn generate_install_summary(config: &InstallerConfig) -> InstallSummary {
    ...
    let (service_file_path, instructions) = match platform.as_str() {
        "linux" => {
            let svc_path = format!(
                "/etc/systemd/system/spectyn-mesh-worker-{}.service",
                config.worker_name
            );
            let instructions = vec![
                format!("1. Copy config to {}/worker.toml", config.install_dir),
                format!("2. Copy service file to {}", svc_path),
                "3. Run: sudo systemctl daemon-reload".to_string(),
                format!(
                    "4. Run: sudo systemctl enable --now spectyn-mesh-worker-{}",
                    config.worker_name
                ),
            ];
        ...
        "macos" => {
            let plist_path = format!(
                "~/Library/LaunchAgents/com.spectyn-mesh.worker.{}.plist",
                config.worker_name
            );
            let instructions = vec![
                format!("1. Copy config to {}/worker.toml", config.install_dir),
...
mod tests {
    ...
    fn test_config() -> InstallerConfig {
        InstallerConfig {
            ...
            auth_token: Some("spectyn_mesh-hub-2026".to_string()),
            ...
            install_dir: "/opt/spectyn-mesh-worker".to_string(),
            binary_path: None,
        }
    }
    ...
    fn test_config_defaults() {
        ...
        assert_eq!(config.install_dir, "/opt/spectyn-mesh-worker");
    ...
    fn test_config_full_deserialize() {
        let json = r#"{
            ...
            "device_type": "light",
            "install_dir": "/home/user/spectyn_mesh",
            "binary_path": "/usr/local/bin/spectyn-mesh"
        }"#;
        let config: InstallerConfig = serde_json::from_str(json).unwrap();
        assert_eq!(config.hub_url, "http://10.0.2.1:7878");
        assert_eq!(config.worker_name, "node-b");
        ...
        assert_eq!(config.binary_path.unwrap(), "/usr/local/bin/spectyn-mesh");
    }
    ...
    fn test_validate_empty_hub_url() {
        ...
        assert!(issues
            .iter()
            .any(|i| i.field == "hub_url" && i.severity == IssueSeverity::Error));
    }
    ...
    fn test_validate_valid_worker_name_with_hyphens() {
        let mut config = test_config();
        config.worker_name = "mac-worker_2".to_string();
        let issues = validate_config(&config);
    ...
    fn test_validate_known_device_types() {
        for dt in &["full", "light", "mobile", "npu"] {
            ...
            let issues = validate_config(&config);
            assert!(
                !issues.iter().any(|i| i.field == "device_type"),
    ...
    fn test_generate_config_file_basic() {
        ...
        assert!(toml.contains("auth_token = \"spectyn_mesh-hub-2026\""));
    ...
    fn test_generate_systemd_unit() {
        ...
        assert!(unit.contains("SPECTYN_MESH_AUTH_TOKEN=spectyn_mesh-hub-2026"));
    }

    #[test]
    fn test_generate_systemd_unit_custom_binary() {
        let mut config = test_config();
        config.binary_path = Some("/usr/local/bin/spectyn_mesh".to_string());
        let unit = generate_systemd_unit(&config);
        assert!(unit.contains("ExecStart=/usr/local/bin/spectyn_mesh worker"));
    }
    ...
    fn test_generate_systemd_unit_no_token() {
        ...
        config.auth_token = None;
        let unit = generate_systemd_unit(&config);
        assert!(!unit.contains("SPECTYN_MESH_AUTH_TOKEN"));
    }
    ...
    fn test_generate_windows_service_custom_binary() {
        let mut config = test_config();
        config.binary_path = Some("C:\\spectyn_mesh\\spectyn-mesh.exe".to_string());
        let script = generate_windows_service(&config);
        assert!(script.contains("C:\\spectyn_mesh\\spectyn-mesh.exe"));
    }
    ...
    fn test_generate_launchd_plist() {
        ...
        assert!(plist.contains(&format!(
            "<string>com.spectyn-mesh.worker.{}</string>",
            config.worker_name
        )));
        ...
        assert!(plist.contains("<key>SPECTYN_MESH_AUTH_TOKEN</key>"));
    }
    ...
    fn test_generate_launchd_plist_no_token() {
        ...
        config.auth_token = None;
        let plist = generate_launchd_plist(&config);
        assert!(!plist.contains("SPECTYN_MESH_AUTH_TOKEN"));
