//! Skill tool catalog — top-10 high-utility tools ported as Rust
//! idiomatic impls. Every tool is gated behind the parent module's
//! `experimental-tools` feature flag.
//!
//! Standard high-utility text/data tools. Each tool lives in its own
//! file and ships with at least one passing unit test.
...
/// The skill tool surface. Mirrors the existing `crate::tools::Tool`
...
pub fn catalog() -> Vec<Box<dyn SkillTool>> {
    vec![
...
mod tests {
    ...
    fn catalog_has_thirty_tools() {
        ...
        assert_eq!(
            cat.len(),
            30,
            "Skill catalog must have exactly 30 tools (T50 expansion)"
        );
    }
