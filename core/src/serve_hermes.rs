//! F400 — RPC endpoints exposing the Hermes skill bank.
//!
//! Three GET routes under `/api/hermes/skills*`:
//!
//!   GET /api/hermes/skills?q=<search>&limit=<n>&offset=<n>
...
//!   marshal). Verified via `core/tests/hermes_rpc_skills.rs::list_endpoint
...
//!
//! Gated behind `experimental-hermes-memory` so the default cargo build is
...
/// F400 — attach the three Hermes skill routes to an existing axum
...
pub fn attach_routes(router: Router<Arc<AppState>>) -> Router<Arc<AppState>> {
    router
        .route("/api/hermes/skills", get(list_skills))
        .route("/api/hermes/skills/:id", get(get_skill_detail))
        .route("/api/hermes/skill-timeline", get(get_skill_timeline))
}
...
async fn list_skills(
    ...
    let mem = match state.hermes_memory.as_ref() {
        Some(m) => m,
        None => {
            return (
                StatusCode::SERVICE_UNAVAILABLE,
                Json(json!({"error": "hermes memory not configured on this node"})),
            )
                .into_response()
        }
    };
    ...
    let (rows, total) = match result {
        ...
        Err(e) => {
            return (
                ...
                Json(json!({"error": format!("hermes query failed: {e}")})),
            )
                .into_response()
        }
    };
...
async fn get_skill_detail(
    ...
    let row = match mem.get_by_id(id).await {
        ...
        Err(e) => {
            return (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({"error": format!("hermes lookup failed: {e}")})),
            )
                .into_response()
        }
    };
...
async fn get_skill_timeline(
    ...
    let rows = match mem.list_since("skill", since_secs, 1024).await {
        ...
        Err(e) => {
            return (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({"error": format!("hermes timeline query failed: {e}")})),
            )
                .into_response()
        }
    };
