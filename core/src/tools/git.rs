...
fn validate_file_arg(file: &str) -> Result<(), String> {
    for ch in [';', '|', '&', '$', '`', '>', '<', '\n', '\r'] {
        if file.contains(ch) {
            return Err(format!("Error: invalid character '{}' in file argument", ch));
        }
    }
    Ok(())
}
...
pub async fn status(args: &Value) -> String {
    ...
    if let Err(e) = validate_git_path(path) { return e; }
        ...
        .output().await
    {
        Ok(out) => {
            ...
            if s.trim().is_empty() { "Working tree clean".into() } else { s }
        }
...
pub async fn diff(args: &Value) -> String {
    ...
    if !file.is_empty() {
        if let Err(e) = validate_file_arg(file) { return e; }
    }
    let mut git_args = vec!["-C", path, "diff", "--stat"];
    if cached { git_args.push("--cached"); }
    if !file.is_empty() { git_args.extend(["--", file]); }
    match tokio::process::Command::new("git").args(&git_args).output().await {
...
pub async fn commit(args: &Value) -> String {
    ...
    if let Err(e) = validate_commit_message(message) { return e; }
    match tokio::process::Command::new("git")
...
pub async fn branch(args: &Value) -> String {
        ...
        .output().await
    {
        Ok(out) => {
            let s = String::from_utf8_lossy(&out.stdout).to_string();
            if s.trim().is_empty() { "No branches".into() } else { s }
        }
        Err(e) => format!("git error: {}", e),
    }
}
...
pub async fn stash(args: &Value) -> String {
    ...
    match tokio::process::Command::new("git").args(&git_args).output().await {
        Ok(out) => {
            ...
            let stderr = String::from_utf8_lossy(&out.stderr).to_string();
            if out.status.success() {
                if stdout.trim().is_empty() { format!("{}(no stashes)", stderr) } else { stdout }
            } else {
                format!("Error: {}{}", stdout, stderr)
            }
        }
...
pub async fn git_branch_list(args: &Value) -> String {
    let repo_path = args["path"].as_str().unwrap_or(".");
    if let Err(e) = validate_git_path(repo_path) { return e; }
        ...
        .output().await
    {
        Ok(out) => {
            ...
            let stderr = String::from_utf8_lossy(&out.stderr).to_string();
            if out.status.success() {
                if stdout.trim().is_empty() { "No branches found.".into() } else { stdout }
            } else {
                format!("Error: {}{}", stdout, stderr)
            }
        }
...
pub async fn git_show(args: &Value) -> String {
    ...
    if let Err(e) = validate_git_path(commit_ref) { return e; }
...
pub async fn git_blame(args: &Value) -> String {
    ...
    if let Err(e) = validate_file_arg(file_path) { return e; }
    let repo_path = args["repo"].as_str().unwrap_or(".");
    if let Err(e) = validate_git_path(repo_path) { return e; }
    match tokio::process::Command::new("git")
        ...
        .output().await
    {
        Ok(out) => {
            ...
            if out.status.success() {
                ...
                let result = lines.iter().take(100).cloned().collect::<Vec<_>>().join("\n");
                if truncated {
    ...
    let files: Vec<String> = match args["files"].as_array() {
        Some(arr) => {
            ...
            for v in arr {
                match v.as_str() {
                    Some(s) => {
                        if let Err(e) = validate_file_arg(s) { return e; }
...
pub async fn git_stash_list(args: &Value) -> String {
        ...
        .output().await
    {
        Ok(out) => {
            ...
            let stderr = String::from_utf8_lossy(&out.stderr).to_string();
            if out.status.success() {
                if stdout.trim().is_empty() { "No stashes.".into() } else { stdout }
            } else {
                format!("Error: {}{}", stdout, stderr)
            }
        }
