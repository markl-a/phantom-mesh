...
//!
//! Lifting this out of the `spectyn` bin (where it used to live, bin-private) is
...
//! 唯一真相。原本藏在 spectyn bin 裡(bin-private),抽到 lib 是統一設計的第一步:
...
/// no default_model" and `spectyn exec` fails for the brand-new user. Baking a
...
pub fn write_onboarding_config(
    ...
    toml.push_str("# spectyn-mesh agents.toml — generated by first-time setup\n");
    ...
    let list = chain
        ...
        .join(", ");
    toml.push_str(&format!("providers = [{}]\n", list));
    toml.push_str("instructions = \"You are spectyn, a helpful AI agent.\"\n");
