...
pub fn record(kind: &str, summary: impl Into<String>) {
    let Some(state_mtx) = STATE.get() else { return; };
...
pub fn events_path() -> Option<PathBuf> {
    STATE.get().and_then(|m| m.lock().ok()).and_then(|g| g.events_log.clone())
}
...
fn write_crash_log(info: &std::panic::PanicHookInfo<'_>) {
    ...
    // write_crash_log too (commit d78f0bf, Mac Bug #24); Z13's commit
...
fn restore_terminal_after_panic() {
    ...
    let _ = out.write_all(b"\x1b[?1049l\x1b[?1000l\x1b[?1002l\x1b[?1003l\x1b[?1015l\x1b[?1006l\x1b[?25h\x1b[0m\n");
    let _ = out.flush();
