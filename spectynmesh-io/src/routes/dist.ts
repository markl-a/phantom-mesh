...
//
// Goal: any Windows box can install spectyn in one command:
...
const BIN_OBJECTS: Record<string, { object: string; contentType: string }> = {
  "spectyn-windows-x86_64.exe": {
    object: "spectyn-windows-x86_64.exe",
    contentType: "application/octet-stream",
  },
  "spectyn-darwin-arm64": {
    object: "spectyn-darwin-arm64",
    contentType: "application/octet-stream",
  },
  "spectyn-darwin-x86_64": {
    object: "spectyn-darwin-x86_64",
    contentType: "application/octet-stream",
  },
  "spectyn-linux-x86_64": {
    object: "spectyn-linux-x86_64",
    contentType: "application/octet-stream",
  },
};
...
export function renderInstallPs1(baseUrl: string): string {
  return `# spectyn mesh — Windows installer
# Run via:
#   iwr -useb ${baseUrl}/install.ps1 | iex
#
# What it does:
#   1. Downloads spectyn.exe to ~/.local/bin (no admin needed)
#   2. Adds ~/.local/bin to user PATH if missing
#   3. Auto-runs \`spectyn login\` to bind to broker + pull LLM keys
#      (skipped on re-install if you already have a fresh token saved).
#      Set $env:SPECTYN_INSTALL_SKIP_LOGIN='1' before piping to skip.
#
# Re-running upgrades the binary in place (kills any running spectyn first).
# Does NOT touch ~/.spectyn-mesh/ — your config + auth survive upgrades.

$ErrorActionPreference = 'Stop'

$BIN_URL  = '${baseUrl}/dist/spectyn-windows-x86_64.exe'
$BIN_DIR  = Join-Path $env:USERPROFILE '.local\\bin'
$BIN_PATH = Join-Path $BIN_DIR 'spectyn.exe'
...
# 1. Stop any running spectyn (lets us overwrite the file)
Write-Host '[1/4] stopping running spectyn...'
Stop-Process -Name spectyn -Force -ErrorAction SilentlyContinue
...
if (Test-Path "$env:USERPROFILE\\bin") {
    Copy-Item $BIN_PATH "$env:USERPROFILE\\bin\\spectyn.exe" -Force
}
...
# 4b. Seed ~/.spectyn-mesh/agents.toml on FIRST install only.
...
# "duplicate key" in TOML parsing). If \`spectyn providers list\` errors
# out, back up + replace.
$shouldSeed = -not (Test-Path $cfgPath)
if (-not $shouldSeed) {
    # Use cmd /c so PS 5.1's native-stderr-as-ErrorRecord quirk doesn't
    # fire $ErrorActionPreference=Stop on a spectyn error message --
    ...
    $tmpOut = Join-Path $env:TEMP "spectyn-providers-check-$PID.txt"
...
if ($shouldSeed) {
    ...
    $defaultCfg = @'
# spectyn-mesh default config — written by phantommesh.io install.ps1
# Edit freely. \`spectyn config pull\` only touches ~/.spectyn-mesh/env
...
# Failover order — spectyn tries each provider:model in turn until one
...
# install scripts (install-spectyn-windows.ps1) wrote agents.toml with
...
#    time on a brand-new install; on a re-install where ~/.spectyn-mesh/
#    auth.json already has a non-expired broker_token, spectyn login
#    short-circuits to a key refresh (no browser).
$skipLogin = $env:SPECTYN_INSTALL_SKIP_LOGIN -eq '1'
if ($skipLogin) {
    Write-Host ''
    Write-Host 'SPECTYN_INSTALL_SKIP_LOGIN=1 set -- skipping login.'
    Write-Host 'Run \`spectyn login\` later to bind this device + pull LLM keys.'
} else {
    Write-Host ''
    Write-Host '=== running spectyn login ==='
    Write-Host '  (opens browser -> Google sign-in -> auto-pulls your LLM keys.'
    Write-Host '   Press Ctrl-C to skip; you can run spectyn login later.)'
    Write-Host ''
    & $BIN_PATH login
}

# 5b. Load ~/.spectyn-mesh/env into THIS PowerShell process so the
#     cluster sync below + the serve we spawn next both see the keys
#     spectyn login just dropped onto disk. Without this, cluster sync
...
$envFile = "$env:USERPROFILE\\.spectyn-mesh\\env"
...
# A brand-new account has no LLM keys saved on the broker — spectyn
...
# 6. Start spectyn serve in the background so cluster RPC works + the
...
Write-Host ''
Write-Host '=== starting spectyn serve ==='
# Always stop first so a stale serve (from before this install) doesn't
# hold the port + serve old key state.
Stop-Process -Name spectyn -Force -ErrorAction SilentlyContinue
& cmd /c 'schtasks /End /TN SpectynMeshServe >nul 2>nul'
Start-Sleep -Milliseconds 400

if ($isAdmin) {
    Write-Host '  (elevated PowerShell -- installing SpectynMeshServe Scheduled Task)'
    & cmd /c 'schtasks /Delete /F /TN SpectynMeshServe >nul 2>nul'
    ...
    $tr = '"' + $BIN_PATH + '" serve'
    & schtasks /Create /TN SpectynMeshServe /TR $tr /SC ONLOGON /RL HIGHEST /F | Out-Null
    $rc = $LASTEXITCODE
    if ($rc -eq 0) {
        & cmd /c 'schtasks /Run /TN SpectynMeshServe >nul 2>nul'
...
$port = 7878
$cfg = "$env:USERPROFILE\\.spectyn-mesh\\agents.toml"
if (Test-Path $cfg) {
...
try {
    ...
    Write-Host '    (run \`spectyn doctor\` to diagnose)'
}
...
# (same effect as SPECTYN_INSTALL_NOLAUNCH=1) and tell them where to go.
if ($emptyVaultHint) {
    ...
    Write-Host '     spectyn config pull   # fetch the new keys to ~/.spectyn-mesh/env'
    Write-Host '     spectyn               # start the TUI'
    Write-Host ''
...
#    Set $env:SPECTYN_INSTALL_NOLAUNCH='1' before piping for CI / scripted
...
$noLaunch = ($env:SPECTYN_INSTALL_NOLAUNCH -eq '1') -or $emptyVaultHint
if ($noLaunch) {
    if (-not $emptyVaultHint) {
        Write-Host 'SPECTYN_INSTALL_NOLAUNCH=1 set -- skipping launch.'
        Write-Host 'Run this when ready:'
        Write-Host '   spectyn         # the TUI'
    }
} else {
    Write-Host 'Press Enter to start spectyn (or Ctrl-C to exit and run it later).'
    [void](Read-Host)
    & $BIN_PATH
}
`;
...
export function renderInstallSh(baseUrl: string): string {
  return `#!/bin/sh
# spectyn mesh — macOS / Linux installer
# Run via:
#   curl -fsSL ${baseUrl}/install.sh | sh
#
# What it does (mirrors install.ps1 step-for-step):
#   1. Detects OS + arch (darwin/linux × arm64/x86_64)
#   2. Stops running spectyn, downloads binary to ~/.local/bin
#   3. (macOS) ad-hoc resigns + adds ~/.local/bin to PATH
#   4. Verifies version
#   5. (macOS) installs launchd plist for 'spectyn serve' auto-start
#   6. Seeds ~/.spectyn-mesh/agents.toml on FIRST install + ensures
#      [providers.opencode] block + sets [agent.master] priority list
#      (auto-recovers from corrupt TOML by backing up + reseeding)
#   7. Auto-runs 'spectyn login' to bind to broker + pull LLM keys
#      (incl. CLUSTER_SECRET). Login itself also auto-registers this
#      machine on the cluster + writes the [cluster] block to
#      agents.toml. Skip with SPECTYN_INSTALL_SKIP_LOGIN=1.
#   8. (macOS) Verifies cluster wiring via 'spectyn cluster status'.
#      Skip with SPECTYN_INSTALL_SKIP_CLUSTER=1.
...
#
# Opt-outs:
#   SPECTYN_INSTALL_NO_LAUNCHD=1     skip macOS launchd plist install
#   SPECTYN_INSTALL_SKIP_LOGIN=1     skip 'spectyn login'
#   SPECTYN_INSTALL_SKIP_CLUSTER=1   skip cluster status check
...
case "\$(uname -s)" in
  Darwin) os=darwin ;;
  Linux)  os=linux  ;;
  *) echo "spectyn: unsupported OS '\$(uname -s)' (need Darwin or Linux)"; exit 1 ;;
esac
case "\$(uname -m)" in
  arm64|aarch64) arch=arm64 ;;
  x86_64|amd64)  arch=x86_64 ;;
  *) echo "spectyn: unsupported arch '\$(uname -m)' (need arm64 or x86_64)"; exit 1 ;;
esac

if [ "\$os" = "linux" ] && [ "\$arch" = "arm64" ]; then
  echo "spectyn: linux/arm64 build not yet available — see https://phantommesh.io"
  exit 1
fi

asset="spectyn-\${os}-\${arch}"
url="${baseUrl}/dist/\${asset}"
bin_dir="\$HOME/.local/bin"
bin_path="\$bin_dir/spectyn"

echo "=== spectyn mesh installer ==="
echo "  source:  \$url"
echo "  target:  \$bin_path"
echo

# 1. Stop any running spectyn (and let launchd quiet for a moment so
# amfid doesn't try to validate the half-written binary)
echo "[1/8] stopping running spectyn..."
if [ "\$os" = "darwin" ]; then
  launchctl bootout "gui/\$(id -u)/ai.spectynmesh.serve" 2>/dev/null || true
fi
...
if command -v curl >/dev/null 2>&1; then
  ...
  echo "spectyn: need curl or wget to download"; exit 1
fi
chmod +x "\$bin_path"
  ...
  *)
    ...
    for rc in "\$HOME/.zshrc" "\$HOME/.bashrc"; do
      if [ -f "\$rc" ] && ! grep -q "\\.local/bin" "\$rc"; then
        echo "" >> "\$rc"
        echo "# Added by spectyn mesh installer" >> "\$rc"
        echo "\$line" >> "\$rc"
      fi
    done
...
if [ "\$os" = "darwin" ] && [ "\${SPECTYN_INSTALL_NO_LAUNCHD:-0}" != "1" ]; then
  echo "[5/8] installing launchd plist for 'spectyn serve' auto-start..."
  plist="\$HOME/Library/LaunchAgents/ai.spectynmesh.serve.plist"
  mkdir -p "\$HOME/Library/LaunchAgents" "\$HOME/.spectyn-mesh"
  cat > "\$plist" <<PLIST
<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
<dict>
  <key>Label</key><string>ai.spectynmesh.serve</string>
  ...
  <key>StandardOutPath</key><string>\$HOME/.spectyn-mesh/serve.out.log</string>
  <key>StandardErrorPath</key><string>\$HOME/.spectyn-mesh/serve.err.log</string>
</dict>
</plist>
PLIST
  launchctl bootstrap "gui/\$(id -u)" "\$plist" 2>/dev/null || launchctl load "\$plist" 2>/dev/null || true
  echo "  -> ai.spectynmesh.serve loaded (logs: ~/.spectyn-mesh/serve.{out,err}.log)"
else
  echo "[5/8] skipped launchd plist (linux or SPECTYN_INSTALL_NO_LAUNCHD=1)"
fi

# 6. Seed ~/.spectyn-mesh/agents.toml on FIRST install only.
...
if [ "\$should_seed" = "1" ]; then
  ...
  cat > "\$cfg_path" <<'TOML'
# spectyn-mesh default config — written by phantommesh.io install.sh
# Edit freely. \`spectyn config pull\` only touches ~/.spectyn-mesh/env
...
if [ "\${SPECTYN_INSTALL_SKIP_LOGIN:-0}" = "1" ]; then
  echo
  echo "[7/8] SPECTYN_INSTALL_SKIP_LOGIN=1 set — skipping login."
  echo '   Run \`spectyn login\` later to bind this device + pull LLM keys.'
  ...
  echo '   Run \`spectyn login\` manually after install completes.'
else
  echo
  echo "[7/8] === running spectyn login ==="
  echo "   (opens browser -> Google sign-in -> auto-pulls LLM keys + CLUSTER_SECRET."
  echo "    Press Ctrl-C to skip; you can run spectyn login later.)"
  echo
  "\$bin_path" login </dev/tty || true
fi

# 8. Verify cluster wiring. \`spectyn login\` already auto-registers this
...
# \`spectyn cluster status\` — if that errors out (most often: bad TOML
# left by the login auto-write), surface the recovery hint.
if [ "\$os" = "darwin" ] && [ "\${SPECTYN_INSTALL_SKIP_CLUSTER:-0}" != "1" ]; then
  echo
  ...
  if echo "\$status_out" | grep -qE 'TOML parse error|not valid TOML'; then
    echo
    echo "  ! agents.toml is broken — 'spectyn login' may have written"
    echo "    an invalid [cluster] block. Manual recovery:"
    echo "      \$EDITOR ~/.spectyn-mesh/agents.toml    # fix or delete [cluster]"
    echo "      spectyn cluster join mac-coordinator   # rewrite cleanly"
  fi
elif [ "\$os" = "linux" ]; then
  echo
  echo "[8/8] cluster: skipped (Linux — no default node name in topology)"
  echo "   set up manually after install:"
  echo "     spectyn cluster join <name>     # see 'spectyn cluster --help'"
fi
...
if [ "\$os" = "darwin" ]; then
  launchctl kickstart -k "gui/\$(id -u)/ai.spectynmesh.serve" 2>/dev/null || true
fi
sleep 1
...
if [ "\$hz" = "200" ]; then
  ...
  echo "  ! healthz probe on :\$port returned \$hz (run 'spectyn doctor' to diagnose)"
fi
...
# broker yet. \`spectyn login\` then drops an env file with only
...
empty_vault=1
if [ -f "\$HOME/.spectyn-mesh/env" ]; then
  if grep -qE '^(OPENAI|ANTHROPIC|GROQ|GOOGLE|GEMINI|OPENROUTER|OPENCODE|MISTRAL|TOGETHER|CEREBRAS|DEEPSEEK|NVIDIA)_API_KEY=..' "\$HOME/.spectyn-mesh/env"; then
    empty_vault=0
  fi
fi
...
if [ "\$empty_vault" = "1" ]; then
  ...
  echo "     spectyn config pull   # fetch the new keys to ~/.spectyn-mesh/env"
  echo "     spectyn               # start the TUI"
  echo
  exit 0
fi
...
# so an 'exec spectyn </dev/tty' only redirects stdin — stdout/stderr
...
# clean enough to be worth the fragility. The user just types 'spectyn'
# in their existing terminal after the installer exits.
echo "Run when ready:"
echo "   spectyn         # the TUI"
echo "   spectyn doctor  # cluster + provider sanity check"
`;
}
