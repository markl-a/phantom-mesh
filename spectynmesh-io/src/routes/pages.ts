...
<html><head>
  ...
  <div class="wrap">
    ...
    <div class="card">
      ...
      <a class="btn" href="https://github.com/markl-a/spectyn-mesh">GitHub</a>
    </div>

    <div class="card" style="margin-top:16px">
      <div style="font-weight:500">Install spectyn CLI</div>
      ...
      <details>
        ...
        <div class="pad">
          <a class="btn" href="/dist/spectyn-darwin-arm64">macOS (Apple Silicon)</a>
          <a class="btn" href="/dist/spectyn-windows-x86_64.exe">Windows x86_64</a>
        </div>
      </details>
    </div>

    <div class="footer">
      Broker version ${c.env.BROKER_VERSION} · spec: <a href="https://github.com/markl-a/spectyn-mesh/blob/main/docs/SPECTYNMESH-IO-DESIGN.md">design doc</a>
    </div>
  </div>
  <script>
...
<html><head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width,initial-scale=1">
  <title>Sign in · spectyn mesh</title>
  <style>${STYLE}</style>
</head><body>
  <div class="wrap">
    <div class="logo">◆</div>
    <h1>Sign in</h1>
    <p class="sub">${isCli
      ? "spectyn is asking permission for this device"
...
export async function accountPage(c: Context<{ Bindings: Env }>) {
  ...
  const deviceRows = devices.length === 0
    ? `<p class="empty">No devices linked yet. Run <code>spectyn login</code> on a machine to link it.</p>`
    : devices.map(d => `
        <div class="device">
          <div>
  ...
  const onboardingHint = hasAnyKey ? "" : `
    <div style="background:#16161c;border:1px dashed #4d4a42;border-radius:6px;padding:12px;
      ...
      <div style="margin-top:8px;color:#6a6a78">
        Paste any one of them in the matching field below; spectyn auto-failovers
  ...
  const settingsBlock = `
    ...
    <p style="color:#8a8578;font-size:13px;margin:0 0 12px">
      Saved here, pulled on each machine via <code>spectyn config pull</code>.
...
<html><head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width,initial-scale=1">
  <title>Account · spectyn mesh</title>
  <style>${STYLE}
  ...
  <div class="wrap">
    ...
    <div class="card">
      ...
      <p style="color:#8a8578;font-size:13px;margin:0 0 12px">
        On any machine that has spectyn installed, paste this once. Token
      ...
      <div style="background:#0c0c10;border:1px solid #2a2a35;border-radius:6px;padding:10px 12px;
        ...
        <div id="cli-cmd-mask" style="color:#4d4a42">spectyn config pull --token=•••••••••••••••• --url=https://phantommesh.io</div>
  ...
  <script>
    ...
    (function () {
      ...
      const realCmd = 'spectyn config pull --token=' + realToken + ' --url=https://phantommesh.io';
...
async function renderClusterPeersBlock(c: Context<{ Bindings: Env }>, userId: number): Promise<string> {
  ...
  return `
      ...
      <p style="color:#8a8578;font-size:13px;margin:0 0 12px">
        Mesh nodes that <code>spectyn cluster join/status</code> dispatches RPC to.
        Pulled by every machine via <code>spectyn config pull</code>; saving here
        immediately propagates on next pull. The <strong>last-seen</strong> badge
        is updated whenever a node runs <code>spectyn login</code> or
        <code>spectyn config pull</code>. URLs are Tailscale (100.x.y.z) or LAN;
