...
function AuthGate({ children }: { children: React.ReactNode }) {
  ...
  if (authed === null) {
    return (
      <div className="flex h-full items-center justify-center text-phantom-muted">
        loading…
      </div>
    );
  }
...
function Sidebar() {
  ...
  return (
    <aside className="flex w-56 flex-col gap-1 border-r border-phantom-border bg-phantom-card p-3">
      <div className="mb-4 px-3 py-2 text-xs uppercase tracking-wider text-phantom-muted">
        phantom mesh
      </div>
        ...
        <main className="flex-1 overflow-auto p-6">
          <Routes>
            ...
            <Route
              path="*"
              element={
                <div className="text-phantom-muted">page not found</div>
              }
            />
          </Routes>
        </main>
