// Authenticated API surface. Used by phantom CLI's `phantom devices`
...
/// (last 4 chars only). Phantom CLI uses a separate /api/me/settings/raw
/// endpoint when it needs the actual values to write to ~/.phantom-mesh/env;
...
/// Body: {name, url, label?}. Used by `phantom login`'s self-register
...
/// `phantom sessions` CLI and the dashboard's "active now" panel.
