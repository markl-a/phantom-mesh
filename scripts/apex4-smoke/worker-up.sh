...
#   1. preflights: phantom on PATH, claude CLI present + logged in, agents.toml
#      has (a) a [cluster].cluster_secret, (b) a claude-capable agent;
#   2. exports PHANTOM_GOVERN_CLI=1 + PHANTOM_HOME so dispatched CLI work is
#      routed through L1 governance and the pending-card writer + serve share a
#      data root;
#   3. starts `phantom serve` and polls /healthz until ready;
...
# secret is never printed. Override the data root with PHANTOM_HOME before
# running, or pass --home <dir>.
#
# Usage:
#   ./worker-up.sh [--home <dir>] [--no-serve] [--foreground]
#
#   --home <dir>    data root (default: $PHANTOM_HOME or ~/.phantom-mesh)
#   --no-serve      run preflight + print the env only; do NOT start serve
#   --foreground    run `phantom serve` in the foreground (default: background,
...
HOME_DIR="${PHANTOM_HOME:-$HOME/.phantom-mesh}"
START_SERVE=1
FOREGROUND=0
while [ $# -gt 0 ]; do
  case "$1" in
...
# ── 1. preflight: phantom on PATH ──────────────────────────────────────────
command -v phantom >/dev/null 2>&1 \
  || die "phantom not on PATH. Build it and add the binary to PATH first."
ok "phantom found: $(command -v phantom)"
...
# Resolution order mirrors the runtime: $PHANTOM_HOME/agents.toml, then
# ~/.phantom-mesh/agents.toml. (config::AgentsConfig::find_and_load.)
AGENTS_TOML=""
for cand in "$HOME_DIR/agents.toml" "$HOME/.phantom-mesh/agents.toml" "./agents.toml"; do
  if [ -f "$cand" ]; then AGENTS_TOML="$cand"; break; fi
done
[ -n "$AGENTS_TOML" ] \
  || die "no agents.toml found (looked in $HOME_DIR, ~/.phantom-mesh, .). Run \`phantom cluster join <name>\` first."
...
[ -n "$SECRET" ] \
  || die "[cluster].cluster_secret missing/empty in $AGENTS_TOML. Run \`phantom cluster join <name>\` so both nodes share a secret."
...
export PHANTOM_GOVERN_CLI=1
export PHANTOM_HOME="$HOME_DIR"
mkdir -p "$HOME_DIR"
ok "PHANTOM_GOVERN_CLI=1"
ok "PHANTOM_HOME=$HOME_DIR"
...
if [ "$START_SERVE" -eq 0 ]; then
  ...
  echo  "    PHANTOM_GOVERN_CLI=1 PHANTOM_HOME='$HOME_DIR' phantom serve" >&2
  exit 0
fi
...
if [ "$FOREGROUND" -eq 1 ]; then
  info "starting \`phantom serve\` in the FOREGROUND (Ctrl-C to stop)…"
  info "in another shell, confirm readiness: curl $HEALTH_URL"
  exec phantom serve
fi

LOG="$HOME_DIR/apex4-smoke-serve.log"
info "starting \`phantom serve\` in the background → $LOG"
# shellcheck disable=SC2069
( PHANTOM_GOVERN_CLI=1 PHANTOM_HOME="$HOME_DIR" phantom serve >"$LOG" 2>&1 & echo $! >"$HOME_DIR/apex4-smoke-serve.pid" )
