# Spectyn Mesh -- Windows worker install
#
# One-liner from a regular PowerShell window (no admin required for the
# user-mode Scheduled Task path):
#
#   $env:COORD = "https://your-coordinator:7878"   # https; plain http needs SPECTYN_ALLOW_INSECURE=1 (trusted tailnet)
...
#   iex (iwr "$env:COORD/scripts/install-spectyn-windows.ps1").Content
#
# What it does (~2 min):
#   1. Download spectyn.exe from the coordinator into ~/.spectyn-mesh/bin/
#      (the same path docs/SESSION-ONBOARDING.md Sec.3.1 expects, and the
#      same path `spectyn service install` registers with the Scheduled
...
#   4. Register the "SpectynServe" Scheduled Task via the binary's own
#      `spectyn service install` (which uses PowerShell's
...
# location is ~/.spectyn-mesh/bin/spectyn.exe, not %LOCALAPPDATA%/SpectynMesh.
...
Write-Host "=== Spectyn Mesh Windows install ===" -ForegroundColor Cyan
Write-Host ""
...
function Require-Https {
    ...
    if ($Url -like 'http://*') {
        if ($env:SPECTYN_ALLOW_INSECURE -eq '1') {
            Write-Warning "SPECTYN_ALLOW_INSECURE=1 - accepting plain http:// URL ($Url). THIS DISABLES MITM PROTECTION."
            return
        }
        throw "Refusing to download over plain http://`n  URL: $Url`n  Use https://, or set `$env:SPECTYN_ALLOW_INSECURE='1' (only safe on a trusted tailnet)."
    }
...
function Verify-Sha256 {
    ...
    if ($env:SPECTYN_SKIP_VERIFY -eq '1') {
        Write-Warning "SPECTYN_SKIP_VERIFY=1 - SKIPPING SHA256 verification of $BinaryPath (a MITM/compromised mirror could swap the binary)."
        return
    }
    ...
    try {
        try {
            Invoke-WebRequest -Uri $sumsUrl -OutFile $sumsFile -UseBasicParsing -TimeoutSec 30 -Headers @{ 'User-Agent' = 'spectyn-installer/1.0' } | Out-Null
        } catch {
            Remove-Item -Force $BinaryPath -ErrorAction SilentlyContinue
            throw "Could not fetch SHA256 sidecar at $sumsUrl ($_). Refusing to install an unverified binary. Set `$env:SPECTYN_SKIP_VERIFY='1' to bypass (NOT recommended)."
        }
...
# -- 1. Download binary -------------------------------------------------------
Write-Host "[1/5] Downloading spectyn.exe ..." -ForegroundColor Cyan
...
# Stop a running spectyn (if any) before overwriting the binary, otherwise
...
$exeUrl = "$COORD/dist/spectyn-x86_64-pc-windows.exe"
...
try {
    ...
    Write-Host "    `spectyn service install` from admin shell also installs the rule." -ForegroundColor Yellow
}

# -- 4. Register Scheduled Task via the binary itself -------------------------
Write-Host "[4/5] Registering Scheduled Task 'SpectynServe' ..." -ForegroundColor Cyan
& $BIN service install
if ($LASTEXITCODE -ne 0) {
    Write-Host "  ! spectyn service install returned $LASTEXITCODE - check output above" -ForegroundColor Yellow
}
...
try {
    ...
    if ($resp.StatusCode -eq 200) {
        Write-Host "  OK spectyn serve responding on :$PORT" -ForegroundColor Green
    } else {
...
Write-Host ""
Write-Host "==================================================================="
Write-Host "    Use spectyn on this Windows box - three ways:" -ForegroundColor Cyan
Write-Host "==================================================================="
Write-Host ""
...
Write-Host ""
Write-Host "  3) Cluster worker - already wired up via Scheduled Task SpectynServe."
if ($tsIp) {
    Write-Host "     From Mac: subagent({ node: '${tsIp}:${PORT}', ... })"
}
Write-Host ""
if (-not $env:OPENROUTER_API_KEY) {
    Write-Host "  ! OPENROUTER_API_KEY not set - spectyn will have no LLM backend." -ForegroundColor Yellow
