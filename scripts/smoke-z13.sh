#!/usr/bin/env bash
# Z13 (Windows) phantom smoke test, run from the Mac over SSH.
#
# Usage: bash scripts/smoke-z13.sh
#
# Prereq: Z13 (yoyogood) reachable on Tailscale 100.87.70.65 with phantom
# already deployed at C:\Users\m4932\.phantom-mesh\bin\phantom.exe and
...
Z13_HOST=m4932@100.87.70.65
Z13_PORT=7879           # NOTE: Windows daemon listens on 7879, not 7878
SOCK="$HOME/.ssh/cm-z13-$$"
TMP=$(mktemp -d)
...
echo "$(bold '◆ Establishing ControlMaster (you will be prompted for the Z13 password ONCE)')"
echo "  $Z13_HOST"
echo ""
...
# Helper: run command on Z13 via the shared connection
remote() {
  ssh -S "$SOCK" "$Z13_HOST" "$@"
}

# Helper: copy file to Z13 via the shared connection
remote_cp() {
  scp -o ControlPath="$SOCK" "$1" "$Z13_HOST:$2"
}

cleanup() {
  ssh -S "$SOCK" -O exit "$Z13_HOST" 2>/dev/null
  rm -f "$SOCK"
}
trap cleanup EXIT
...
section "3. HTTP daemon (Z13 :7879)"

# Reach via Tailscale
if curl -sf -m 4 "http://100.87.70.65:$Z13_PORT/healthz" >/dev/null 2>&1; then
  ok "GET /healthz from Mac (Tailscale)" "200 OK on :$Z13_PORT"
else
  fail "GET /healthz from Mac" "unreachable on Tailscale 100.87.70.65:$Z13_PORT"
fi

# /api/version
ver=$(curl -sf -m 4 "http://100.87.70.65:$Z13_PORT/api/version" 2>&1)
...
for ep in /api/cost /api/sessions /api/nodes /api/todos /api/status /api/tools/history /api/providers/health /api/dashboard/status; do
  code=$(curl -s -o /dev/null -w "%{http_code}" -m 4 "http://100.87.70.65:$Z13_PORT$ep" 2>&1)
  [[ "$code" == "200" ]] \
    && ok "GET $ep" "200" \
...
# Run via SSH so the Z13's local agents.toml + provider keys are used.
# Don't quote the marker — let it be a literal arg to phantom.
out=$(remote 'powershell -NoProfile -Command "& $env:USERPROFILE\.phantom-mesh\bin\phantom.exe run \"Use shell to run echo phantom-z13-marker-99 and report what it printed\""' 2>&1 | tr -d '\r' | tail -10)
if echo "$out" | grep -q "phantom-z13-marker-99"; then
...
section "6. phantom doctor on Z13"
...
if (( red_n == 0 )); then
  ok "phantom doctor on Z13" "$green_n green / $warn_n warn / $red_n red"
else
  fail "phantom doctor on Z13" "$red_n red findings"
  echo "  ── doctor red lines: ──"
  echo "$doc_out" | grep '✗' | head -5 | sed 's/^/    /'
fi

# Save full output for inspection
echo "$doc_out" > "$TMP/z13-doctor.txt"
echo "  (full doctor output saved to $TMP/z13-doctor.txt)"

# ── 7. cluster reverse-direction ───────────────────────────────────────────
section "7. cluster cross-machine reachability"

# From Z13's POV, can it reach Mac's phantom serve at :7878?
...
if [[ -n "$mac_tail" ]]; then
  ...
  if [[ "$reach" == "200" ]]; then
    ok "Z13 → Mac :7878 reachable (Tailscale 雙向)" "$mac_tail"
  else
    fail "Z13 → Mac :7878 reachable" "got: $reach"
  fi
else
  skip "Z13 → Mac reverse" "Mac Tailscale IP not detected"
fi
