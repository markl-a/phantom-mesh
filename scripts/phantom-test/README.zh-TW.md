# phantom-test - 黑箱場景測試 harness

[English version](README.md)

這是一套 bash-based 測試 harness。它透過 public surfaces 操作已經啟動的
`phantom`：CLI、HTTP/RPC、磁碟狀態與真實 LLM/RPC round-trip。它不需要
`cargo build`，與 repo 內的 `cargo test` 互補，不是替代品。

| 層級 | 位置 | 驗證內容 | 執行間是否重置 |
|---|---|---|---|
| `cargo test` | `core/src/**/*.rs` | 內部 invariant、render path、parser state | 每個測試全新 |
| `phantom-test` | `scripts/phantom-test/` | Wire protocol、磁碟狀態、CLI、真實 round-trip | Side effects 會累積，這是刻意設計 |
...
| 變數 | 預設值 | 用途 |
|---|---|---|
| `PHANTOM_BIN` | `phantom` | PATH 上的 binary 或絕對路徑 |
| `PHANTOM_HOST` | `127.0.0.1` | Serve host |
| `PHANTOM_PORT` | `7879` | Serve port |
| `PHANTOM_CLUSTER_SECRET` | `phantom-cluster-2026` | HMAC RPC secret |
| `PHANTOM_CONFIG_DIR` | `~/.phantom-mesh` | DB 與 event storage |
...
PNG 會存到 `~/.phantom-mesh/snapshots/`。

## 已知限制

| 缺口 | 後續方案 |
|---|---|
| 對執行中的 TUI 注入按鍵 | 新增 `phantom tui --headless --script keys.txt` |
| Headless TUI buffer dump | 新增 `phantom tui render --state fixture.json --output snap.txt` |
...
適合使用 `phantom-test` scenario 的情況：
