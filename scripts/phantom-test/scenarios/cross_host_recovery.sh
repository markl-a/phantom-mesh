...
#   Phase 3.  Kill `phantom serve` on node B over SSH.
...
#             × 1.1, configurable via $PHANTOM_E001_HEARTBEAT_DEADLINE_SECS).
...
#   Phase 6.  Restart `phantom serve` on B.
...
#   PHANTOM_NODE_B_SSH       — same for Node B (this is the one that gets killed)
...
#   PHANTOM_NODE_A_PORT      — default 7878
#   PHANTOM_NODE_B_PORT      — default 7878
...
#   PHANTOM_E001_HEARTBEAT_DEADLINE_SECS
...
#                                       PHANTOM_HEARTBEAT_DEADLINE_S (alias).
#   PHANTOM_HEARTBEAT_INTERVAL_SECS   — interval written into agents.toml on
#                                       both nodes (default 30).
#   PHANTOM_HEARTBEAT_FAILURE_THRESHOLD
...
#   PHANTOM_CLEANUP_SKIP=1   — leave processes running after the script exits.
#
# Flags
# -----
...
#   bash scripts/phantom-test/scenarios/cross_host_recovery.sh
#
# Local smoke:
...
HB_INTERVAL="${PHANTOM_HEARTBEAT_INTERVAL_SECS:-30}"
HB_THRESHOLD="${PHANTOM_HEARTBEAT_FAILURE_THRESHOLD:-3}"

# --quick collapses the heartbeat window to ~10s so localhost smokes finish fast.
if [ "$QUICK" = "1" ]; then
  HB_INTERVAL="${PHANTOM_HEARTBEAT_INTERVAL_SECS:-5}"
  HB_THRESHOLD="${PHANTOM_HEARTBEAT_FAILURE_THRESHOLD:-2}"
fi

# Deadline default = interval × threshold × 1.1 (matches spec's "± 10%").
# Honour either the spec-named PHANTOM_E001_HEARTBEAT_DEADLINE_SECS or the
# shorter alias used by some operator runbooks.
_default_deadline=$(( HB_INTERVAL * HB_THRESHOLD * 11 / 10 + 2 ))
DEADLINE="${PHANTOM_E001_HEARTBEAT_DEADLINE_SECS:-${PHANTOM_HEARTBEAT_DEADLINE_S:-$_default_deadline}}"
...
step "node A: $NODE_A_NAME @ $PHANTOM_NODE_A_URL"
step "node B: $NODE_B_NAME @ $PHANTOM_NODE_B_URL (THE one that gets killed/restarted)"
...
fail_and_dump() {
  fail "$1"
  dump_peers "$PHANTOM_NODE_A_URL" "$NODE_A_NAME"
  dump_peers "$PHANTOM_NODE_B_URL" "$NODE_B_NAME"
...
# Kill the phantom serve listening on $1 (port). On Windows, `nohup
# ./phantom.exe &` records $! as the bash subshell PID — not the real
# phantom.exe Windows PID — so PID-based kills miss the actual binary. The
...
title "F003 · sweep — kill any stale phantom serve from a prior run"
sweep_one() {
  LBL="$1"; SSHP="$2"; PORT="$3"
  step "node $LBL: killing whatever listens on :$PORT (stale phantom)"
  remote_run "$SSHP" "
    sh -c '$KILL_PORT_SNIPPET' _ '$PORT'
    rm -f \$HOME/$REMOTE_HOME/phantom-serve.pid
  " || true
}
sweep_one "$NODE_A_NAME" "$PHANTOM_NODE_A_SSH" "$NODE_A_PORT"
sweep_one "$NODE_B_NAME" "$PHANTOM_NODE_B_SSH" "$NODE_B_PORT"

# ── Phase 1c: launch phantom serve on both nodes ────────────────────────────
title "F003 · launch — phantom serve on both nodes (heartbeat ON)"
launch_one() {
  ...
  "; then
    fail_and_dump "launch: node $LBL: failed to spawn phantom serve"
  fi
...
cleanup_processes() {
  if [ "${PHANTOM_CLEANUP_SKIP:-0}" = "1" ]; then return; fi
  for tuple in "${NODE_A_NAME}|${PHANTOM_NODE_A_SSH}|${NODE_A_PORT}" "${NODE_B_NAME}|${PHANTOM_NODE_B_SSH}|${NODE_B_PORT}"; do
...
if elapsed=$(wait_peer_online "$PHANTOM_NODE_A_URL" "$NODE_B_NAME" "true" "$DEADLINE"); then
  pass "baseline: A sees B online ($elapsed)"
else
  fail_and_dump "Phase 2: A never saw B as online within ${DEADLINE}s"
fi

RESP_BASE=$(dispatch_with_caps "$PHANTOM_NODE_A_URL" '["vision"]' \
...
step "killing phantom serve on $NODE_B_NAME (port-based kill on :$NODE_B_PORT)"
if ! remote_run "$PHANTOM_NODE_B_SSH" "sh -c '$KILL_PORT_SNIPPET' _ '$NODE_B_PORT'"; then
...
# wrapper rather than phantom.exe), so the HTTP probe is the load-bearing
...
for i in 1 2 3 4 5; do
  code=$(curl -sS -o /dev/null -w '%{http_code}' --max-time 2 \
              "$PHANTOM_NODE_B_URL/healthz" 2>/dev/null)
  ec=$?
...
if elapsed=$(wait_peer_online "$PHANTOM_NODE_A_URL" "$NODE_B_NAME" "false" "$DEADLINE"); then
  pass "Test 1: A marked B online=false ($elapsed within ${DEADLINE}s window)"
else
  dump_peers "$PHANTOM_NODE_A_URL" "$NODE_A_NAME"
  fail_and_dump "Test 1: A never flipped B to online=false within ${DEADLINE}s (heartbeat interval=${HB_INTERVAL}s threshold=${HB_THRESHOLD}; verify the phantom binary was built with --features experimental-cluster-heartbeat)"
fi
...
start=$(date +%s)
RESP_DOWN=$(dispatch_with_caps "$PHANTOM_NODE_A_URL" '["vision"]' \
...
step "restarting phantom serve on $NODE_B_NAME"
launch_one "$NODE_B_NAME" "$PHANTOM_NODE_B_SSH" "$PHANTOM_NODE_B_URL" "$NODE_B_PORT"
...
RESP_OK=$(dispatch_with_caps "$PHANTOM_NODE_A_URL" '["vision"]' \
