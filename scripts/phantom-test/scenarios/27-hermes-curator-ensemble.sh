#!/usr/bin/env bash
# 27-hermes-curator-ensemble.sh — verify `phantom evolve --judge --ensemble N`
...
# binary must have been built with --features experimental-hermes-curator.
#
# Primary correctness coverage lives in core/tests/curator_v2_integration.rs
# (wiremock-driven, runs in regular cargo test --features experimental-hermes-curator).
...
if [ "${PHANTOM_HERMES_ENSEMBLE:-0}" != "1" ]; then
  warn "skip: set PHANTOM_HERMES_ENSEMBLE=1 to enable (binary must also be built with --features experimental-hermes-curator)"
  exit 77
fi
