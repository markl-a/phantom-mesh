...
#    (per `phantom-test` perf assertion)"
#
...
#      per-node agents.toml + start `phantom serve` on both, /healthz wait).
...
#   4. Asserts p99 < PHANTOM_E001_P99_BUDGET_MS (default 500 ms). On miss,
#      prints the full sorted distribution plus min/max and exits non-zero.
#   5. Cleans up (kills both `phantom serve`, retains logs) — idempotent.
#
# Environment contract (REQUIRED — same as F002)
# ----------------------------------------------
#   PHANTOM_NODE_A_SSH       — full ssh prefix for Node A
#   PHANTOM_NODE_B_SSH       — same for Node B
#   PHANTOM_NODE_A_URL       — Node A base URL (reachable from this host + B)
#   PHANTOM_NODE_B_URL       — same for Node B
#   PHANTOM_CLUSTER_SECRET   — shared HMAC secret
#
# Environment contract (OPTIONAL)
# -------------------------------
#   PHANTOM_BIN_LOCAL        — phantom binary on THIS host to SCP
#                              (default: $(command -v phantom))
#   PHANTOM_NODE_A_NAME      — default "node-a"
#   PHANTOM_NODE_B_NAME      — default "node-b"
#   PHANTOM_NODE_A_PORT      — default 7878
#   PHANTOM_NODE_B_PORT      — default 7878
#   PHANTOM_REMOTE_HOME      — default ".phantom-mesh-e001" (shared with F002)
#   PHANTOM_SCP              — default "scp" (set to "cp" for local smoke)
#   PHANTOM_HEALTHZ_TIMEOUT_S — default 30
#   PHANTOM_DISPATCH_TIMEOUT_S — default 15
#   PHANTOM_E001_P99_BUDGET_MS — perf gate (default 500)
#                                — looser for cross-WAN runs without code
#                                  changes per F004 acceptance criterion
#   PHANTOM_PERF_SAMPLES      — number of forwarding requests (default 100)
#   PHANTOM_BOOTSTRAP_SKIP=1  — assume both `phantom serve` already running
#                               (fast re-runs against an already-warmed bed)
#   PHANTOM_CLEANUP_SKIP=1    — leave both `phantom serve` running on exit
#
# Exit codes
# ----------
#   0   — all 100 forwards landed; p99 < PHANTOM_E001_P99_BUDGET_MS
...
#   bash scripts/phantom-test/scenarios/cross_host_perf.sh
#
...
P99_BUDGET_MS="${PHANTOM_E001_P99_BUDGET_MS:-500}"
SAMPLES="${PHANTOM_PERF_SAMPLES:-100}"
...
case "$SAMPLES" in
  ''|*[!0-9]*) warn "PHANTOM_PERF_SAMPLES must be a positive integer, got '$SAMPLES'"; exit 77 ;;
esac
if [ "$SAMPLES" -lt 1 ]; then
  warn "PHANTOM_PERF_SAMPLES must be >= 1, got $SAMPLES"; exit 77
fi

step "node A: $NODE_A_NAME @ $PHANTOM_NODE_A_URL"
step "node B: $NODE_B_NAME @ $PHANTOM_NODE_B_URL"
...
dispatch_timed_oneline() {
  ...
  out=$(curl -sS --max-time "$DISPATCH_TIMEOUT" \
       ...
       -H "Content-Type: application/json" \
       -d "$body" \
       -w '\n__PHANTOM_TIME__ %{time_total} %{http_code}\n' \
       2>/dev/null)
  rc=$?
  if [ $rc -ne 0 ]; then
  ...
  tline=$(printf '%s\n' "$out" | grep '^__PHANTOM_TIME__ ' | tail -n 1)
  rbody=$(printf '%s\n' "$out" | grep -v '^__PHANTOM_TIME__ ' | tr -d '\r\n')
...
EOF
  ...
  title "F004 . sweep - kill any stale phantom serve from a prior run"
  sweep_one() {
    LBL="$1"; SSHP="$2"
  ...
  title "F004 . launch - phantom serve on both nodes"
  launch_one() {
...
while [ "$i" -le "$SAMPLES" ]; do
  ...
  if [ $((i % 2)) -eq 1 ]; then
    src_url="$PHANTOM_NODE_A_URL"; src_lbl="$NODE_A_NAME"
    caps='["vision"]'; expected_dst="$NODE_B_NAME"
  else
    src_url="$PHANTOM_NODE_B_URL"; src_lbl="$NODE_B_NAME"
...
# ── Step 5: cleanup ─────────────────────────────────────────────────────────
if [ "${PHANTOM_CLEANUP_SKIP:-0}" != "1" ] && [ "${PHANTOM_BOOTSTRAP_SKIP:-0}" != "1" ]; then
  title "F004 . cleanup - killing phantom serve on both nodes (logs retained)"
  cleanup_one() {
    LBL="$1"; SSHP="$2"
    remote_run "$SSHP" "
      pidf=\$HOME/$REMOTE_HOME/phantom-serve.pid
