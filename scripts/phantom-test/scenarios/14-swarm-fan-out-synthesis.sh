...
scenario "phantom swarm — fan-out to all online peers + synthesize"
...
self="$(rpc_url 2>/dev/null || echo "http://${PHANTOM_HOST}:${PHANTOM_PORT}")"
online_peers=$("$PHANTOM_BIN" peer list 2>/dev/null \
  | sed -E 's/\x1b\[[0-9;]*m//g' \
...
out=$("$PHANTOM_BIN" swarm "Reply with exactly your hostname and nothing else." 2>&1 \
      | sed -E 's/\x1b\[[0-9;]*m//g')
