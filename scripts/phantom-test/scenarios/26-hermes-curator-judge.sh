#!/usr/bin/env bash
# 26-hermes-curator-judge.sh — verify `phantom evolve --judge` parses the flag
# and (when run with PHANTOM_HERMES_CURATOR=1 + a real ANTHROPIC_API_KEY) writes
# a numeric score into the latest EvolveCheckpoint.
#
# This scenario is opt-in (PHANTOM_HERMES_CURATOR=1) because (a) it requires
# a real API key and (b) the binary must have been built with
# --features experimental-hermes-curator. Otherwise it skips (exit 77).
source "$PHANTOM_TEST_LIB/common.sh"
scenario "phantom evolve --judge writes a JudgeVerdict to the latest checkpoint"

if [ "${PHANTOM_HERMES_CURATOR:-0}" != "1" ]; then
  warn "skip: set PHANTOM_HERMES_CURATOR=1 to enable (binary must also be built with --features experimental-hermes-curator)"
  exit 77
fi
...
if ! echo "$help_out" | grep -q -- '--judge'; then
  ...
  warn "binary usage does not advertise --judge — may indicate missing --features experimental-hermes-curator build"
fi
...
step "running phantom evolve --max-rounds 1 --judge (180s hard cap)"
timeout 180 "$PHANTOM_BIN" evolve --max-rounds 1 --judge "echo hello world; this is a no-op evolve goal for the judge scenario" \
  > "$PHANTOM_TEST_TMP/judge-stdout.log" 2>&1 || true
...
if command -v jq >/dev/null 2>&1; then
  ...
  if [ -n "$SCORE" ]; then
    ...
    fail "judge_score.score is empty (binary may not be built with --features experimental-hermes-curator)"
  fi
