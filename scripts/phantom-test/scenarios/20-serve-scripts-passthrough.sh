...
scenario "phantom serve — /scripts/ passthrough returns repo PowerShell file as text"
...
target="install-phantom-windows.ps1"
...
if printf '%s' "$preview_body" | grep -q 'script not found in any candidate path'; then
    warn "serve's cwd has no reachable scripts/ dir; skipping (not a serve bug — re-run from a phantom started inside the project root)"
    exit 77
fi
...
# content-disposition: inline; filename="install-phantom-windows.ps1" — note INLINE not attachment for scripts.
...
ASSERT_CONTAINS "$body" 'PhantomMesh' "body references PhantomMesh"
