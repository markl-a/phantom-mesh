...
# Phantom's RPC layer mixes two error modes:
...
# 2026-05-02: phantom currently SILENTLY DEFAULTS to the master agent when
    ...
    202|200)
        ...
        if [ -z "$jidB" ]; then
            ...
            case "$stB" in
                    ...
                    pass "missing-agent → reached terminal state ($stB; phantom silently defaulted)"
    ...
    202|200)
        ...
        if [ -z "$jidC" ]; then
            ...
            case "$stC" in
                    ...
                    pass "unknown-agent → reached terminal state ($stC; phantom silently defaulted)"
