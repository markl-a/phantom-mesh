#!/usr/bin/env bash
# 26-curator-judge.sh — verify `phantom evolve --judge` parses the flag
...
if [ "${PHANTOM_HERMES_CURATOR:-0}" != "1" ]; then
  warn "skip: set PHANTOM_HERMES_CURATOR=1 to enable (binary must also be built with --features experimental-curator)"
  exit 77
fi
