...
# or auto-discover the first non-self online peer from `phantom peer list`.
target="${PHANTOM_REMOTE_PEER:-}"

if [ -z "$target" ]; then
  step "PHANTOM_REMOTE_PEER unset — auto-discovering first online non-self peer"
  require_cmd "$PHANTOM_BIN"

  self_url="$(rpc_url)"
  raw=$("$PHANTOM_BIN" peer list 2>&1 | sed -E 's/\x1b\[[0-9;]*m//g')

  # Each peer line in `phantom peer list` looks like:
  ...
  if [ -z "$target" ]; then
    warn "no online non-self peer in phantom peer list — skipping"
    exit 77
  fi
...
PHANTOM_HOST=$(echo "$target" | sed -E 's|^https?://([^:/]+).*|\1|')
PHANTOM_PORT=$(echo "$target" | sed -E 's|^https?://[^:]+:([0-9]+).*|\1|')
[ -z "$PHANTOM_PORT" ] && PHANTOM_PORT=80

step "target host=$PHANTOM_HOST port=$PHANTOM_PORT"

# Cheap reachability check first.
ASSERT_HTTP "http://$PHANTOM_HOST:$PHANTOM_PORT/healthz" 200 "remote serve healthz"
