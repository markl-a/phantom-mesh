...
# Use a path under $HOME/.phantom-mesh — both MSYS bash and the Windows
# native shell agree on $HOME, so there's no /tmp ↔ C:\tmp translation
# trap. Path is unique per run.
probe_a_dir="$HOME/.phantom-mesh/.test-anti-halluc-$$"
mkdir -p "$probe_a_dir"
sentinel="PHANTOM-TEST-SENTINEL-$$-$(date +%s)"
...
# shell phantom dispatches commands through.
...
before_ms=$(now_ms)
out_a=$("$PHANTOM_BIN" repl --agent master -c "$prompt_a" 2>&1 | sed -E 's/\x1b\[[0-9;]*m//g')
...
before_ms=$(now_ms)
out_b=$("$PHANTOM_BIN" repl --agent master -c "$prompt_b" 2>&1 | sed -E 's/\x1b\[[0-9;]*m//g')
