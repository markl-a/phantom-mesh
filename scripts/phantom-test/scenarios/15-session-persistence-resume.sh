...
# to find which file phantom wrote to. The id space is shared with the
# default cwd-hashed sessions, so we pick a clearly-test-prefixed name.
sid="phantom-test-resume-$$-$(date +%s)"
PHANTOM_CONFIG_DIR="${PHANTOM_CONFIG_DIR:-$HOME/.phantom-mesh}"
session_file="$PHANTOM_CONFIG_DIR/conversations/${sid}.jsonl"
...
out1=$("$PHANTOM_BIN" repl --agent master --session "$sid" -c \
...
out2=$("$PHANTOM_BIN" repl --agent master --session "$sid" -c \
