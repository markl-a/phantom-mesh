...
scenario "LLM roundtrip via phantom repl --agent master -c"
...
start_ts=$(date +%s)
out=$("$PHANTOM_BIN" repl --agent master -c "Reply with exactly the two characters: ok" 2>&1)
...
# Cost-line presence: phantom repl emits "[↑ $X.XXXX  ∑ $X.XXXX  Ts]" at end.
