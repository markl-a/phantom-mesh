...
# the FIRST tree whose core/src/tui.rs contains it. PHANTOM_TUI_TREE env
...
find_tree_with_marker() {
    ...
    if [ -n "${PHANTOM_TUI_TREE:-}" ]; then
        if [ -f "$PHANTOM_TUI_TREE/core/src/tui.rs" ] \
           && grep -q "$marker" "$PHANTOM_TUI_TREE/core/src/tui.rs" 2>/dev/null; then
            printf '%s\n' "$PHANTOM_TUI_TREE"
            return 0
        fi
    fi
...
run_cargo_for() {
    ...
    local target_dir="${CARGO_TARGET_DIR:-D:/tmp/phantom-windows-host-target}"
...
[ $failed -eq 0 ] && [ "$PHANTOM_TEST_FAILED" -eq 0 ]
