#!/usr/bin/env bash
# harness.sh — run spectyn-test scenarios and aggregate results.
#
# Usage:
#   scripts/spectyn-test/harness.sh                   # run all scenarios
#   scripts/spectyn-test/harness.sh 02 04             # run scenarios whose name starts with these prefixes
#   scripts/spectyn-test/harness.sh --list            # print scenario list, no execution
#
...
#   - calls assertion helpers (ASSERT_*); they update SPECTYN_TEST_PASSED/FAILED
...
export SPECTYN_TEST_LIB="$LIB_DIR"
...
LIST_ONLY=0
RETRY_FAILED=0
RETRY_COOLDOWN_S="${SPECTYN_TEST_RETRY_COOLDOWN_S:-30}"
PATTERNS=()
for arg in "$@"; do
  case "$arg" in
    --list|-l) LIST_ONLY=1 ;;
    ...
    --help|-h)
      ...
      printf '  SPECTYN_TEST_RETRY_COOLDOWN_S=<sec>  override the 30s cooldown\n'
      exit 0 ;;
    *) PATTERNS+=("$arg") ;;
  esac
done
