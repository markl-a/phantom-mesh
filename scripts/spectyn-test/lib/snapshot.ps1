...
#
# Output: writes PNG to $env:USERPROFILE\.spectyn-mesh\snapshots\<timestamp>.png
#         and prints the path to stdout (one line).
#
# Use from bash scenarios via:
#   snap=$(powershell -ExecutionPolicy Bypass -File lib/snapshot.ps1 -Window spectyn)
#
# Notes:
...
#   - For TUI capture, ensure spectyn's Windows Terminal pane is visible.

[CmdletBinding()]
param(
    [string]$Window = '',
    [string]$OutDir = (Join-Path $env:USERPROFILE '.spectyn-mesh\snapshots')
)
