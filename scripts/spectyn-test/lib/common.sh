...
# common.sh — shared helpers for spectyn-test scenarios.
...
SPECTYN_TEST_TMP="${TMPDIR:-/tmp}/spectyn-test-$$-${SPECTYN_TEST_NAME%.sh}"
mkdir -p "$SPECTYN_TEST_TMP"
trap 'rm -rf "$SPECTYN_TEST_TMP"' EXIT
...
pass() {
  SPECTYN_TEST_PASSED=$((SPECTYN_TEST_PASSED + 1))
  printf '  %s✓%s %s\n' "$C_GREEN" "$C_RESET" "$*"
}
fail() {
  SPECTYN_TEST_FAILED=$((SPECTYN_TEST_FAILED + 1))
  printf '  %s✗%s %s\n' "$C_RED" "$C_RESET" "$*" >&2
}
warn() {
  printf '  %s⚠%s %s\n' "$C_YELLOW" "$C_RESET" "$*" >&2
...
SPECTYN_BIN="${SPECTYN_BIN:-spectyn}"
SPECTYN_HOST="${SPECTYN_HOST:-127.0.0.1}"
SPECTYN_PORT="${SPECTYN_PORT:-7879}"
SPECTYN_CLUSTER_SECRET="${SPECTYN_CLUSTER_SECRET:-changeme-cluster-secret}"
