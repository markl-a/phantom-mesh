...
#
# Usage:
#   bash scripts/spectyn-test/scenarios/cross_host_dispatch.smoke.sh
#
# Env knobs:
#   SPECTYN_BIN_LOCAL    — path to spectyn binary (default: target/release or PATH)
...
# ~/.spectyn-mesh. Each "node" gets its own $HOME so its
# ~/.spectyn-mesh-e001-smoke maps to a different physical directory.
...
# Cleanup on exit: kill spawned spectyns and remove scratch.
cleanup() {
...
export SPECTYN_CLUSTER_SECRET="f002-smoke-secret"
...
# Tag the smoke run so SPECTYN_TEST_TMP-style assumptions stay scoped.
