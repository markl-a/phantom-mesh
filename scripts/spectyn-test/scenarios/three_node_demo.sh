...
# Sibling : scripts/spectyn-test/scenarios/cross_host_dispatch.sh (F002, 2 nodes)
#
# What it proves
# --------------
# Given THREE spectyn-mesh nodes on the same Tailscale tailnet with NON-
...
# `setup-android-termux.sh`, and the workstation already has spectyn). We
# only POST dispatches and assert.
#
# Environment contract (REQUIRED)
# -------------------------------
#   SPECTYN_NODE_A_URL       — Workstation base URL, e.g.
#                              http://workstation.tail.ts.net:7878
#   SPECTYN_NODE_B_URL       — Cloud  base URL, e.g. http://cloud.tail.ts.net:7878
#   SPECTYN_NODE_C_URL       — Mobile base URL, e.g. http://mobile.tail.ts.net:7878
#   SPECTYN_CLUSTER_SECRET   — same 64-hex secret all three nodes were configured with
#
# Environment contract (OPTIONAL)
# -------------------------------
#   SPECTYN_NODE_A_NAME      — workstation's node_name (default: "workstation")
#   SPECTYN_NODE_B_NAME      — cloud's node_name       (default: "cloud")
#   SPECTYN_NODE_C_NAME      — mobile's node_name      (default: "mobile")
#   SPECTYN_DISPATCH_TIMEOUT_S — per-dispatch curl timeout (default: 20)
#   SPECTYN_HEALTHZ_TIMEOUT_S  — preflight /healthz timeout (default: 15)
#
# Exit codes
# ----------
...
# Operator workflow
# -----------------
#   export SPECTYN_NODE_A_URL=http://workstation.tail.ts.net:7878
#   export SPECTYN_NODE_B_URL=http://cloud.tail.ts.net:7878
#   export SPECTYN_NODE_C_URL=http://mobile.tail.ts.net:7878
#   export SPECTYN_CLUSTER_SECRET=$(cat ~/.spectyn-mesh/cluster_secret)
#   bash scripts/spectyn-test/scenarios/three_node_demo.sh
#
# ─────────────────────────────────────────────────────────────────────────────
...
REQUIRED_ENV="SPECTYN_NODE_A_URL SPECTYN_NODE_B_URL SPECTYN_NODE_C_URL SPECTYN_CLUSTER_SECRET"
missing=""
...
# Defaults
NODE_A_NAME="${SPECTYN_NODE_A_NAME:-workstation}"
NODE_B_NAME="${SPECTYN_NODE_B_NAME:-cloud}"
NODE_C_NAME="${SPECTYN_NODE_C_NAME:-mobile}"
HEALTHZ_TIMEOUT="${SPECTYN_HEALTHZ_TIMEOUT_S:-15}"
DISPATCH_TIMEOUT="${SPECTYN_DISPATCH_TIMEOUT_S:-20}"

step "node A (workstation, gpu)        : $NODE_A_NAME @ $SPECTYN_NODE_A_URL"
step "node B (cloud,       always_on)  : $NODE_B_NAME @ $SPECTYN_NODE_B_URL"
step "node C (mobile,      mobile)     : $NODE_C_NAME @ $SPECTYN_NODE_C_URL"
...
preflight_one "$NODE_A_NAME" "$SPECTYN_NODE_A_URL"
preflight_one "$NODE_B_NAME" "$SPECTYN_NODE_B_URL"
preflight_one "$NODE_C_NAME" "$SPECTYN_NODE_C_URL"
...
for U in "$SPECTYN_NODE_A_URL" "$SPECTYN_NODE_B_URL" "$SPECTYN_NODE_C_URL"; do
...
run_dispatch_test "dispatch 1" "$NODE_A_NAME" "$SPECTYN_NODE_A_URL" '["always_on"]' "$NODE_B_NAME"
# 2. From workstation, require mobile → mobile
run_dispatch_test "dispatch 2" "$NODE_A_NAME" "$SPECTYN_NODE_A_URL" '["mobile"]'    "$NODE_C_NAME"
# 3. From mobile, require gpu → workstation
run_dispatch_test "dispatch 3" "$NODE_C_NAME" "$SPECTYN_NODE_C_URL" '["gpu"]'       "$NODE_A_NAME"
# 4. From cloud, require camera → mobile
run_dispatch_test "dispatch 4" "$NODE_B_NAME" "$SPECTYN_NODE_B_URL" '["camera"]'    "$NODE_C_NAME"
...
printf '%s  Routed %d tasks across 3 nodes / 3 architectures /          %s\n' \
       "$C_GREEN" "$SPECTYN_TEST_PASSED" "$C_RESET"
