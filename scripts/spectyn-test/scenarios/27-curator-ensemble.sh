#!/usr/bin/env bash
# 27-curator-ensemble.sh — verify `spectyn evolve --judge --ensemble N`
...
# EvolveCheckpoint.
#
# Gated behind SPECTYN_HERMES_ENSEMBLE=1 — like scenario 26, this is opt-in
...
scenario "spectyn evolve --judge --ensemble N writes a judge_ensemble verdict"

if [ "${SPECTYN_HERMES_ENSEMBLE:-0}" != "1" ]; then
  warn "skip: set SPECTYN_HERMES_ENSEMBLE=1 to enable (binary must also be built with --features experimental-curator)"
  exit 77
fi
...
step "running spectyn evolve --max-rounds 1 --judge --ensemble 3 (180s hard cap)"
timeout 180 "$SPECTYN_BIN" evolve --max-rounds 1 --judge --ensemble 3 \
  "echo hello world; this is a no-op evolve goal for the ensemble scenario" \
  > "$SPECTYN_TEST_TMP/ensemble-stdout.log" 2>&1 || true
