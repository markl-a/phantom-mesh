...
# then verify spectyn returns a non-zero exit + a recognizable error message
# rather than panicking or hanging. We do NOT touch global env — the unset
# scope is just this subprocess.

step "running spectyn repl with all common LLM keys unset"
...
out=$(
  env -u ANTHROPIC_API_KEY \
      ...
      -u OPENCODE_API_KEY \
    "$SPECTYN_BIN" repl --agent master -c "ping" 2>&1
)
ec=$?
set -u
