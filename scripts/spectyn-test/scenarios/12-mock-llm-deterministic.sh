...
# Build a temp cwd with the mock agents.toml; spectyn prefers ./agents.toml
# over $HOME/.spectyn-mesh/agents.toml.
agents_dir=$(mock_temp_agents_dir)
step "using temp agents.toml at $agents_dir"

# ── Test 1: ping → pong ────────────────────────────────────────────────────
step "test 1: 'ping' should produce scripted 'pong'"
out=$(cd "$agents_dir" && "$SPECTYN_BIN" repl --agent master -c "ping" 2>&1 \
      | sed -E 's/\x1b\[[0-9;]*m//g')
ASSERT_CONTAINS "$out" "pong" "ping → pong"

# ── Test 2: '2 + 2' regex match → '4' ──────────────────────────────────────
step "test 2: '2 + 2' should produce scripted '4'"
out=$(cd "$agents_dir" && "$SPECTYN_BIN" repl --agent master -c "What is 2 + 2?" 2>&1 \
      | sed -E 's/\x1b\[[0-9;]*m//g')
...
out=$(cd "$agents_dir" && "$SPECTYN_BIN" repl --agent master -c "this prompt has no match" 2>&1 \
      | sed -E 's/\x1b\[[0-9;]*m//g')
