...
log="$SPECTYN_CONFIG_DIR/autoevolve.log"
prev_lines=$(wc -l < "$log" 2>/dev/null | tr -d ' \n')
prev_lines="${prev_lines:-0}"

step "running spectyn autoevolve --once --target check (60s hard cap)"
ts=$(date +%s)
...
out=$(timeout 60 "$SPECTYN_BIN" autoevolve --once --target check 2>&1 \
      | sed -E 's/\x1b\[[0-9;]*m//g')
ec=$?
elapsed=$(( $(date +%s) - ts ))
