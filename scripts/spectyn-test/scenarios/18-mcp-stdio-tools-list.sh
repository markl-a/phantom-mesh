...
scenario "spectyn mcp — stdio server returns initialize + tools/list"
...
step "spawning spectyn mcp + writing 3 JSON-RPC frames"
out=$(printf '%s\n%s\n%s\n' \
    '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"spectyn-test","version":"1.0"}}}' \
    '{"jsonrpc":"2.0","method":"notifications/initialized"}' \
    '{"jsonrpc":"2.0","id":2,"method":"tools/list"}' \
    | timeout 10 "$SPECTYN_BIN" mcp 2>&1)
ec=$?
step "spectyn mcp exit=$ec"
