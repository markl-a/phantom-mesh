...
#
# Verifies:
#   1. `spectyn keys list` accepts / surfaces *_API_KEY entries for the 4 H4
#      providers (Mistral, xAI, Together, Fireworks)
#   2. for at least ONE provider whose API key IS in the operator's env, a
#      `spectyn repl --agent master -c "<short prompt>"` round-trip through
...
#   tight signal is: spectyn's response to `provider.type = "mistral"` in
...
scenario "H4 — spectyn keys lists 4 new providers + at least 1 round-trip"
require_cmd "$SPECTYN_BIN"

# ── Step 1: spectyn keys list — must run cleanly ────────────────────────────
keys_list=$("$SPECTYN_BIN" keys list 2>&1 | sed -E 's/\x1b\[[0-9;]*m//g')
keys_ec=$?
if [ "$keys_ec" -ne 0 ]; then
  fail "spectyn keys list exited non-zero ($keys_ec) — basic CLI sanity broken"
  printf '%s\n' "$keys_list" | head -10 | sed 's/^/    /'
  exit 1
fi
pass "spectyn keys list runs cleanly"
...
# Which H4 *_API_KEY entries are visible to spectyn (live env OR saved)?
declare -a HAVE_KEYS=()
...
for p in "${H4_PROVIDERS[@]}"; do
  ...
  if [ -n "${!env_var:-}" ]; then
    ...
    HAVE_KEYS+=("$p")
    pass "  $p: saved key for $env_var (will be auto-loaded by spectyn)"
  else
    MISSING_KEYS+=("$p")
...
if [ "${#HAVE_KEYS[@]}" -eq 0 ]; then
  ...
  warn "      ~/.spectyn-mesh/env. Set at least one to run the round-trip:"
  warn "      e.g.  spectyn keys set mistral <key>"
  exit 77
fi
...
step "calling spectyn repl --agent master -c '<short>' against $TARGET (90s cap)"
out=$( cd "$WORKDIR" && timeout 90 "$SPECTYN_BIN" repl --agent master -c "Reply with exactly the two characters: ok" 2>&1 \
       | sed -E 's/\x1b\[[0-9;]*m//g' )
...
if printf '%s' "$out" | grep -qiE 'unknown provider type|no such provider|provider type.*not.*found'; then
  warn "skip: SPECTYN_BIN does not recognize provider type '$TARGET' — pre-H4 build."
...
wrong_url=$(printf '%s' "$out" \
  ...
  | grep -v -F "$expected" \
  | grep -vE 'spectyn-mesh|localhost|127\.0\.0\.1' \
  | head -1 || true)
...
# Provider-side soft-fails — independent of spectyn plumbing.
if printf '%s' "$out" | grep -qiE 'HTTP 429|rate.?limit|quota|usage limit|too many requests'; then
  warn "skip: $TARGET returned rate-limit / quota — provider-side, not spectyn-side"
  exit 77
fi
if printf '%s' "$out" | grep -qiE 'HTTP 401|invalid.*api.*key|authentication.*failed|unauthorized'; then
  warn "skip: $TARGET returned auth failure — key may be invalid; not spectyn-side"
  exit 77
fi
