...
#   - SPECTYN_BIN doesn't recognize 'mistral' → exit 77 (pre-H4 binary; T11
...
#     spectyn call returned an error                regression we're guarding)
#
# References:
...
class H(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        # client, spectyn reports "stream completed with no content".
        want_stream = False
        try:
        ...
        if want_stream:
            ...
            for piece in ("o", "k"):
                ...
                self.wfile.flush()
            # Final chunk with usage so spectyn records cost.
...
# ── Drive a single spectyn call ────────────────────────────────────────────
step "spectyn repl --agent master -c '...' (60s cap; expect 1×429 then 1×200)"
start_ts=$(date +%s)
out=$( cd "$WORKDIR" && timeout 60 "$SPECTYN_BIN" repl --agent master -c "Reply with exactly the two characters: ok" 2>&1 \
       | sed -E 's/\x1b\[[0-9;]*m//g' )
...
if [ "$attempts" -eq 0 ]; then
  ...
  if printf '%s' "$out" | grep -oE 'https?://[A-Za-z0-9./_-]+' \
       | grep -qvE "127\.0\.0\.1|localhost|spectyn-mesh"; then
...
# ── And the user-facing outcome: spectyn should report success ──────────────
if printf '%s' "$out" | grep -qiE 'HTTP 429|rate.?limit|too many requests' \
  ...
  pass "spectyn emitted a cost line — caller saw the 200, not the 429"
else
