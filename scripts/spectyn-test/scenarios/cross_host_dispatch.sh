...
#   1. SCPs a pre-built spectyn binary to each node (no compile-on-Pi).
...
#            SPECTYN_FORWARD_ON_CAPS_MISMATCH=1 in the launch env.
#   3. Starts `spectyn serve` on each node (nohup + PID file), waits for
...
#   6. Greps both nodes' spectyn serve logs for the forwarding audit event
#      ("spectyn::dispatch::forward" target) — defence in depth.
#   7. On failure: prints last 50 lines of BOTH nodes' spectyn serve logs,
#      then exits non-zero with a `FAIL: <reason>` line.
#   8. Cleanup: kills `spectyn serve` on both nodes (via PID files), leaves
...
#   SPECTYN_NODE_A_PORT      — Node A's serve port (default: 7878)
#   SPECTYN_NODE_B_PORT      — Node B's serve port (default: 7878)
...
#                              has already started both `spectyn serve`
#                              instances by hand). Useful for fast re-runs.
#   SPECTYN_CLEANUP_SKIP=1   — leave both `spectyn serve` instances running
...
#   bash scripts/spectyn-test/scenarios/cross_host_dispatch.sh
#
# Local smoke (CI / dev): use the sibling
#   scripts/spectyn-test/scenarios/cross_host_dispatch.smoke.sh
# which spins up two `spectyn serve` processes on 127.0.0.1 and reuses this
# script with SPECTYN_NODE_*_SSH set to a local-exec shim.
#
...
#   SPECTYN_FORWARD_ON_CAPS_MISMATCH=1; this script sets the gate on
...
step "node A: $NODE_A_NAME @ $SPECTYN_NODE_A_URL (SSH: $SPECTYN_NODE_A_SSH)"
step "node B: $NODE_B_NAME @ $SPECTYN_NODE_B_URL (SSH: $SPECTYN_NODE_B_SSH)"
...
# Honours $SPECTYN_SCP so the smoke harness can sub in `cp` (in which case
...
title "F002 · sweep — kill any stale spectyn serve from a prior run"
sweep_one() {
  LBL="$1"; SSHP="$2"
...
# ── Step 3: start spectyn serve on both nodes ───────────────────────────────
title "F002 · launch — spectyn serve on both nodes"
...
launch_one() {
  ...
  # Determine the spectyn binary name to invoke (Windows smoke: .exe).
  bin_name="spectyn"
...
# `spectyn serve` pings peers every 30s. To avoid waiting we hit /rpc/ping
...
RESP_AB=$(dispatch_with_caps "$SPECTYN_NODE_A_URL" '["vision"]' \
...
RESP_BA=$(dispatch_with_caps "$SPECTYN_NODE_B_URL" '["gpu"]' \
...
# The tracing target `spectyn::dispatch::forward` (mesh.rs) is emitted on the
...
audit_one() {
  LBL="$1"; SSHP="$2"
  step "node $LBL: scanning ~/$REMOTE_HOME/spectyn-serve.log for forwarding evidence"
...
if [ "${SPECTYN_CLEANUP_SKIP:-0}" != "1" ]; then
  title "F002 · cleanup — killing spectyn serve on both nodes (logs retained)"
  cleanup_one() {
    LBL="$1"; SSHP="$2"
    remote_run "$SSHP" "
      pidf=\$HOME/$REMOTE_HOME/spectyn-serve.pid
