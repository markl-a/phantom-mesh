...
EVENTS="$SPECTYN_CONFIG_DIR/events.jsonl"
COSTS_DB="$SPECTYN_CONFIG_DIR/costs.db"
COSTS_JSON="$SPECTYN_CONFIG_DIR/costs.json"

if [ ! -f "$EVENTS" ]; then
  warn "skip: $EVENTS not found — run any spectyn command first to seed config dir"
  exit 77
fi
...
# Helper: ask spectyn to call its master agent with the long prompt; return
# trimmed transcript. Strips ANSI for grep-ability.
call_master() {
  local label="$1"
  step "$label: spectyn repl --agent master -c '<long prompt>'"
  ( cd "$WORKDIR" && timeout 90 "$SPECTYN_BIN" repl --agent master -c "$LONG_PROMPT" 2>&1 ) \
    | sed -E 's/\x1b\[[0-9;]*m//g'
}
...
detect_rate_limit_or_skip() {
  ...
  if printf '%s' "$body" | grep -qiE 'HTTP 429|rate.?limit|quota|usage limit|too many requests|overloaded'; then
    warn "$label: provider rate-limit / overloaded — skipping (not a spectyn-side regression)"
    exit 77
  fi
...
if [ "$events_have_cache" -eq 0 ] && [ "$costs_have_cache" -eq 0 ] && [ "$db_has_cache" -eq 0 ]; then
  ...
  warn "      Rebuild SPECTYN_BIN from a post-PR-#31 commit to enable this scenario."
  exit 77
fi
...
if [ "$read_tokens_call2" -gt 0 ]; then
  ...
  warn "    propagation delay. Inspect manually: spectyn debug --tail 50"
