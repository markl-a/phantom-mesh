# spectyn-mesh 評測框架（eval harness，採用 promptfoo，offline-first 離線優先）

一套小型的 [promptfoo](https://promptfoo.dev) 評測套件，針對 spectyn-mesh 的
...
asserts（斷言）（`contains`、`regex`、`is-json`、`javascript`）來驗證 spectyn *自身*的
...
- `run.sh` 會匯出 `PROMPTFOO_DISABLE_TELEMETRY=1` 與 `PROMPTFOO_DISABLE_UPDATE=1`
  —— 沒有分析數據、沒有更新檢查的網路請求。
- 預設的 provider 是一個本機 Node harness（`spectyn_harness.js`），它驅動
  本機的 `spectyn` CLI 以及一個純 in-process（行程內）的 route resolver（路由解析器）。**預設案例不會碰到任何
  網路、LLM 或 API key**。
...
|---|------------|-----------------|--------------|
| 1 | `version` | `spectyn --version` 輸出一個格式正確的 `spectyn <semver> (<sha>+, <target>, built <date>)` 橫幅（banner） | `contains`、`regex` |
| 2 | `help` | 頂層 `spectyn --help` 揭示核心命令面（`spectyn exec` / `spectyn serve` / `spectyn selftest`） | `contains` x3 |
| 3 | `exec-help` | `spectyn exec --help` 描述 headless（無介面）CI 契約：從 `stdin` 讀取 prompt、支援 `--json` 機器可讀輸出、標記為 "Headless" | `contains`、`regex` |
...
案例 4 與 5 編碼了 spectyn 已記載的 `provider:model` 優先序規則
（`agent.X.model` / `provider:model` 語法會釘選一個特定模型；裸 provider 則
使用其 `default_model`）。它們完全在行程內執行，零 subprocess（子行程）且
零網路，所以即使 `spectyn` 二進位檔不存在它們也會通過。

## 需求

- **Node.js**（給 harness 與 promptfoo 使用）。
- 案例 1-3 需要 **`spectyn` CLI 在 PATH 上**。如果 `spectyn` 不在 PATH 上但位於
  `~/.cargo/bin/spectyn`，`run.sh` 會自動把它加進去。你也可以用
  `SPECTYN_BIN=/path/to/spectyn` 讓 harness 指向某個特定的二進位檔。如果 `spectyn`
...
- `spectyn_harness.js` —— 離線 Node provider；驅動 `spectyn` CLI 與
