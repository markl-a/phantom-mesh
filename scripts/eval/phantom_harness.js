// phantom_harness.js — deterministic, OFFLINE provider harness for promptfoo.
//
// This is a promptfoo custom JS provider (referenced as `file://phantom_harness.js`).
// It does NOT call any LLM and never touches the network. Instead it drives the
// local `phantom` CLI (or a stubbed binary via PHANTOM_BIN) for commands that
...
//   - "version"   → `phantom --version`           (build/version banner)
//   - "help"      → `phantom --help`               (top-level command surface)
//   - "exec-help" → `phantom exec --help`          (headless run contract)
...
const PHANTOM_BIN = process.env.PHANTOM_BIN || 'phantom';
...
function runPhantom(args) {
  const r = spawnSync(PHANTOM_BIN, args, {
    encoding: 'utf8',
    timeout: 20000,
  ...
  // phantom writes --help / --version banners to STDERR (and exits 0), while
...
// Pure, dependency-free model of phantom's provider:model precedence rule
...
class PhantomOfflineProvider {
  constructor(options) {
    this.options = options || {};
    this.providerId =
      (this.options.id && String(this.options.id)) || 'phantom-offline-harness';
  }
  ...
  async callApi(prompt, context) {
    ...
    const res = runPhantom(args);
    if (!res.ok) {
      return { error: `phantom ${args.join(' ')} failed: ${res.text}` };
    }
    return { output: res.text };
  }
}

module.exports = PhantomOfflineProvider;
