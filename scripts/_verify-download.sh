...
#                                  SPECTYN_ALLOW_INSECURE=1 is set)
...
#   SPECTYN_ALLOW_INSECURE=1  — allow plain http:// download URLs
#   SPECTYN_SKIP_VERIFY=1     — skip SHA256 verification entirely (loudly warns)

set -u

# Detect which sha256 tool is available. We do this once per source.
_SPECTYN_SHA256_TOOL=""
if command -v sha256sum >/dev/null 2>&1; then
  _SPECTYN_SHA256_TOOL="sha256sum"
elif command -v shasum >/dev/null 2>&1; then
  _SPECTYN_SHA256_TOOL="shasum -a 256"
else
  ...
  case "$url" in
    ...
    http://*)
      if [ "${SPECTYN_ALLOW_INSECURE:-0}" = "1" ]; then
        echo "    ⚠ SPECTYN_ALLOW_INSECURE=1 — accepting plain http:// URL ($url)" >&2
        echo "      THIS DISABLES MITM PROTECTION. Set SPECTYN_ALLOW_INSECURE=0 to" >&2
        echo "      force HTTPS." >&2
        return 0
      fi
      echo "    ✗ Refusing to download binary over plain http://" >&2
      echo "      URL: $url" >&2
      echo "      Use an https:// URL, or set SPECTYN_ALLOW_INSECURE=1 explicitly" >&2
...
sha256_local() {
  ...
  local path="$1"
  if [ -z "$_SPECTYN_SHA256_TOOL" ]; then
    echo "    ✗ sha256 tool not available — cannot verify $path" >&2
    return 1
  fi
  $_SPECTYN_SHA256_TOOL "$path" | awk '{print tolower($1)}'
}
...
verify_sha256() {
  ...
  if [ "${SPECTYN_SKIP_VERIFY:-0}" = "1" ]; then
    echo "    ⚠ SPECTYN_SKIP_VERIFY=1 — SKIPPING SHA256 verification of $bin" >&2
    echo "      This means a MITM or compromised mirror can replace the" >&2
    echo "      spectyn binary. Do not use except on an air-gapped first install" >&2
  ...
  local sums_file
  sums_file="$(mktemp -t spectyn-sha256.XXXXXX 2>/dev/null || mktemp)"
  ...
  if ! curl -fsSL --max-time 30 "$sums_url" -o "$sums_file"; then
    ...
    echo "      If you genuinely need to bypass this, set SPECTYN_SKIP_VERIFY=1" >&2
