...
#
# Runs the cheap, offline `phantom test gate map --check` resolve-lint and asserts
...
selftest_feature_meta() {
  ...
  echo "priority=P1"
  echo "requires="
  echo "description=SPEC-60 ship-gate-map resolve-lint (phantom test gate map --check)"
  echo "hints=docs/superpowers/specs/v060-deep-spec/appendix/ship-gate-map.toml core/src/test_report/mod.rs"
}

selftest_run() {
  local out rc
  out=$("$PHANTOM" test gate map --check 2>&1)
  rc=$?
  T_ARTIFACT_TEXT="$out"
  T_REPRO="$PHANTOM test gate map --check"
  if [ "$rc" -eq 0 ]; then
