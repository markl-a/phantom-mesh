# Spectyn Mesh - Windows native build helper
#
# Wraps `cargo build --release --bin spectyn` with the gotchas the
# node-a (Windows + Android-APK build host) session needs:
#
#   1. Stops any running spectyn.exe first - Windows refuses to overwrite
#      a .exe held open by Defender real-time scan or by an active
#      `spectyn serve` process.
#   2. Sets CARGO_TARGET_DIR outside the worktree (default
#      D:\tmp\spectyn-windows-target). The worktree's own
...
#   3. Optionally deploys to ~/.spectyn-mesh/bin/spectyn.exe (the path
#      install-spectyn-windows.ps1 + `spectyn service install` both
#      use) and ~/.local/bin/spectyn.exe (so PowerShell PATH picks it
#      up without further wiring).
#
# Usage:
#   ./scripts/build-windows.ps1                       # build + verify
#   ./scripts/build-windows.ps1 -Deploy               # build + copy to ~/.spectyn-mesh/bin + ~/.local/bin
...
Write-Host "=== Spectyn Mesh Windows build ===" -ForegroundColor Cyan
Write-Host ""
Write-Host "  repo        : $RepoRoot"
Write-Host "  CARGO_TARGET: $TargetDir"
Write-Host ""

# -- Stop any running spectyn.exe so the link step doesn't hit access-denied --
$running = Get-Process spectyn -ErrorAction SilentlyContinue
if ($running) {
    Write-Host "[pre] Stopping $($running.Count) running spectyn.exe instance(s)..." -ForegroundColor Yellow
...
Write-Host "[build] cargo build --release --bin spectyn" -ForegroundColor Cyan
Push-Location $CoreDir
try {
    cargo build --release --bin spectyn --message-format=short
    if ($LASTEXITCODE -ne 0) { throw "cargo build failed (exit $LASTEXITCODE)" }
} finally {
    Pop-Location
}

$exe = Join-Path $TargetDir 'release\spectyn.exe'
...
if ($Deploy) {
    ...
    $serveDir = Join-Path $env:USERPROFILE '.spectyn-mesh\bin'
    $localDir = Join-Path $env:USERPROFILE '.local\bin'
    New-Item -ItemType Directory -Force $serveDir | Out-Null
    New-Item -ItemType Directory -Force $localDir | Out-Null
    Copy-Item $exe (Join-Path $serveDir 'spectyn.exe') -Force
    Copy-Item $exe (Join-Path $localDir 'spectyn.exe') -Force
    Write-Host "  [+] $serveDir\spectyn.exe (used by SpectynServe Scheduled Task)" -ForegroundColor Green
    Write-Host "  [+] $localDir\spectyn.exe (on PowerShell User PATH)" -ForegroundColor Green

    Write-Host ""
    Write-Host "[verify] spectyn --version" -ForegroundColor Cyan
    & (Join-Path $localDir 'spectyn.exe') --version
}
