#!/usr/bin/env bash
# spectyn Mac — end-to-end test sweep.
#
# Runs every automatable check across the macOS surface spectyn ships.
# Prints PASS / FAIL / SKIP per row + final summary.
#
# Usage:
#   ./scripts/test-mac.sh
#
# Read-only; safe to run while spectyn serve is live.
...
echo "═══ spectyn Mac end-to-end test sweep ═══"
echo "  binary  : $SPECTYN"
echo "  coord   : $COORD"
...
for sec in binary config "provider keys" "spectyn serve" network tools "MLX local LLM" autoevolve "macOS integrations"; do
...
if grep -q "registered : .*yes" "$TMP/svc"; then
  ...
  fail "service registered" "spectyn service install"
fi
...
if launchctl print "gui/$(id -u)/ai.spectynmesh.serve" >/dev/null 2>&1; then
  ok "launchctl print works" "gui/$(id -u)/ai.spectynmesh.serve"
else
...
for f in spectyn-aarch64-apple-darwin spectyn-aarch64-linux-android spectyn-x86_64-pc-windows.exe spectyn-mesh-android.apk spectyn-mesh-ios.ipa; do
...
for f in termux-setup.sh windows-bootstrap.ps1 install-spectyn-windows.ps1; do
...
if probe tmutil; then
  ok "tmutil present" ""
  "$SPECTYN" snapshot list > "$TMP/snaps" 2>&1
  ...
  if "$SPECTYN" snapshot apply 1999-01-01-000000 --cwd 2>&1 | grep -q "not found"; then
