...
# Uses a throwaway HOME — does NOT touch your real ~/.spectyn-mesh.
# Run from the repo root:  bash scripts/verify-onboarding-gate.sh
set -u
cd "$(dirname "$0")/.." || exit 1

echo "▶ building spectyn (debug)…"
( cd core && cargo build --bin spectyn 2>/dev/null ) || { echo "build failed"; exit 1; }
BIN="$PWD/core/target/debug/spectyn"

H=$(mktemp -d); C=$(mktemp -d)            # throwaway HOME + project dir
mkdir -p "$H/.spectyn-mesh"; printf 'x' > "$H/.spectyn-mesh/identity.key"
cfg="$H/.spectyn-mesh/agents.toml"
say() { printf '\n\033[36m== %s ==\033[0m\n' "$1"; }
run() { ( cd "$C" && HOME="$H" "$BIN" "$@" ); }
...
say "escape hatch  SPECTYN_TRUST_ALL=1  → expect write allowed"
esc=$( cd "$C" && SPECTYN_TRUST_ALL=1 HOME="$H" "$BIN" tool file_write \
...
rm -rf "$H" "$C"
echo; echo "✓ done — temp dirs cleaned; your ~/.spectyn-mesh was untouched."
