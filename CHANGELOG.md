...
- **HIGH（高風險）** `core/src/tools/web_fetch.rs`、`core/src/tools/http_client.rs::{get,post}`：新增 SSRF（伺服器端請求偽造）防護；設定 `PHANTOM_FETCH_ALLOW_LOCAL=1` 可重新允許（opt back in）（T13-N6）。
...
- 新增共用模組 `core/src/auth_gate.rs`，將 `require_cluster_auth` 抽出，讓 `phantom serve` 路由器（router）與守護程序（daemon）路由器共用同一份 HMAC 檢查。
...
**TUI（終端機使用者介面）** — `phantom tui` 開啟一個全螢幕 ratatui 介面（持續存在的輸入框、可捲動的對話記錄、狀態列、斜線指令）

**自我迭代（Self-iteration）** — `phantom evolve` 已在此 repo 上端到端（end-to-end）驗證：在 Groq 免費方案（free tier）以 $0 成本自主修正了一個 `core/src/cost.rs` 警告（見 `docs/SELF-EVOLVE.md`）
...
- **瀏覽器式上手（onboarding）**（`phantom onboarding`）— 自動啟動 serve、開啟瀏覽器至設定頁、等待設定寫入
- **內嵌式 web 儀表板**（`phantom serve` → `http://localhost:7878`）：
...
- **MCP stdio 伺服器**（`phantom mcp`） — 可隨插即用（drop-in）的 subagent（子代理），適用於 Claude Code、Cursor、任何 MCP 客戶端
- **Codex 相容的 WebSocket JSON-RPC**（`phantom serve` → `ws://host:7878/ws`）
...
- `phantom evolve --distributed` 跨 cluster 的平行 agent 群集（swarm）
- `phantom coordinator` 透過 mDNS 的零設定（zero-config）對等節點探索
- 持久化的對話歷史（每個 chat_id 一份 JSONL）

### 新增 — 整合（integrations）
- Telegram 機器人頻道，含使用者允許清單（allowlist）
- PHANTOM.md 與 AGENTS.md 專案上下文檔案（自工作目錄自動載入）
