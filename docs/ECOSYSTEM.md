# Phantom Mesh Ecosystem — 7 個衛星項目

> **底層 = phantom-mesh,衛星 = 6 個專業領域延伸 + 1 個對外網站**
...
### ② [phantom-training](https://github.com/markl-a/phantom-training)

**Agentic post-training orchestrator on phantom-mesh.**

跑在 phantom-mesh 上的 AI agent 自動 fine-tune / 蒸餾 / benchmark 自己的小模型。
站在 Unsloth + Axolotl 巨人肩膀,加 phantom 的 multi-agent + cross-device 優勢。
...
### ③ [phantom-ai-feed](https://github.com/markl-a/phantom-ai-feed)
...
### ④ [phantom-secure-connector](https://github.com/markl-a/phantom-secure-connector)

**PHI redactor + time-series anomaly + compliance check + red/blue team simulation + MCP bridge.**

phantom-mesh 的安全資料 + 工具暴露套件,讓敏感資料安全進來、可信工具安全出去。

- **包含**:
  - PHI redactor(從 Medical RAG 抽出 sanitized)
  - anomaly detector(從 AHI Detection v2 通用化)
  - compliance checker(SOC2 / HIPAA / GDPR / 個資法)
  - secops simulator(從 [phantom-secops](https://github.com/markl-a/phantom-secops) 整合)
  - MCP bridge(phantom → Claude Desktop / Cursor)
- **目標客戶**:趨勢科技 / 中信 / 國泰 / 富邦 / 醫材公司 / 工研院生醫 / 鼎新

### ⑤ [phantom-enterprise](https://github.com/markl-a/phantom-enterprise)

**LDAP / SSO / VPN / MES / ERP / Confluence 企業 on-prem connector pack。**

phantom-mesh 在企業內網無痛跑 — 接公司 SSO、跨企業 VPN routing、串接 ERP/MES。

- **連接器**:LDAP / SAML 2.0 / OIDC / Tailscale-aware / 鼎新 T100 / 鴻海 MES / GitLab on-prem
- **目標客戶**:鼎新 / 中信 / 國泰 / 鴻海 / 聯發科 / 中型 SI

### ⑥ [phantom-flow](https://github.com/markl-a/phantom-flow)
...
phantom-mesh 上的 n8n / Zapier —從 phantom 的 trigger / RAG / skill 組成 pipeline,自動對外執行。
...
### ⑦ [phantom-companion](https://github.com/markl-a/phantom-companion)

**Personal behavior analytics + LLM insight + proactive optimization(Life Track 主推)。**

不只是 daily coach — phantom-companion 觀察你怎麼用 phantom + sensor 資料,
...
1. **手動觸發** `phantom sync release <repo>` — sanitize + push
...
```
M1 ──── W1 phantom-mesh polish + markl-ai.space 升級
        W2-4 ⑥ phantom-flow(已有最完整改裝資產)

M2 ──── W5-7 ④ phantom-secure-connector(招聘覆蓋最廣)
        W8 ③ phantom-ai-feed MVP

M3 ──── W9-10 ⑦ phantom-companion(需 phantom 跑 2 月後有資料)
        W11-12 ② phantom-training

M4 ──── W13-14 ⑤ phantom-enterprise
        W15-16 7 篇英文 blog + 對外發布 + 衝 star
```

## 為什麼 phantom-mesh 是必要核心

- 跨衛星共用 FTS5 → 跨領域 insight(phantom-companion 用 phantom-ai-feed 學的 + phantom-flow 觸發紀錄 + phantom-secure-connector 健康)
- 跨衛星共用 provider trait → 全域 LLM 成本 view
- 跨衛星共用 evolve loop → 用 phantom 一段時間後,所有衛星都「更懂你」
- **如果衛星各自重做底層,我們會變成另一個 LangChain ecosystem**(分散、無 cross-vertical insight)。phantom-mesh 是這個生態系的 keystone
...
| # | 招聘 | 副業 | 人生 | 總分 |
|---|---|---|---|---|
| ① phantom-mesh | 5 | 4 | 5 | **14** ⭐ |
| ② phantom-training | 5 | 4 | 3 | 12 |
| ③ phantom-ai-feed | 3 | 4 | 5 | 12 |
| ④ phantom-secure-connector | 5 | 4 | 5 | **14** ⭐ |
| ⑤ phantom-enterprise | 5 | 3 | 2 | 10 |
| ⑥ phantom-flow | 4 | 5 | 5 | **14** ⭐ |
| ⑦ phantom-companion | 4 | 3 | 5 | 12 |

**三個 14 分項目**:phantom-mesh、phantom-secure-connector、phantom-flow → 主力 ship 序。
