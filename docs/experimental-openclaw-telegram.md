...
**狀態：** experimental（實驗性）— 等待 PR #28（`wt/openclaw-telegram`）合併。
**Cargo feature（功能旗標）：** `experimental-openclaw-telegram`（會拉入 `teloxide`）。
**程式碼狀態：** 模組檔 `core/src/openclaw/telegram.rs` 存在於
`wt/openclaw-telegram` 分支，但尚未進入 `main`。`core/Cargo.toml` 中的
feature（功能）那一行目前是被註解掉的。

## 它的功能（PR #28 合併之後）

OpenClaw 的 Telegram channel adapter（頻道轉接器）。實作了：
...
pub struct OpenclawTelegramBot { /* opaque */ }
