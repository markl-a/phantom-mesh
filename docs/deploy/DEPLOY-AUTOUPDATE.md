...
> **範圍**：本檔專講「**簽章金鑰 + tag 觸發的 release CI + Tauri 自動更新（OTA）**」這條發布鏈
> ── GitHub Secrets 一次性設定、各平台簽章憑證、`git tag` 觸發 release workflow、Oracle VM systemd 服務。
>
> 要把節點實際部署起來、組成 mesh（coordinator / 本地 worker / 雲端節點 / Tailscale / Telegram 行動存取、
> 完整逐步驗收與疑難排解），請看 canonical 部署指南 **[DEPLOYMENT.md](DEPLOYMENT.md)**。
