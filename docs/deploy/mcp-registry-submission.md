...
…或從本 PR 中的 `docs/deploy/server.json` 手動複製。
...
| 路徑 | 用途 |
|---|---|
| `docs/deploy/mcp-registry-submission.md`（本檔） | 提交準備備忘錄 |
| `docs/deploy/server.json` | 草稿 `server.json`，待 operator 填入 SHA-256 並發布 release 後即可用於 `mcp-publisher publish` |
