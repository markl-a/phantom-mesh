...
# spectyn-mesh 定位（2026-05-15）

本文件是 spectyn-mesh 對外自我描述的權威來源（canonical source）——
...
> **spectyn-mesh 是一套個人、多裝置的 AI 代理人執行環境（agent runtime）——
...
- **Closed-loop self-improvement（閉環自我改進）** — `spectyn autoevolve` 監看
  倉庫（repo），在測試轉紅（失敗）時生成 `spectyn evolve`，轉綠（通過）時自動提交。
  每次迭代都會附加寫入 `~/.spectyn-mesh/autoevolve.log`；下一輪
...
| "No project simultaneously has Rust + P2P + agent runtime" | OpenFang（github.com/RightNow-AI/openfang，MIT，約 16.8K stars，Rust + 自有 P2P + MCP + A2A）已於 2026 年推出此組合。該主張在事實上是錯誤的。 | "Personal multi-device focus + closed-loop self-improvement + 5-platform mobile story" —— spectyn-mesh 現今真正的差異化。 |
| "Syncthing for AI agents" | 尚未在真實場景中驗證；可留作備案。不要拿它當主打。 | 無 —— 除非使用者自然而然地採用它，否則捨棄。 |

## 3. spectyn-mesh 在 2026 生態系中的位置
...
- **他們今日勝過 spectyn-mesh 之處：** A2A 協定支援、更大的
  社群、更成熟的排程代理人語意。
- **spectyn-mesh 仍具差異化之處：** 行動原生（mobile-native）的故事（已簽章的
  Tauri APK + 已簽章的 IPA + Android 上的 Termux 真實 worker（工作節點））、`spectyn evolve`
  閉環自我修復、Apple Silicon 上的 MLX 本地 LLM、在每個支援的作業系統上
  單一二進位檔（single-binary）安裝。
- 誠實的框架：OpenFang 是偏向桌面／伺服器的 Rust 代理人 mesh，
  具備更豐富的協定面；spectyn-mesh 則是行動 + 自我改進
  + 單一二進位檔處處可用的變體。
...
- **與 spectyn-mesh 的關係：** 互補，而非競爭。我們可以
  成為 OpenCode 使用者的 spectyn-mesh-as-provider 外掛（約 200 LOC），而
  OpenCode 也可以被無頭（headlessly）呼叫，作為 spectyn-mesh 的子代理人（subagent）。
...
- spectyn-mesh 提供一個指向 `/zen/v1` 的 `opencode` 供應商型別；
...
- 我們仍持續推薦 Tailscale 作為
  spectyn-mesh 叢集（cluster）的建議覆蓋層（overlay）（見 `README.md` §Cluster）—— Aperture 與 spectyn-
  mesh 並不衝突，只是位於不同的層。
...
- A2A（`a2a-protocol.org`）—— 代理人 ↔ 代理人，Linux Foundation，150+ 組織，
  約 22K stars，v1.2。
- spectyn-mesh 今日已支援 MCP（`spectyn mcp` 將 50 個工具公開為
  `mcp__spectyn__*`）。A2A 仍在路線圖上；尚未推出。
...
- 我們不是無程式碼（no-code）視覺化建構工具（Langflow／Flowise／Dify）。
  那些是用於原型開發；spectyn-mesh 是執行環境基礎設施（runtime infra）。
