# experimental-extra-providers

**狀態：** experimental（實驗性）。預設為關閉（Default OFF）。
**Cargo feature（Cargo 功能旗標）：** `experimental-extra-providers`（不引入新的 crate（套件）相依）。
...
預期的最後一行：`experimental-extra-providers OK`。離開碼（exit code）為 0。
