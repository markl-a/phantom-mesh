...
**Cargo feature（Cargo 功能旗標）：** `experimental-curator`（會一併引入 `serde_yaml`）。
...
```bash
cargo build --features experimental-curator
```
...
預期的最後一行：`experimental-curator OK`。結束碼（Exit code）0。

## 原始碼

- `core/src/skillbank/curator.rs` — Curator、`build_judge_user_prompt`、`parse_judge_reply`、`verdict_from_parsed`、`RUBRIC_VERSION`、`DEFAULT_JUDGE_MODEL`。
- `core/src/skillbank/skill.rs` — `SkillDocument`、`SkillFrontmatter`、`parse_str`、`serialize`、`SkillError`。
- `core/src/skillbank/mod.rs` — re-exports（重新匯出）。
...
V2 與 V1 同樣位於 `experimental-curator` cargo feature 後面。
...
- `core/src/skillbank/curator_ensemble.rs` — `JudgeProvider`、`AnthropicJudge`、`OpenAICompatJudge`、`EnsembleCurator`、`aggregate`、`median_score`、`population_stddev`。
- `core/src/skillbank/curator.rs` — `parse_judge_reply_strict`（與 V1 的 `parse_judge_reply` 並存的新增項）。
