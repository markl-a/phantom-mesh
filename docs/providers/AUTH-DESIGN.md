# Phantom Auth / Identity / Onboarding — 設計定案
...
- 裝置加入靠 Tailscale 自己的 auth / pre-auth key。phantom 不插手。

### ② mesh-信任 plane — phantom 信任哪些節點/client ✅ 已實作+驗證
...
- **無靜態 secret 當主、無每台 phantom OAuth**。已用 iPhone 跨裝置驗證免 401。
- opt-in:`[cluster] trust_tailnet_peers = true`。
- ⚠️ **精確說法(別誤會成「零登入」)**:裝置加入 tailnet **仍需 Tailscale 一次性認證**(SSO 或 pre-auth key)。phantom 的 win 不是「不用登入」,而是 **沿用 Tailscale 那層你本來就要做的認證、不另加 phantom 自己的帳號/OAuth**——**一次認證而非兩次**;**每台入網時一次,而非每 session**。**安全 mesh 必有某種 auth**(SSO / 自發 key / 憑證 / 預共享鑰),差別只在「放哪」與「多頻繁」。要連第三方帳號都不要 → Headscale(自架控制面、自己的 user + pre-auth key)或 Nebula(憑證式 PKI,無登入)。
...
- 現況:Google 走 broker(`phantom login broker`)可用;`phantom login google` 直連壞(client_secret missing);**Apple BLOCKED**(broker /api/health 無 apple,#4 portal+deploy 未做)。

## Onboarding = mesh-first
新裝置 = ① 加 tailnet(Tailscale up / auth key)② 跑 `phantom serve`(或 thin client 連 coordinator)。**不每台點瀏覽器 OAuth**。OAuth 只在 opt-in 雲端功能時。

## 開源版信任根策略(tiered)+ tag-gate 預設
OSS 版把信任根做成分層,**「自帶 tailnet」為預設**(最方便),同時保留無-vendor 與無-mesh 退路:
| 層 | 給誰 | 信任根 | 門檻 |
|---|---|---|---|
| **預設(最方便)** | 一般 self-host | **Tailscale SaaS**(自帶帳號,免費 tier)| 裝 Tailscale + 跑 phantom |
...
為何 OSS 靠 Tailscale 划算:重用其 NAT 穿透 / 裝置認證 / whois → phantom **零信任系統維護**;目標用戶(homelab/self-host)多已有 Tailscale;定位「**跑在你現有 tailnet 上**」(Zeabur 等同模式)。
...
- **OSS 預設應 gate by `tag:phantom`**(只信任打了 phantom tag 的 peer)。
- 機制:`trust_tailnet_peers` 之外加 `trust_tailnet_tag`(預設 `"tag:phantom"`);單人方便模式才放寬成「任何 peer」。
...
問題:phantom 要不要內建類似 Tailscale 的功能?**決策:不重做核心,只做一層有界的 LAN 內建 mesh。**

**不自建**(協調面 + 中繼 relay + NAT 穿透):
- 離題——phantom 的價值在 agent mesh 編排,不在網路傳輸。
...
- mDNS 區網發現(serve 已在做:`_phantom-mesh._tcp` 註冊)。
...
## Naming canonical = `ai.phantommesh.*`
理由:出貨 app(iOS/Android/桌面)bundle id 已是 `ai.phantommesh.app`,改不得 → 其餘配合它。
- Apple Services ID → **`ai.phantommesh.signin`**(取代文件的 `io.` / 早期決定的 `com.`)
- Apple App ID(Sign in with Apple)→ `ai.phantommesh.app`
- 衛星 launchd `com.markl-a.phantom-*` → 可選 `ai.phantommesh.<name>`(cosmetic,低優先)
- 修 `docs/install/APPLE-SIGN-IN-SETUP.md` 的 `io.phantommesh.signin` → `ai.phantommesh.signin`
- 已一致:Tauri/iOS/Android = `ai.phantommesh.app`;launchd serve/nosleep/autoevolve = `ai.phantommesh.*`;顯示名 `Phantom Mesh`;網域 `phantommesh.io`。
