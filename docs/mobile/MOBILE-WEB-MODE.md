...
Spectyn Mesh 出貨一份完整的 React 桌面 UI（使用者介面）（`app/src/`），以及一份獨立的
輕量級 web 前端（`core/web/`，由 `spectyn serve` 在連接埠（port）
...
3. 後續啟動時，會以 `window.location.replace` 跳轉至
   `http://<host>:<port>/`，也就是 `spectyn serve` 對外公開的 URL。
...
1. 在你的 Mac／PC 上，啟動伺服器：
   ```bash
   spectyn serve            # listens on 0.0.0.0:7878
   ```
2. 將你的手機連上同一個 Tailnet（推薦）或 LAN（區域網路）。
3. 在手機上啟動 Spectyn Mesh 的 Tauri 應用程式。
...
```js
localStorage.removeItem('SPECTYN_HOST');
location.reload();
```

可選的鍵（key）：`SPECTYN_PORT`（預設 `7878`）、`SPECTYN_SCHEME`
