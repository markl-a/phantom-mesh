...
```
┌───────────────────────────────────────────────────────────────┐
│  spectyn-core (Apache 2.0, public GitHub)                     │
...
└───────────────────────────────────────────────────────────────┘
                          ...
                          ▼
┌───────────────────────────────────────────────────────────────┐
│  spectyn-cloud / spectyn-team-broker (Commercial, source-     │
...
**一個 spectyn-core 可以對話的服務**，透過一個薄客戶端
（`spectyn-cloud-client` crate，藏在 `--cloud` cfg feature 之後）來溝通
...
2. **開源二進位檔（binary）自身完全可用。** 一位沒有任何雲端帳號、沒有網際網路、
   且使用 BYOK（自帶金鑰，Bring Your Own Key）的使用者，能做到今天 `spectyn`
   二進位檔能做的一切——這是該二進位檔終其一生的契約。
3. **遙測（telemetry）預設關閉且永遠採選擇加入（opt-in）。** 開啟時，原始資料存放於
   `~/.spectyn-mesh/telemetry.jsonl`。使用者得執行 `spectyn telemetry upload` 才會送出；
   我們絕不自動回傳（auto-phone-home）。
4. **商業層可自架。** Team/Enterprise 的 broker 跑在客戶自己的基礎設施上
   （`docker run spectyn/broker:latest`）。我們賣的是支援 + 託管便利——不是
   存取該二進位檔的權利。
...
若某項功能必須違反上述任何一條，我們就把它移進 `spectyn-cloud` repo，
它永遠不會出現在 `spectyn` 裡。
...
|---|---|---|---|---|
| 所有已發行的 spectyn-core（CLI/TUI/serve/MCP/autoevolve/MLX/snapshot/全部 50 個工具） | ✅ | ✅ | ✅ | ✅ |
...
這些層級買到的是**便利與維運**，而非 spectyn-core 的能力。
一位有決心的開源使用者，靠著自己跑一個基於開源 `spectyn-cloud-broker`
...
```
spectyn login                 # device-flow OAuth, opens browser
spectyn devices               # lists this user's mesh
spectyn claim <hostname>      # adds this device to the user's mesh
```

底層運作：
- 一個無協調者（coordinator-less）變體的 `/rpc/peers`，跑在
  `broker.spectynmesh.dev/api/peers/<account-id>`
- 每台裝置都跑 spectyn-core，並可選擇性地呼叫
  `broker.spectynmesh.dev` 並註冊它的 WireGuard 端點
  （或者，在沒有 WG 的情況下，一個經身分驗證的 long-poll 連線）
- 該 broker 本身就是 spectyn-cloud-broker——Pro 使用者用我們的，
...
```
spectyn telemetry status            disabled (default forever)
spectyn telemetry enable            opt-in, prints what it captures
spectyn telemetry disable           one-shot off + clears local buffer
spectyn telemetry upload            send buffered file to broker
spectyn telemetry preview           print the next-to-be-sent payload
```

擷取了什麼（開啟時）：
- slash 命令、工具呼叫、agent 派發（dispatch）的次數計數
- 錯誤分類（沒有堆疊追蹤、沒有提示詞、沒有輸出）
- 平台 / spectyn 版本 / 設定了哪些供應商
...
`~/.spectyn-mesh/telemetry.jsonl`。在他們 `upload` 之前，沒有任何東西離開
...
——而這是對的。Pro 是給那些想要 `spectyn login` 能「直接就動」的
使用者的便利層。
...
- `spectyn-core/` — Apache 2.0
- `spectyn-cloud-client/`（只有在 `--features cloud` 時才會編進去）
  — Apache 2.0（如此一來廠商鎖定 vendor lock-in 不可能發生）
- `spectyn-cloud-broker/` — **BSL 1.1，4 年後變更日期（change date）轉為
  Apache 2.0**（Tailscale 的同款做法）。Source-available，你可以稽核 + 在自己的
  基礎設施上跑，但 4 年內不得轉售為服務。
- `spectyn-cloud-web/`（儀表板 dashboard）— 同樣 BSL。
- 商標 `spectyn-mesh` — 由我們持有，AGPL 風格：可以跑去品牌化（unbranded）的
...
在 spectyn-core 達到 1k stars + 約 50 個每週活躍自架者（從開源目前的二進位檔而來）
...
| 階段 | 月份 | 範圍 |
|---|---|---|
| **0：開源 spectyn-core** | 現在 | 把目前的程式碼以 Apache 2.0 開源，GitHub release + `brew tap markl-a/spectyn-mesh` |
| **1：穩定開源版** | 第 1-3 月 | 只做修錯 + 文件 + 社群 PR。不寫商業程式碼。 |
| **2：雲端 broker MVP** | 第 4-6 月 | `spectyn login`、peer 探索、免費層 3 台裝置。託管於 Fly.io / Render。一開始先單一定價（$7/月 Pro）。 |
...
- **Costs.json 是本機的。** 遙測關閉時資料只存在磁碟上。開啟時，由使用者驅動的
  `spectyn telemetry upload` 是它離開裝置的唯一路徑。
- **launchd / systemd / Scheduled Task 自動啟動開源二進位檔。**
  雲端功能透過獨立的 LaunchAgents 疊加於其上
  （`ai.spectynmesh.cloud-relay.plist`）——可獨立解除安裝。

如果你在審查一個 PR，而它會強制走純雲端路徑，審查者範本應該問
「這在純開源模式下也能運作嗎？若不能，請移到 spectyn-cloud。」
...
| 問題 | 傾向 |
|---|---|
| 單一二進位檔 `spectyn` 配 `--features cloud`，還是兩個二進位檔 `spectyn` + `spectyn-cloud-client`？ | **單一二進位檔**——Pro/Team build 用 `cargo build --features cloud`。UX 較簡單。 |
