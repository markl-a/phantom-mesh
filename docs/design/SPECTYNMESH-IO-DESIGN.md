...
使用者一執行 `spectyn login`（不帶參數）的當下，spectyn CLI 就會以
...
core/src/bin/spectyn.rs）。新增欄位是可累加的（additive） — CLI 會
忽略它們。
...
- **自架在 Mac coordinator（協調者）上** — spectyn serve 已經
...
CREATE TABLE devices (
    device_id    TEXT PRIMARY KEY,      -- the spectyn CLI's uuid
...
| 天 | 範圍 |
|---|---|
| **1** | 把 Cloudflare 帳號 + D1 + KV + Workers 專案初始化好。`/api/health` 回 200。spectyn CLI 的 broker 探測從「離線」變成「上線」 — 但其他一切仍是 404。 |
...
讓 spectyn autoevolve 幫忙做掉部分工作會更快（把 OAuth 函式庫的文件
放進 context，對 broker repo 跑 `spectyn evolve`）。
...
- CLI 在 `127.0.0.1:48181/oauth/callback` 上監聽，接受 GET（帶
  ...
  挑 broker 端比較好做的那種。
- CLI 會把 broker 回傳的任何 JSON 存到 `~/.spectyn-mesh/auth.json`，
...
- `device_id` 是一個穩定的 UUID v4，CLI 在首次登入時產生，並在
  ...
  視為標準的「Alice 的這台 Mac」。
- 使用者可用
  `SPECTYN_AUTH_URL=https://my-self-hosted-broker.example.com`
...
| ❌ 別做 | 原因 |
|---|---|
| 要求登入才能使用 `spectyn` | OSS-binary 契約（開源二進位檔契約） — 永遠不需帳號即可運作。 |
...
| 問題 | 傾向 |
|---|---|
| 要保留「Sign in with spectyn-mesh GitHub OAuth」嗎？ | 第 2 週加入作為第 4 個供應商。受眾與我們的使用者群高度重疊。 |
