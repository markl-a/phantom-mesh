...
自我測試框架（self-test harness）是 phantom-mesh 的端對端煙霧測試（smoke-test，基本可用性驗證）層。它會像真實使用者（或 LLM agent，大型語言模型代理）那樣操作一個*已建置*的 `phantom` 二進位檔——執行 CLI（命令列介面）子命令、透過 HTTP 打本機 daemon（常駐服務）、驅動 TUI（終端機使用者介面）、檢查 MCP（模型上下文協議）與 cluster（叢集）RPC（遠端程序呼叫）介面——並同時輸出人類可讀的表格與機器可讀的 JSON 報告。

它刻意與 `cargo test` 分開：單元／整合測試驗證 Rust 內部實作，而自我測試框架驗證實際交付的產物及其執行時行為。每個 feature（功能）恰好擁有一個可直接放入的測試檔，因此新增一個功能等於新增一支腳本，而非編輯一份巨型檔案。

唯一的進入點是 `phantom selftest`，這讓此框架可從人類的 shell、從 CI（持續整合），以及從 phantom 自身（透過它的 `shell` 工具）觸達。
...
| `scripts/selftest.d/10-doctor.sh`, `15-doctor-json.sh` | `phantom doctor` 的文字 + JSON 檢查。 |
...
| `core/src/bin/phantom.rs` | `phantom selftest` 的 Rust 墊片（shim）（`run_selftest`、`locate_selftest_script`、`find_bash`）。定位 `scripts/selftest.sh`、找到可用的 bash，並 exec（執行）它。 |

## 資料流（Data flow）

1. 呼叫者執行 `phantom selftest [flags]`。Rust 墊片（`core/src/bin/phantom.rs` 中的
   `run_selftest`）會定位 `scripts/selftest.sh`——依序檢查
   `$PHANTOM_SELFTEST_SCRIPT`、目前目錄、上層目錄、可執行檔所在目錄，以及
   `~/.phantom-mesh/scripts/`——找到一個 bash 直譯器，並 exec 協調器、轉送所有旗標。
2. `selftest.sh` 解析出 `phantom` 二進位檔（`$PHANTOM_BIN` 或 PATH），在
...
```mermaid
flowchart TD
  A["phantom selftest"] --> B["Rust 墊片：定位腳本 + bash"]
  B --> C["selftest.sh 協調器"]
...
```bash
phantom selftest                 # 所有功能，文字表格
phantom selftest --json          # 在 stdout 輸出機器可讀的 JSON
phantom selftest --p0-only       # 只跑阻擋交付的檢查
phantom selftest --feature serve # 單一功能
phantom selftest --list          # 列出已登錄的功能
```
