...
`phantom` 自我演進迴圈（self-evolution loop）的工作佇列（work queue）。autoevolve
...
1. `phantom evolve goals ...` CLI 子命令，
...
| `core/src/bin/phantom.rs` | CLI 進入點。`run_evolve_goals()` 實作 `next` / `list [--json]` / `add "<text>"` / `mark-done <line>`，並解析 `--file` 路徑（預設為 `EVOLVE-GOALS.md`）。 |
...
1. 操作者附加一個目標 —— `phantom evolve goals add "Add --json to list"` ——
   它會呼叫 `GoalsFile::load()`、`add_pending()`，然後 `save()`（若檔案不存在，
   會在第一次 save 時建立）。
2. evolve 迴圈執行 `phantom evolve goals next`；`next_pending()` 回傳 `## Pending`
   底下第一個未勾選的項目。**純目標文字**會印到 stdout（標準輸出，裝飾性訊息則送到
   stderr，標準錯誤輸出），如此便能與 shell 管線（pipeline）組合使用。
3. 工作進行；完成後執行 `phantom evolve goals mark-done <line> --sha <sha>`，
...
- **新增 CLI 子命令** —— 在 `run_evolve_goals()`（`core/src/bin/phantom.rs`）的
...
- **CLI 整合測試（integration tests）**位於 `core/tests/test_evolve_goals.rs`，
  透過 `CARGO_BIN_EXE_phantom` 呼叫建置好的二進位檔：
