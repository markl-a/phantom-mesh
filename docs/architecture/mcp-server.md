...
> spectyn-mesh 的模型上下文協議（Model Context Protocol，MCP）整合 — 同時涵蓋**伺服器（server）**端與**用戶端（client）**端。

## 用途（Purpose）

這個子系統讓 spectyn-mesh 在 MCP 生態系（協議修訂版 `2024-11-05`）中扮演兩個互補的角色：

1. **伺服器（`spectyn mcp`）** — spectyn 透過 JSON-RPC 2.0 stdio（標準輸入輸出）傳輸，把自己約 40 個內建工具公開給任何 MCP host（主機，如 Claude Code、Codex、Goose……）。同一個 dispatcher（派工器）也能經由 `spectyn serve` 的 `POST /mcp` endpoint（端點）透過 HTTP 取用。這讓外部 agent（代理）能像呼叫原生工具一樣呼叫 spectyn 工具（`file_read`、`shell`、`web_search`、叢集工具 `spectyn_swarm`……）。

2. **用戶端** — spectyn 本身也能扮演 MCP *host（主機）*：它把 `agents.toml` 裡宣告的外部 MCP 伺服器以子行程（child process）的形式啟動，執行交握（handshake），並把它們的工具重新公開給 spectyn 自己的 agent runtime（代理執行時），讓 LLM（大型語言模型）能像呼叫任何內建工具一樣呼叫它們。
...
| `core/src/bin/spectyn.rs` | CLI 接線。把 `mcp` 子指令路由到 `mcp::run_stdio`；在 serve/REPL 啟動時呼叫 `mcp_client::init_global`；實作 `/mcp [test NAME]` REPL 指令。 |
...
1. Host 把以換行分隔的 JSON-RPC 2.0 訊息寫入 spectyn 的 **stdin（標準輸入）**；stderr（標準錯誤）保留給診斷訊息使用。
...
4. `tools/list` 把每個內建工具加上兩個合成的叢集工具（`spectyn_swarm`、`spectyn_evolve_distributed`）映射成 MCP 描述子（descriptor）。
...
- **新增一個合成／叢集工具** — 在 `handle` 的 `tools/list` 分支附加一個描述子，並在 `tools/call` 分支加一個對應的分支（參見 `spectyn_swarm` / `spectyn_evolve_distributed`）。
...
- `scripts/selftest.d/30-mcp.sh` — `spectyn selftest` 對 stdio 交握、`tools/list` 與最低工具數量的端對端檢查（純 bash、跨平台）。
