...
> `phantom` 命令列介面（CLI，command-line interface）子系統的架構說明文件。

## Purpose（用途）

`cli-phantom` 是 phantom-mesh 面向使用者的進入點，也就是 `phantom`
...
1. **Interactive（互動式）** — 執行不帶子命令的 `phantom` 進入 ratatui 終端機
   UI（TUI，文字使用者介面），或執行 `phantom repl` 進入以行為單位的讀取-求值-列印迴圈（REPL，read-eval-print loop）。
2. **One-shot subcommands（單次子命令）** — `serve`、`doctor`、`mcp`、`evolve`、`selftest`、
   `cluster`、`keys`、`providers`，以及約 40 個其他子命令（見 `KNOWN_SUBCOMMANDS`）。
3. **Long-running daemons（長時間執行的常駐服務）** — `phantom serve` 啟動由
   桌面／行動應用程式所使用的 HTTP API；`phantom mcp` 透過標準輸入輸出對外提供一個
   Model Context Protocol（MCP，模型上下文協議，AI 客戶端使用的工具呼叫協議）伺服器。

CLI 位於整個堆疊的最上層：它掌管引數解析與行程
生命週期，然後把所有實際工作委派給 `phantom_mesh` library crate（函式庫套件）。

## Key files（關鍵檔案）

| File（檔案） | Role（角色） |
| --- | --- |
| `core/src/bin/phantom.rs` | `phantom` 執行檔。手寫的引數解析器、頂層子命令分派，以及 `doctor`、`selftest`、`evolve`、TUI／REPL 啟動器與許多小型子命令的實作。 |
| `core/src/main.rs` | `phantom-mesh` 執行檔——HTTP 伺服器目標。建構 Axum router（路由器）並提供 dashboard／agent 端點。由 `phantom serve` 重複使用。 |
| `core/src/serve.rs` | HTTP serve 管線：伺服器使用面所共用的請求處理器與路由接線。 |
| `core/src/lib.rs` | `phantom_mesh` library crate 的根。對外公開 `AppState`、`start_http_server`，以及 CLI 呼叫進去的模組樹。 |
| `core/src/cli_config.rs` | 實作 `phantom keys` 與 `phantom providers`；以 `toml_edit` 編輯使用者設定檔（`~/.phantom-mesh/env`、`agents.toml`）以保留註解。啟動時也會執行 `auto_load_env`。 |
| `core/src/mcp.rs` | `phantom mcp` 背後的 MCP stdio 伺服器。 |
...
```mermaid
flowchart TD
    A["使用者輸入 phantom &lt;sub&gt; (user runs command)"] --> B["main(): diag::init + auto_load_env"]
...
1. `main()` 呼叫 `phantom_mesh::diag::init()`，在任何其他環節可能失敗之前
   先裝上 panic／crash（當機）攔截掛鉤。
2. `cli_config::auto_load_env()` 載入 `~/.phantom-mesh/env`，讓 API 金鑰的
...
- **Add a subcommand（新增子命令）** — 在 `core/src/bin/phantom.rs` 的
  `KNOWN_SUBCOMMANDS` 中加入它的 slug，在 `main()` 中加入一個分派分支，並實作該
  處理器（小型處理器放在執行檔內；具份量的邏輯應放在
  `core/src/` 下的 library 模組）。加入一行用法說明條目，讓 `phantom help`
  與拼字建議器能夠抓到它。
...
- **Manage config surfaces（管理設定使用面）** — `phantom keys`／`phantom providers` 的邏輯位於
...
- **Unit tests（單元測試）** — `core/src/bin/phantom.rs` 中內嵌的 `#[test]`／`#[tokio::test]`
...
- **End-to-end smoke tests（端對端冒煙測試）** — `scripts/selftest.d/` 是 `phantom selftest`
