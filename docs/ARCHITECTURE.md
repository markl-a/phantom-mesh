# Spectyn Mesh — Architecture (as built)
...
Spectyn Mesh is a **single Rust binary** (`spectyn`) that acts as a personal AI runtime:
...
┌────────────────────────────── spectyn node ──────────────────────────────┐
│                                                                          │
│  entrypoints    spectyn (TUI) · repl · exec · serve · mcp · evolve ·     │
...
- **Subscription-CLI backends** (`claude_agent`, `codex_agent`, `opencode_agent`, …) —
  spectyn drives a locally signed-in coding-agent CLI through the **L0 `cli_session`
...
`spectyn serve` starts an Axum server: WebSocket (`/ws`), REST (`/api/*`), cluster RPC
...
cross-review) that spectyn's own development runs on.
...
key-value store under `~/.spectyn-mesh/`. The **owned-memory** layer indexes events,
...
Spectyn is both an **MCP server** (stdio for Claude Desktop / Cursor, plus `/mcp` over
...
`spectyn evolve` runs a test-driven fix loop (optionally distributed across the mesh,
