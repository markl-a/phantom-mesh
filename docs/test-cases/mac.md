...
| ID | Type | Auto | Setup | cmd | expected | Verifies | last_run | 狀態 |
|---|---|---|---|---|---|---|---|---|
| MAC-CUJ01-SIGN-001 | e2e | ✅ | install 完 | `~/.local/bin/spectyn --version` | exit 0、輸出匹配 `spectyn 0\.6\.0(-rc\.[0-9]+)?`（P1-5 容忍判準：切版 0.6.0 後不 false-FAIL） | Gatekeeper passes | 2026-05-31 02:25 | 🟡 |
| MAC-CUJ01-SIGN-002 | manual | ❌ | 不 ad-hoc | 手動 cp binary、跑 | macOS 印「無法驗證開發者」 | SPEC-63 codesign | ⬜ | ⬜ |
| MAC-CUJ01-SIGN-003 | e2e | ✅ | R2 + local | size 比較 R2 vs `~/.local/bin/spectyn` | size 差 < 200KB | resign 期望差 | 2026-05-31 02:25 (差 -139KB) | 🟡 |
| MAC-CUJ01-SIGN-004 | manual | ❌ | - | `ls -la@ ~/.spectyn-mesh/bin/spectyn` | 行尾無 `@` (xattr 已清) | provenance xattr | 2026-05-31 02:05 | ✅ |
...
| MAC-CUJ01-INIT-002 | integ | ✅ | TempDir | 跑兩次 `spectyn habit water --qty 1` | `stat ~/.spectyn-mesh/identity.key` inode 不變 | identity 持久 | ⬜ | ⬜ |
| MAC-CUJ01-INIT-003 | unit | ✅ | - | `cargo test --lib key_derivation` | test result: ok | HKDF derive | ⬜ | ✅ existing |
| MAC-CUJ01-INIT-004 | integ | ✅ | TempDir HOME | first-run + `sqlite3 ~/.spectyn-mesh/events.sqlite ".tables"` | `.tables` 僅需含 `fts5_events`（SPEC-16 P0-1 as-built；`events` 主表 = v0.7.0+ planned） | SPEC-16 schema | ⬜ | ✅ existing（as-built table = `fts5_events`） |
| MAC-CUJ01-INIT-005 | integ | ✅ | TempDir | `spectyn habit palette list` 首跑 | 印 12 列、含 "水" "咖啡" | SPEC-22 G1 starter | ⬜ | 🟡 (cuj02 verified palette OK) |
...
| MAC-CUJ01-INIT-008 | integ | ⚠ unset HOME | `HOME=` (empty) | `spectyn habit water --qty 1` | exit 非 0 + 印 "HOME unset" | env graceful | ⬜ | 🟡 |
...
| ID | Type | Auto | Setup | cmd | expected | Verifies | last_run | 狀態 |
|---|---|---|---|---|---|---|---|---|
| MAC-CUJ01-FH-001 | integ | ✅ | TempDir | `spectyn habit water --qty 250` | exit 0、stdout 含 "streak" | CUJ-01 step 5 / SPEC-22 G2 | ⬜ | ✅ (cuj02 covered) |
...
| MAC-CUJ01-FH-004 | integ | ❌ | post-FH-001 | `sqlite3 ~/.spectyn-mesh/events.sqlite "SELECT content FROM fts5_events WHERE fts5_events MATCH '水' LIMIT 1"` | 至少 1 列 | FTS5 search 入庫 | 2026-05-31 audit | 🔴 NOT-IMPL: `index_fts5` 唯一 caller 是測試 (event_storage_wire.rs:1159)；無 production capture path 填 `fts5_events`。record_checkin 只寫加密 EventStore。需先把 indexer 接進 capture 流程 (見 AUTONOMOUS-WORKLOG G9-FH004) |
...
| MAC-CUJ01-FH-006 | integ | ✅ | TempDir | `spectyn habit "讀完 SICP ch3"` | exit 0、event 寫入、chip_id="freetext" | SPEC-22 G5 freetext | 2026-05-31 02:50 | ✅ (cuj02) |
...
### 2.A `spectyn food` photo capture (8 條)

| ID | Type | Auto | Setup | cmd | expected | Verifies | last_run | 狀態 |
|---|---|---|---|---|---|---|---|---|
| MAC-CUJ02-FOOD-001 | integ | ✅ | temp HOME + seeded agents.toml + EventKey seed + wiremock Gemini (`SPECTYN_MESH_GEMINI_BASE_URL`) | `cargo test --test cuj02_capture_hermetic cuj02_food_001` | `record_food` → Ok(event_id)、event 加密寫入 + `read_event`/`query_events` 讀回 + P4 no-plaintext | SPEC-20 G1 | 2026-06-11 cargo | 🟡 cuj02_capture_hermetic 蓋 library pipeline（wiremock LLM + inlineData + 加密落地 + 讀回）；CLI `spectyn food` → serve daemon hop 仍只有 key-gated e2e（life_node_capture_e2e.rs，CI skip）|
| MAC-CUJ02-FOOD-002 | integ | ✅ | TempDir + image | (post-001) `head -c 24 ~/.spectyn-mesh/events/*/modality_0.* \| xxd` | 開頭是 age v1 magic bytes (`age-encryption.org/v1`) | SPEC-13 P4 image 加密 | ⬜ | ♻️ DRIFT(P0-1 修檔名)：as-built（`storage.rs`）image modality 檔名是 `modality_<idx>.<ext>`（如 `modality_0.jpeg`，依 mime 取 ext，`storage.rs:185–193`），**非** `modality_image.json`；且整檔 age 加密（`write_file:126–134`）。原 glob 撞不到任何檔→會誤判。in-module 證據：`encrypted_store_image_modality_is_encrypted_on_disk`（`storage.rs:594–616`）|
| MAC-CUJ02-FOOD-003 | integ | ⚠ all-fail mock | wiremock 所有 provider 失敗 | `spectyn food --image x.jpg` | exit 非 0、不寫 event | provider fail graceful | ⬜ | 🟡 |
...
| MAC-CUJ02-FOOD-005 | manual | ❌ | 7MB jpg | `spectyn food --image big.jpg` | exit 非 0 + "size limit" | size guard | ⬜ | ⬜ |
| MAC-CUJ02-FOOD-006 | manual | ❌ | 0 bytes file | `spectyn food --image empty.jpg` | exit 非 0 + "non-image" | corrupt guard | ⬜ | ⬜ |
...
### 2.B `spectyn focus` audio capture (7 條)
...
| MAC-CUJ02-FOC-002 | integ | ✅ | TempDir base + identity.key seeded | `spectyn focus start` → `spectyn focus stop` | 寫 kind=focus event；summary=固定格式；model_id=`local-focus-timer` | SPEC-21 P1-7 as-built timer | ⬜ | 🟡 [v0.7.0+ deferred / SPEC-21 P1-7 DRIFT-blocked]：audio/ASR takeaway 子句 N/A |
| MAC-CUJ02-FOC-003 | manual | ❌ | 無麥克風互動 | `spectyn focus start` | 無 mic permission 流程；建立/更新 `focus-session.json`，不寫垃圾 | SPEC-21 P1-7 as-built timer | ⬜ | ⬜ [v0.7.0+ deferred / SPEC-21 P1-7 DRIFT-blocked] |
| MAC-CUJ02-FOC-004 | integ | ⏸ | 真 wav 1 min | ~~`spectyn focus process tests/fixtures/1min.wav`~~ | 子命令不存在；audio process 驗收移 v0.7.0 backlog | SPEC-21 audio backlog | ⬜ | ⏸ [v0.7.0+ deferred / SPEC-21 P1-7 DRIFT-blocked] |
...
### 2.C `spectyn habit` chip + freetext (10 條)

| ID | Type | Auto | Setup | cmd | expected | Verifies | last_run | 狀態 |
|---|---|---|---|---|---|---|---|---|
| MAC-CUJ02-HAB-001 | integ | ✅ | TempDir | `for i in $(seq 1 13); do spectyn habit palette add --id "t$i" --zh t$i --en t$i; done` | 第 13 加應 fail + "PALETTE-FULL" | SPEC-22 G1 limit | 2026-05-31 02:50 | ✅ (cuj02) |
| MAC-CUJ02-HAB-002 | integ | ✅ | post-001 + remove | `spectyn habit palette remove --id t1; spectyn habit palette add --id t13 --zh t13 --en t13; spectyn habit palette list` | 12 列 | CRUD cycle | ⬜ | ⬜ |
| MAC-CUJ02-HAB-003 | integ | ✅ | TempDir | `spectyn habit palette add --id new --zh 新 --en N; spectyn habit palette reorder --id new --to 0; spectyn habit palette list` | 第 1 列是 new | reorder | ⬜ | ⬜ |
| MAC-CUJ02-HAB-004 | integ | ⚠ time mock | TempDir + mockable clock | 寫 5 連天 events、`spectyn habit streak --chip water` | current_streak=5 | SPEC-22 G3 | ⬜ | 🔴 (mockable clock 缺) |
| MAC-CUJ02-HAB-005 | integ | ⚠ time mock | 同上 | 寫 1 day 1+2+4 (跳 3) → streak | current_streak=1 | streak 中斷邏輯 | ⬜ | 🔴 |
| MAC-CUJ02-HAB-006 | integ | ✅ | TempDir | `spectyn habit palette add --id bad --freq custom --cron 'not cron'` | exit 非 0 + InvalidCron | cron parse | 2026-05-31 02:50 | ✅ (cuj02) |
| MAC-CUJ02-HAB-007 | unit | ✅ | - | `cargo test --lib habit_frequency_serializes_with_kind_tag` | test 過 | wire shape stable | ⬜ | ✅ existing |
| MAC-CUJ02-HAB-008 | integ | ✅ | TempDir | `spectyn habit water --qty 0; --qty -1; --qty 10000` | 邊界處理 (預期 spec 補) | qty validate | ⬜ | ⬜ |
| MAC-CUJ02-HAB-009 | manual | ❌ | desktop UI | Cmd+Shift+H 按 | popover 出 + chip list | global shortcut | ⬜ | ⬜ |
| MAC-CUJ02-HAB-010 | integ | ⚠ data | 30 天 events | `spectyn habit grid --chip water --weeks 4` | ASCII 7×4 cells | SPEC-22 grid render | ⬜ | ⬜ |

### 2.D coach engine + delivery (11 條)

| ID | Type | Auto | Setup | cmd | expected | Verifies | last_run | 狀態 |
|---|---|---|---|---|---|---|---|---|
| MAC-CUJ02-COA-001 | integ | ⚠ LLM | mock LLM + 1 event | `spectyn coach review --date 2026-05-30` | exit 0、stdout markdown | SPEC-23 G1 | 2026-05-31 | ✅ cuj04_coach_review_llm (happy path → Completed) |
| MAC-CUJ02-COA-001a | integ | ✅ | TempDir + 25h-ago event | `spectyn coach review --date yesterday` | review 不含 25h 前 event | aggregator window | ⬜ | ⬜ |
...
| MAC-CUJ02-COA-003 | integ | ✅ | mock skill DB | `spectyn coach review --include-skills` | review 提到 pattern | SPEC-25 skill 拉 | ⬜ | ⬜ |
| MAC-CUJ02-COA-004 | e2e | ✅ | TempDir | `spectyn coach schedule install` | exit 0、`~/Library/LaunchAgents/ai.spectynmesh.coach.plist` 存在 | SPEC-23 G1 launchd | ⬜ | 🟡 ship |
| MAC-CUJ02-COA-005 | manual | ❌ | 1 天 | install + 等 7:00 | `~/.spectyn-mesh/reviews/<date>.md` 存在 | launchd 觸發 | ⬜ | ⬜ |
...
| ID | Type | Auto | Setup | cmd | expected | Verifies | last_run | 狀態 |
|---|---|---|---|---|---|---|---|---|
| MAC-CUJ03-LOG-001 | e2e | ⚠ broker mock | `BROKER_URL=mock` | `spectyn login --provider google` | exit 0、`broker.json` 存在 | OAuth flow | ⬜ | ✅ existing |
| MAC-CUJ03-LOG-002 | integ | ✅ | post-001 + clear cache | `spectyn cluster status` | token 有效、200 OK | token persistence | ⬜ | ⬜ |
| MAC-CUJ03-LOG-003 | integ | ⚠ mock | mock broker 401 | `spectyn cluster status` | UI 印 "重新登入"、不 panic | token expired UX | ⬜ | 🟡 |
...
| ID | Type | Auto | Setup | cmd | expected | Verifies | last_run | 狀態 |
|---|---|---|---|---|---|---|---|---|
| MAC-CUJ03-CLU-001 | e2e | ⚠ peer | 同網段 spectyn serve | `spectyn cluster join` | mdns list ≥ 1 peer | mDNS discovery | 2026-05-31 02:25 (1/4) | ✅ partial |
...
| MAC-CUJ03-CLU-004 | e2e | ✅ | post-CLU-001 | `spectyn cluster status` | 印 ✓/✗ 每節點 ping ms | status command | 2026-05-31 02:25 | ✅ |
...
| MAC-CUJ03-SYN-003 | e2e | ⚠ 2 device | 2 mac + 同帳號 broker | `mac1: spectyn habit water; mac2: spectyn habit streak --chip water` | mac2 看得到 ≤ 5s | CUJ-03 5s SLO | ⬜ | 🔴 沒量過 |
...
| ID | Type | Auto | Setup | cmd | expected | Verifies | last_run | 狀態 |
|---|---|---|---|---|---|---|---|---|
| MAC-CUJ04-OFF-001 | manual | ❌ | wifi off | `spectyn habit water --qty 250` | exit 0、event 寫本地 | local-first | ⬜ | ⬜ |
| MAC-CUJ04-OFF-002 | integ | ⚠ mock | mock all LLM fail | `spectyn coach review` | exit 0、stats-only output | stats-only mode (#142) | 2026-05-31 | ✅ cuj04_stats_only_fallback |
...
| MAC-CUJ04-LLM-002 | e2e | ⚠ broken cfg | agents.toml 含 404 model | `spectyn exec test` | fallback 跳第 2 | model 過期 skip | 2026-05-31 02:25 | ✅ (今天觸發過) |
...
| MAC-CUJ04-LLM-004 | integ | ✅ | TempDir 無 LLM key | `spectyn coach review --date yesterday` | exit 非 0 或友善 "設定 → providers" | no-key UX | ⬜ | ⬜ |
...
| MAC-CUJ04-IK-001 | integ | ✅ | TempDir + corrupt identity | `spectyn habit water --qty 250` | exit 非 0、fail-loud (不退化明文) | P4 fail-loud | ⬜ | 🟡 |
...
| MAC-CUJ04-DB-001 | integ | ✅ | TempDir + 壞 sqlite header | `spectyn habit water --qty 250` | exit 0、rename to `.corrupt-<ts>`、新建 fresh | sqlite recovery (✅ #143) | 2026-05-31 02:55 | ✅ ship |
...
> **2026-05-31 audit**: E004 Task 6 已 ship `spectyn data export` (JSON/MD) + `spectyn data delete --all --yes` (本機 events_dir). 新加 `spectyn backup --to PATH` (tar.gz 全 backup, commit 391bd38b).

| ID | Type | Auto | Setup | cmd | expected | Verifies | last_run | 狀態 |
|---|---|---|---|---|---|---|---|---|
| MAC-CUJ05-EXP-001 | integ | ✅ | isolated HOME (empty vault) | `spectyn data export --format json` | exit 0 + stdout parses as JSON array | E004 export | 2026-05-31 cargo | ✅ cuj05_data_export::cuj05_data_export_json_emits_parseable_array |
| MAC-CUJ05-EXP-002 | integ | ✅ | isolated HOME (empty vault) | `spectyn data export --format md` | exit 0 + stdout 含 `# Life Node export` header | E004 export | 2026-05-31 cargo | ✅ cuj05_data_export::cuj05_data_export_md_has_life_node_header |
| MAC-CUJ05-EXP-003 | manual | ❌ | mid-export | Ctrl+C | 半成 file 刪 | atomic | ⬜ | ⬜ |
| MAC-CUJ05-EXP-004 | integ | ✅ | TempDir + 4 sample files | `spectyn backup --to /tmp/x.tar.gz; tar -xzf /tmp/x.tar.gz -C /tmp/restore; diff -r ~/.spectyn-mesh /tmp/restore/.spectyn-mesh` | 0 diff | CUJ-05 tar.gz roundtrip (✅ #139) | ⬜ | ✅ ship |
| MAC-CUJ05-EXP-005 | integ | ✅ | TempDir | `spectyn backup` (missing --to) | exit 非 0 + 印 "--to" hint | required-flag | ⬜ | ✅ ship |
| MAC-CUJ05-EXP-006 | integ | ✅ | TempDir 無 ~/.spectyn-mesh/ | `spectyn backup --to /tmp/x.tar.gz` | exit 非 0、不留垃圾 | first-run guard | ⬜ | ✅ ship |
| MAC-CUJ05-DEL-001 | integ | ✅ | TempDir + events | `spectyn data delete --all --yes` | exit 0、`ls events/` 空 | E004 delete | ⬜ | ✅ ship |
| MAC-CUJ05-DEL-001a | integ | ✅ | post-DEL-001 | `ls ~/.spectyn-mesh/identity.key && ls ~/.spectyn-mesh/agents.toml` | 兩檔仍存 | scope invariant | ⬜ | ✅ ship |
| MAC-CUJ05-DEL-002 | e2e | ⚠ broker mock | mock broker DELETE | `spectyn data delete --all --yes --include-broker` | broker DELETE 收 req | broker wipe (🔴 #140) | ⬜ | 🔴 待 #140 |
| MAC-CUJ05-DEL-003 | integ | ✅ | TempDir | `spectyn data delete` (no --all) | exit 非 0 + "require --all" | safety flag | ⬜ | ✅ ship |
| MAC-CUJ05-DEL-004 | integ | ✅ | TempDir | `spectyn data delete --all` (no --yes) | exit 非 0 + confirm prompt | confirm flag | ⬜ | ✅ ship |
| MAC-CUJ05-REI-001 | e2e | ⚠ 2-step | backup → delete-all → reinstall + restore | `tar -xzf backup.tar.gz -C ~/; spectyn habit streak --chip water` | 之前 event 全可解讀 | reinstall roundtrip (🔴 #141) | ⬜ | 🔴 待 #141 |
| MAC-CUJ05-REI-002 | manual | ❌ | reinstall + 沒 identity | (open spectyn) | 印「沒 key 無法解 events」hint | no-key-import UX (🔴 #141) | ⬜ | 🔴 待 #141 |
...
| ID | Type | Auto | Setup | cmd | expected | Verifies | last_run | 狀態 |
|---|---|---|---|---|---|---|---|---|
| MAC-PLAT-LCD-001 | e2e | ✅ | TempDir | `spectyn service install` | LaunchAgent .plist 寫入 | SPEC-40 LaunchAgent | ⬜ | ✅ existing |
| MAC-PLAT-LCD-002 | e2e | ✅ | post-LCD-001 | `launchctl kickstart -k gui/$(id -u)/ai.spectynmesh.serve` | service 重啟 | LaunchAgent restart | 2026-05-31 02:25 (install.sh 用) | ✅ |
| MAC-PLAT-LCD-003 | e2e | ✅ | post-LCD-001 | `spectyn service uninstall` | .plist 刪、service 停 | LaunchAgent teardown | ⬜ | ⬜ |
...
| MAC-PLAT-PRO-001 | manual | ❌ | 新 build | `./spectyn --version` | 不被 Gatekeeper 擋 | provenance OK exec | 2026-05-31 02:25 | ✅ |
| MAC-PLAT-PRO-002 | manual | ❌ | new build + TCC default | `cp target/.../spectyn /tmp/p` | "Operation not permitted" | provenance blocks cp | 2026-05-31 02:30 | ✅ (今天碰過) |
| MAC-PLAT-PRO-003 | manual | ⚠ sudo | new build | `sudo xattr -c <path>; cp <path> /tmp/p` | cp 可進行 | provenance clear via sudo | 2026-05-31 02:05 | ✅ |
| MAC-PLAT-SPT-001 | manual | ❌ | post first-run | `mdfind ~/.spectyn-mesh/events.sqlite` | 不出現 (Spotlight 不 index 或不洩明文) | SPEC-13 P4 Spotlight | ⬜ | ⬜ |
...
| MAC-PERSONA-003 | e2e | ❌ Intel | x86_64 Mac + 假設 (a) 終裁已落地 | (發行後) `install-mac.sh` 在 Intel 上 | exit 0 + `~/.local/bin/spectyn` 存在 + `spectyn --version` 印 `darwin-x64`/`x86_64` | **[未來驗收 / 阻於 operator 裁決 (a)]** 若選兌現 persona 則此 case 必須轉綠 | ⬜ | ⬜ BLOCKED: 待裁決(a)+x86_64 artifact (現況不存在 → 必 FAIL) |
...
| ID | Type | Auto | Setup | cmd | expected | Verifies | last_run | 狀態 |
|---|---|---|---|---|---|---|---|---|
| MAC-INV-P4-001 | integ | ✅ | TempDir + 1 event | `head -c 30 ~/.spectyn-mesh/events/*/meta.json \| xxd` | 開頭是 age-v1 magic (`age-encryption.org/v1\n`) | events metadata 加密（含 metadata，**取代** P4-002） | ⬜ | ✅ as-built 正解：`storage.rs` 在有 `identity.key` 時把 `meta.json` 整檔 age 加密（`write_file:126–134` + `:229`），故 metadata 加密保證即由此 cell 涵蓋。證據 test `encrypted_store_writes_age_format_on_disk`（`storage.rs:381–402`）|
| MAC-INV-P4-002 | integ | ❌ | post-FH-001 | ~~`sqlite3 ~/.spectyn-mesh/events.sqlite "SELECT length(metadata_json) FROM events LIMIT 1"`~~ | ~~length > 0 + base64/binary~~ | metadata 加密（已併入 P4-001） | ⬜ | ♻️ DRIFT/NOT-IMPL(P0-1)：as-built **無** sqlite `events` 主表、**無** `metadata_json` 欄（`CREATE TABLE events`/`INSERT INTO events`/`metadata_json` 寫入於 `core/src` 0 命中；`events.sqlite` 只有 FTS5 `fts5_events`）。此 cmd 會 sqlite error（no such table: events）→ 必誤判。metadata 加密保證改由 **P4-001**（加密 `meta.json`）驗；本 cell 標 retired，待 SPEC-16 §7.1.1 sqlite 主表落地（v0.7.0+）再復用 |
| MAC-INV-P4-003 | integ | ✅ | TempDir + identity | `grep -r "$(head -c 16 ~/.spectyn-mesh/identity.key)" ~/.spectyn-mesh/events/` | 0 matches | identity 不洩 events | ⬜ | ✅ |
...
>
> **驗收核心不變量 (CLDSTART-INV)**: 在「全新 HOME、0 事件」下，所有召回 / 回顧 / 教練動詞**必須** exit 0 (或對 coach 為 exit 0 友善退化) + 印**引導式空狀態文案** (指向 `spectyn food`/`spectyn note`)，**絕不** 回裸 error、空表、或 Rust panic/backtrace。
>
> **測試名對齊**: surface-mac-cli.md UX finding #1 fix 建議的 `T-mac-cli-cold-start-empty`；CLDSTART-001~003 為其分解 case。

| ID | Type | Auto | Setup | cmd | expected | Verifies | last_run | 狀態 |
|---|---|---|---|---|---|---|---|---|
| MAC-CLDSTART-001 | integ | ✅ | 全新 isolated HOME (mktemp)，0 事件、有 identity.key | `spectyn recall water` | exit 0 + stdout 含引導文案 (如 "No events yet" / "尚無事件" + 指向 `spectyn food`/`spectyn note`)；**不** 印裸 error 也**非**空白 | surface-mac-cli §5 first-time-empty / BIG-GOAL ability-2C / SPEC-03 §8 | ⬜ | ⬜ [現況待驗] |
| MAC-CLDSTART-002 | integ | ✅ | 同上，0 事件 | `spectyn review` | exit 0 + stdout 含 "今日無資料" / "Nothing logged" 類友善空回顧 (review 為離線無 LLM，必 deterministic)；無 stack trace | surface-mac-cli §5 / SPEC-03 §8 no-dead-end | ⬜ | ⬜ [現況待驗] |
| MAC-CLDSTART-003 | integ | ⚠ mock LLM | 同上 + mock LLM (或 `--offline`)，0 事件 | `spectyn coach review --date today` | exit 0 + 印 shame-free "今日尚無紀錄" (零事件不得回 LLM error；空輸入應 short-circuit 不打 provider)；對齊既有 COA-002 (空 events.sqlite → "昨日無資料") | surface-mac-cli §5 finding#1(b) / SPEC-23 G1 0-data graceful | ⬜ | ⬜ [現況待驗] |
| MAC-CLDSTART-004 | integ | ✅ | 全新 HOME，0 事件 | `spectyn recall --semantic water` | **不得 panic**：應 fallback 到 FTS 並印 "(semantic recall 尚未可用 — 改用全文檢索)"；或 gate 為非零 exit + 友善 "Stage 4 (SPEC-25) 才有" | **[現況 FAIL / code-backlog]** skill_wire.rs:882 `embedding_search()` = `unimplemented!()` → runtime panic；surface-mac-cli §5「Semantic recall must NOT panic」+ finding#2 | ⬜ | 🟥 FAIL/code-backlog: as-built panic (BRK-1)；需 CLI 層在 panic site 前 gate |
| MAC-CLDSTART-005 | integ | ✅ | 全新 HOME，0 事件 | `spectyn skill store` (或 `spectyn skill run` 觸 store path) | **不得 panic**：應非零 exit + 友善 "Skill store 於 Stage 4 (SPEC-25) 落地；今天可用 recall 捕捉修正" | **[現況 FAIL / code-backlog]** skill_wire.rs:1127 `skill_store()` = `unimplemented!()` → runtime panic；surface-mac-cli §5 finding#2 (#1-differentiator 指令 panic 直接違反 moat) | ⬜ | 🟥 FAIL/code-backlog: as-built panic (BRK-1) |
| MAC-CLDSTART-006 | e2e | ⚠ 引導 | 全新 HOME，**剛跑完** onboarding (有 identity，0 capture) | `spectyn recall x; spectyn review; spectyn coach review --date today` 三連跑 | 三者皆 exit 0 + 皆印引導式空狀態 (無一回 error/panic)；模擬 onboarding 完成後最常見第一互動序列 | surface-mac-cli §5「most common first real interaction」/ INV-15 流程完整度 | ⬜ | ⬜ [現況待驗：004/005 路徑會 panic] |
| MAC-CLDSTART-007 | integ | ✅ | 全新 HOME，0 事件 | `spectyn recall water --json` | exit 0 + stdout 為**合法 JSON** (空陣列 `[]` 或 `{"events":[],"hint":"..."}`)，不得是空字串或非 JSON 文字 → 機器消費者 (手機 app) day-0 不炸 | surface-mac-cli §5 / cli-references §B-13 JSON 一致性 | ⬜ | ⬜ [現況待驗] |
| MAC-CLDSTART-008 | integ | ✅ | 全新 HOME，0 事件 | `spectyn data export --format json` | exit 0 + stdout 為合法空 JSON array (對齊既有 EXP-001 isolated-empty-vault 已 ✅)；day-0 匯出不報錯 | E004 export / surface-mac-cli §5 可逆性 day-0 | 2026-05-31 cargo | ✅ (既有 cuj05_data_export_json_emits_parseable_array 涵蓋空 vault) |

> **CLDSTART code-backlog 彙整 (docs-only，不修碼)**:
> - **CLDSTART-004 / 005 = [現況 FAIL]**：`spectyn recall --semantic` / `spectyn skill store` 今天 panic (skill_wire.rs:882/1127 `unimplemented!()`，= 母計畫 BRK-1)。修法屬 code：在 CLI 層 panic site 之前 gate 成友善 "not yet" (見 surface-mac-cli §5 finding#2 fix)，**非**本 docs slice 範圍。已記成可重複驗收：跑 cmd → 期望非 panic + 友善文案 → 現況 panic → FAIL。
...
TEMP=$(mktemp -d)
export SPECTYN_TEST_BIN=$(realpath core/target/aarch64-apple-darwin/release/spectyn)
...
HOME=$TEMP $SPECTYN_TEST_BIN habit water --qty 250 | grep -q streak  # FH-001
fts_rows=$(sqlite3 "$TEMP/.spectyn-mesh/events.sqlite" "SELECT count(*) FROM fts5_events")
test "${fts_rows:-0}" -ge 1  # FH-002; depends on indexer wiring (G9-FH004)
$SPECTYN_TEST_BIN habit "讀完 SICP ch3" | grep -q '✓\|streak'  # FH-006
HOME=$TEMP $SPECTYN_TEST_BIN data export --format json | jq -e '.events | length >= 1'  # EXP-001
$SPECTYN_TEST_BIN backup --to /tmp/backup-test.tar.gz && tar tzf /tmp/backup-test.tar.gz | grep -q identity.key  # EXP-004

# CUJ-03 cluster (assumes 1+ peer running)
$SPECTYN_TEST_BIN cluster status | grep -qE "✓|✗"  # CLU-004
...
appId: ai.spectynmesh.app
---
- launchApp
- assertVisible: "Spectyn Mesh"  # MAC-CUJ01-FH-007
- tapOn: "水"
- inputText: "250"
- tapOn: "✓"
...
osascript -e 'tell application "Terminal" to do script "spectyn habit water --qty 250"'
sleep 2
