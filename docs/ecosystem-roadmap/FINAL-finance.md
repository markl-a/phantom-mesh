# FINAL Development Plan — phantom-finance
...
> `D:\Projects\phantom-finance`. Every item below is checked against real source lines.

## What it is + current state

phantom-finance is the **personal-finance satellite of the phantom-mesh life-track ecosystem**:
...
emits a mesh `event` under `~/.phantom-mesh/events/` for phantom-companion cross-domain
...
`events.py`, `paths.py` (env-overridable: `PHANTOM_FINANCE_HOME` / `PHANTOM_MESH_HOME`), plus a
...
| **P2** | **Wire the local-first LLM fallback hook** | `LlmCategorizer` signature is fixed (`categorize.py:55`) but never injected. Wire to the **phantom-mesh local model router only** (never a remote API by default) — financial privacy is the niche. Local rules win first; LLM only fills `uncategorized`. |
...
| **P3** | **`phantom skill` top-down query integration** | README Tier-3: answer "這個月外食多少?" via a skill that reads the ledger — the phantom-mesh user-facing surface. |
...
- Add a `phantom-finance rule add <keyword> <category>` subcommand that writes `rules.json` (keeps users out of Python).
...
- Build a `phantom-mesh router` adapter implementing `LlmCategorizer = Callable[[str], Optional[str]]`, calling the **local** model endpoint; hard-fail closed (return `None`) if no local router is reachable — never silently hit a remote API.
...
- **Added** beyond agy: recurring-charge detection, `phantom skill` query integration, and
