# FINAL Development Plan — phantom-flow

## What it is + current state
`phantom-flow` is the weakest satellite in the phantom-mesh ecosystem: a ~500-LOC local-first
YAML workflow runner (`phantom_flow/runner.py`) with a pluggable block registry
(http / regex / filter / llm-summarize / subprocess), a `phantom exec` LLM wrapper
...
- **P1 — Real test contract + CI beyond import-smoke.** pytest for `runner.py`
  ...
  (provider-noise cleanup, timeout/error fallback, `PHANTOM_FLOW_STUB_LLM`), all network/LLM-free.
- **P1 — Harden the subprocess boundary** in `PhantomLLM.complete` and `_block_subprocess`:
  reliable cross-OS `phantom exec` invocation, timeout, captured stderr, graceful degradation to
...
- ~~Cluster-aware dispatch (`phantom dispatch`, capability labels)~~ — explicit overclaim to
...
- ~~Sweep vendored LLM call-sites onto PhantomLLM~~ — eliminated once subtrees are deleted.
...
- `runner.py` tests: `_resolve` placeholder substitution, `_gate_passes` (`when`), block-registry
  ...
  offline.
- `llm_driver.py` tests: provider-noise stripping, timeout/error → fallback, `PHANTOM_FLOW_STUB_LLM`
  short-circuit (no network, no real `phantom`).
- CI: run `pytest` + `python -m phantom_flow.runner flows/<flow>.yaml --dry-run --json` as the gate
  (replace the import-only smoke).

### P1 — Harden subprocess boundary
- In `PhantomLLM.complete` / `_block_subprocess`: resolve the `phantom` binary robustly, set an
  explicit timeout, capture+surface stderr, and fall back to the stub on non-zero/timeout.
- Add a `PHANTOM_FLOW_STUB_LLM` deterministic path so tests and offline/sandboxed machines never
  shell out.
