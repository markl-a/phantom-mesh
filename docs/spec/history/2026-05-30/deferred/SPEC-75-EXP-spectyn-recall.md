# SPEC-75-EXP · spectyn recall — 時間 / 記憶層（time / memory layer）

> **EXP（experimental，實驗性）spec — 不是 v0.6.0 contract。** 本檔目的是把「未來想做」的設計輪廓畫出來，讓 v0.7.0+ cycle 開實作 spec 時有起點；§7-§10 等需要 wire-level（線層 = 程式對程式介面層）精度的章節在 EXP 階段刻意留白（OoS — out of scope，暫不做），不是漏寫。spectyn recall 是 spectyn-mesh 上的「時間層」——24/7 環境捕捉（ambient capture）+ 跨裝置 mesh + 端到端加密 + 三種表面（pull / realtime / push），把「上次 X 講啥」這種問題從「猜不到」變成「30 秒回放」。
...
| Field | Value |
|---|---|
| Spec ID | `SPEC-75-EXP-spectyn-recall` |
| Title | `spectyn recall — 時間 / 記憶層` (English subtitle: `spectyn recall — the time / memory layer`) |
...
| Epic | `v0.7.0+ EXP02 spectyn-recall` |
...
> - **age（加密格式 v1）** — Filippo Valsorda 設計的現代簡化加密格式，spectyn-mesh 既有 SPEC-13 標準
...
> - **mesh peer（網狀節點）** — spectyn-mesh 裡的一台機器（手機 / 筆電 / 桌機 / 平板都算）
...
2. **dogfood 反覆撞同一問題**——Operator 自己用 spectyn 一個月，最常想要的「就一個 query」是「上次跟 X 講過 Y 那段在哪」。既有 chat history 不夠（不涵蓋語音 + 螢幕）。
3. **競品 verified empty**——Rewind.ai 停運（2026-Q1），Limitless 把功能鎖在自家 hardware（不跨 OS），ChatGPT memory 只在 chatgpt.com 內。spectyn-mesh 的 cross-5-OS + E2E + no-cloud 是市場唯一未填的座標。
...
- [`SPEC-12-PROTOCOL-identity-keypair.md`](SPEC-12-PROTOCOL-identity-keypair.md) — chunk encryption key 從 `~/.spectyn-mesh/identity.key` HKDF-SHA256 派生。
...
- `[G1]` Substrate 在所有 5 個 OS（macOS / Windows / Linux / iOS / Android）的 spectyn-mesh peer 上能以 ≤ 8% 平均 CPU + ≤ 200 MB RAM 跑 continuous capture 24 小時不漏 chunk。`(verifies via: T-recall-substrate-24h-soak)`
...
- `[NG4]` **不做 RLHF（reinforcement learning from human feedback，人類回饋強化學習）**——recall 是記憶，不是訓練；想把 recall 拿去 fine-tune 模型是另一個 repo 的工作（`spectyn-training`）。
- `[NG5]` **預設不啟用**——install spectyn 後 recall substrate 預設關閉，需 user 在 native app 明示啟用 + 每個 source 個別勾選；不接受任何「裝完就開始錄」設計。
- `[NG6]` **不主動上傳到 LLM provider**——chunk 內容只送本地 SLM 做 transcript + embedding；若 user 顯式按「用 frontier model 重寫摘要」才送雲端，且要顯示一次性確認框。
- `[NG7]` **不做 always-on hotword 喚醒**（「Hey spectyn」之類）——substrate 是 continuous passive capture，不是 voice assistant；hotword wake 屬 SPEC-23 coach delivery 範疇。
...
- `[J1]` **When** 我跟媽通完電話 3 週後想起她講過某句話但記不清細節，**I want to** 在 spectyn 打字問「3 週前媽電話那段她說啥？」**so I can** 拿到 30 秒 transcript clip 確認原意，不必憑記憶腦補。 (→ G3)
...
- `[J4]` **When** 我在 Mac 跑 spectyn 一週、然後切到手機問同樣 NL query，**I want to** Mac 上 capture 的 chunk 在手機上也搜得到（cluster 內一致），**so I can** 不必記「哪段在哪台」。 (→ G6)
- `[J5]` **When** 我用 spectyn 6 個月後想知道半年前自己對某個 project 的 first reaction，**I want to** 用一句 NL query「半年前第一次提到 project X 在哪」就跳到當時 transcript，**so I can** 回到那個 mental state 重啟思考。 (→ G3)
- `[J6]` **When** 我換新筆電並 setup spectyn，**I want to** substrate 預設關閉、我主動勾選後才開始 capture，**so I can** 知道自己對 capture 範圍有完整掌控。 (→ G5, NG5)
...
擁有 3-5 台裝置 mesh、同時用 spectyn 做 work（code / research）+ life（食物 / focus / habit）的人。期待：recall 把「我這週做了啥」從碎片變整合視角，You-Echo Mirror 是週末自然反思的最低 friction 形式。

### 5.2 Privacy-conscious individuals（BIG-GOAL Audience #2）

記者、律師、研究員、異議人士。期待：substrate 端到端可驗證 broker / 雲端 / 任何第三方都看不到明文；opt-in 預設關閉；單一指令完全刪除（沿用 `spectyn data delete --all --yes`）。
...
| `recall::wipe_all()` | 沿用 `spectyn data delete --all --yes` 路徑，刪光所有 chunk |
...
- **per-source opt-in 預設關閉**——install spectyn 後 substrate 不會自動 capture；user 必須在 native app 設定頁主動勾 audio / screen / sensor 三選一以上才開始。
...
- `recall::wipe_all()` 沿用既有 `spectyn data delete --all --yes` 路徑
...
| R7 | Rewind.ai 復活 + cross-platform | 低-中 | 中（市場 narrative 弱化） | spectyn 的差異化在 mesh + E2E + no-cloud；narrative 強調這三點而非「我們有時間機」 |  |
...
**方案**：放棄 24/7 substrate，只在 user 主動按「現在記」按鈕或喚「Hey spectyn 記一下」hotword 時 capture。完全 consent-explicit、零法律風險、零電力風險。

**為何沒選**：摧毀本檔核心 value——「上次 X 講啥」這類 query 的前提是「user 當時沒意識到該記」。若每段都要 user 主動記，spectyn recall 就退化成「進階版備忘錄」，跟 Apple Notes + Otter.ai 沒差。BIG-GOAL line 29「陪你進步」+ line 17「越用越懂你」要求 spectyn 能「在 user 沒留意時也累積長期素材」——這需要 ambient。

**什麼條件會回來**：如果跨多管轄區法律風險評估後發現 ambient 在 spectyn-mesh 目標市場（北美 / 歐盟 / 日本 / 台灣）普遍不可行，可能退守此方案。

### 17.3 第三方 capture device（Limitless / Friend / Humane 風）

**方案**：spectyn-mesh 自己不做 capture wire，只當記憶層；capture 由 user 戴 Limitless 吊牌 / Friend 項鍊 / 智慧戒指 / smart glass 等專用 hardware 收，via BLE / USB sync 到 spectyn-mesh。
...
| Q7 | 法律 boilerplate 由 spectyn-mesh 維護還是引用第三方法律 SaaS？ | **自己維護一份 per-region 模板**，禁用任何 user-tracking 第三方 | v0.7.0 legal review |
...
- 同期姊妹候選：`scripts/ai/output/agent-1000/CANDIDATE-B-spectyn-personas.md`（persona 層；與本檔 paired flagship，獨立 spec 另寫）
