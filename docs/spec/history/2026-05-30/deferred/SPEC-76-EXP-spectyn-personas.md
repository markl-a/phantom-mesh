# SPEC-76-EXP · spectyn personas — 多重自我層 (persona / multi-self layer)
...
| Field | Value |
|---|---|
| Spec ID | `SPEC-76-EXP-spectyn-personas` |
| Title | `spectyn personas — 多重自我層 (persona / multi-self layer)` |
...
| Epic | `v0.7.0+ EXP03 spectyn-personas` |
...
| Depends on | `SPEC-25-SYSTEM-skill-extraction`（提供 skill / value / writing-style 來源資料）、`SPEC-75-EXP-spectyn-recall`（提供 life-log substrate；sibling EXP 平行撰寫中，本檔 reference 不阻塞）、`SPEC-14-PROTOCOL-llm-providers`（local SLM 走 provider trait，例如 mlx 或 ollama backend） |
...
**問題**：spectyn-mesh 目前的 agent（智能體 = AI 代理人）都是「外部助理」— 你問它答、你派 task（任務）它跑。但人在做重大決定時最需要的不是一個外部助理，而是「**多個我自己**」可以辯論、預演、陪伴。具體場景：(a) 接不接某 job offer（工作邀約）— 想聽自己的多版本意見；(b) 下週要面試 / 加薪 / 分手 — 想先跟對方的 digital twin（數位分身）adversarial rehearse（對抗式預演）幾次；(c) 24/7 ambient（環境式）想要一個本地、私密、用 life-log 形成的「第二人」陪聊但 cloud LLM 違反 P4。

**方案**：在 spectyn-mesh 上加一個 **persona 層**（人格層），與 SPEC-25 skill 層、SPEC-75 recall 層平行。Substrate（基板）為本地 SLM（small language model，小型語言模型，例如 Qwen-3B）+ spectyn recall 提供的 life-log + 從 skill 萃取的 values（價值觀）/ writing-style（書寫風格） + 可選 bone-conduction earbud（骨傳導耳機）通道。Persona 層提供三種 mode（模式）：**Mode B council（內在會議）** 5 個你的版本辯論 / **Mode C rehearse（對抗演練）** 對手 twin 模擬 / **Mode A solo（24/7 第二人）** ambient 陪伴。Ship order（出貨順序）為 B → C → A（先快、再痛點、最後 ambitious）。
...
> - **agent（智能體）** — spectyn 既有的角色化 LLM 執行單元，例如 `agent.coach` / `agent.master`
...
v0.6.0 cycle 焦點是 4 pillars + 2 tracks 主幹（mesh / multimodal / evolve / encryption）跟 7 個 Sunday-deadline epic（截止期限週末交付的史詩級任務）。spectyn personas 是「v0.6.0 GA（general availability，正式上市）後的 reward queue（獎勵佇列）」之一，跟 SPEC-75 spectyn recall 同列為下一波 sci-fi 級 flagship（旗艦功能）。

觸發此檔的觀察：
- Operator（操作者，本檔指 spectyn 擁有者）做重大決定時（接不接某 offer、要不要分手、面試準備）反覆出現「希望多個版本的自己辯論」「想先預演對方反應」的需求，但既有 `agent.coach` 是單一外部助理人格，不擅長扮演**你自己的多重版本**。
- bone-conduction earbud 在 operator 日常已成標配（跑步 / 通勤 / 散步），形成持續可用的低干擾互動通道，卻沒被 spectyn-mesh 任一現有 agent 使用。
- 本地 SLM（Qwen-3B 量級）在 Mac M-series + iPad / iPhone A-series 上已可達 100-200 tokens/s，到了「足夠 ambient 對話」的門檻 — 不必再讓「私人第二人」依賴 cloud LLM。

### 2.2 在 BIG-GOAL 哪裡

- **P3 進化網（Evolve Mesh）** — BIG-GOAL §Four pillars P3「Spectyn self-evolves with each user」。Persona 層是 self-evolution 的下一階：skill 層（SPEC-25）解決「我學會做什麼」，persona 層解決「我形成成什麼樣的角色」。council 5 forks 隨 life-log 累積會更貼近真實使用者的多面性，是 evolution 的具象呈現。
...
- [`SPEC-75-EXP-spectyn-recall.md`](SPEC-75-EXP-spectyn-recall.md) — sibling EXP（平行撰寫中）。提供 life-log substrate；本檔 reference 但不阻塞 — Mode A solo 在 SPEC-75 ship 前可先用 SPEC-16 event_storage 既有資料 degrade 運作。
...
- `[G1]` Operator 用 `spectyn personas --council "<decision>"` 在 ≤ 3 分鐘內得到 5 個 self-fork（自我分身）的辯論結果與 ranked recommendation（排序建議），全程本地 SLM。`(verifies via: T-personas-council-latency)`
- `[G2]` Operator 用 `spectyn personas --rehearse <topic> --counterparty <linkedin-url>` 可建立一個 counterparty twin 並進行至少 5 輪對話演練，twin 風格在 5 輪後仍未漂移成 agreement-machine（迎合機）。`(verifies via: T-personas-rehearse-adversarial-stability)`
...
- `[NG5]` **不上 spectyn-mesh broker 同步 persona state** — persona state 只跨 user 自己的 peer（節點）同步（透過既有 mesh + SPEC-13 encryption），broker 完全不介入。
...
- `[J1]` **When** 我收到一份 job offer 想決定要不要接，**I want to** 跑 `spectyn personas --council "should I accept this Acme offer?"` 讓 5 個我（年輕 / 年長 / 最佳 / 陰影 / 導師）辯論並給出排序建議，**so I can** 在做決定前看到自己內在多面性的整理，而不是只憑當下情緒。 (→ G1, G5)
- `[J2]` **When** 我下週要面試一位特定主管，**I want to** 跑 `spectyn personas --rehearse interview --counterparty linkedin.com/in/jane-doe` 跟一個用她公開風格建出的 twin 預演 5 輪，**so I can** 上場時不會被她的提問風格嚇到。 (→ G2)
...
- `[J4]` **When** 我擔心自己的 multi-self 樣本（最私密的內在對話）會不會被任何雲端服務看到，**I want to** 從 `spectyn personas --status` 確認所有 mode 皆為 local-only、broker 端 zero traffic，**so I can** 信任 P4 加密為先承諾在 persona 層也成立。 (→ G4)
...
- `[J6]` **When** 我第一次裝完 spectyn 還沒準備好用 persona 層，**I want to** persona 預設關閉、不會自動讀 life-log、不會佔 CPU 跑 SLM，**so I can** 在自己準備好 + 看清會用什麼資料前不被驚嚇。 (→ G6)
...
BIG-GOAL Audience #1，v0.6.0 主要目標族群。本檔最直接受眾：他們已經有用 spectyn 的習慣，Life Track 教練（coach）對他們有效；persona 層是把教練從「外部助理」升級成「多重自己」的下一步。期待：council 幫做 weekly review 時的「自我對話」、solo 幫日常情緒消化。
...
flowchart TB
    ...
    Sealed[("Persona state<br/>~/.spectyn-mesh/personas/<br/>（age sealed）")]
    ...
    Mesh[("spectyn-mesh peers<br/>（persona state sync via SPEC-13 encryption）")]
...
sequenceDiagram
    autonumber
    actor U as "使用者"
    participant C as "spectyn personas CLI"
    ...
    U->>C: "spectyn personas --council<br/>'should I accept Acme offer?'"
...
| 2 | **Mode B council** | ~2 週 | 最快可 dogfood；operator 現在就有決策需要（job offer / spectyn roadmap）；不需 hardware；純 LLM prompting |
...
### 9.1 CLI 子指令（`spectyn personas <subcmd>`）

| Subcommand | Purpose |
|---|---|
| `spectyn personas --council "<question>"` | 跑一次 5-fork 內在辯論，回 ranked recommendation |
| `spectyn personas --rehearse <topic> --counterparty <url-or-handle>` | 建 twin 並起一次 rehearse session |
| `spectyn personas --solo` | 起 Mode A solo 24/7 daemon（背景進程，需 earbud BLE 配對） |
| `spectyn personas --status` | 列當前 active mode、persona 數量、最後 SLM 推論時間（J4 路徑） |
| `spectyn personas --enable <mode>` / `--disable <mode>` | Opt-in 切換 |
| `spectyn personas --wipe <persona_id>` / `--wipe-all` | 刪除 persona state（reversible 原則） |
...
- Persona state P2P 同步的具體 wire protocol — **OoS**，沿用 spectyn-mesh 既有 mesh sync（SPEC-26 / SPEC-13），但具體 envelope schema 留 v0.7.0+。
...
| # | Surface | 載體 | 一句用途 |
|---|---|---|---|
| 1 | CLI panel | `spectyn personas --council` 終端輸出 | 純文字 ranked recommendation + 5 fork breakdown |
...
| Persona state on-disk per user | < 100 MB | < 1 GB hard cap | `spectyn personas --status` |
...
- **Persona state always sealed at-rest**。`~/.spectyn-mesh/personas/` 目錄下所有檔案以 user identity key（SPEC-13）age v1 seal；offline 機器抓硬碟也只看到 ciphertext（密文）。
...
- **Reversible**。`spectyn personas --wipe-all` 一個指令刪光所有 persona state；解除 BLE earbud 一個按鈕；停用某 mode 立即生效不留 background daemon。對齊 BIG-GOAL §Three operational principles 第三條。
...
| Information disclosure | 攻擊者讀 `~/.spectyn-mesh/personas/` | 該目錄全 sealed；唯一明文是 transient in-memory inference |
...
| **Reversible** | `spectyn personas --wipe-all`；mode 切換即時；earbud 拔對即停 |
...
**為何沒選**：
- 破 P4「資料加密，只你能讀」— 你最私密的 multi-self 樣本送上雲，是 spectyn-mesh 哲學的根本背叛。
...
- 缺 sci-fi 級 differentiator — 市場上類 *Her* 的 single-self companion app 已存在（Replika 系列）；spectyn 進場沒新貢獻。
...
**為何沒選**：
- **法律風險**：deepfake 立法在多國（美 / 歐 / 台）2024-2026 都在推緊；voice cloning 未經本人同意已開始被視為 unlawful（不合法）使用。spectyn-mesh 不想踩這條線。
- **倫理風險**：即使 user 個人使用「練面試」，voice clone 樣本一旦被 leak（如 spectyn 被駭、user device 被 forensic），可被用於詐騙 / 冒充。產品方有責任不創造此 attack surface。
...
**什麼條件會回來**：**none**。本檔承諾 NG2 永久禁。即使社群要求、即使法律允許，spectyn personas 不做 real-person voice cloning。
...
- 從產品差異化：市場上的 RLHF 迎合機已過剩（多數 frontier LLM 都偏 sycophancy）；spectyn 進場若再加一個，沒有貢獻。
...
- SPEC-75 spectyn-recall（life-log substrate，sibling EXP）
