...
| Source | 配色 | 語意 |
|---|---|---|
| SPEC-43 §8.1 | **綠點**（`phantom-tray-working.ico`） | cluster active task 的 working state（與 N peers running 同色） |
| hero mockup §412 | **phantom-warning 飽和橘** | 「請小心、正在錄音」warning 語意 |

**本檔決議：採 phantom-warning 橘**。理由：
...
3. **計時器配色一致** — hero mockup §548 cross-platform invariant：「Recording 中計時器 = `phantom-warning`」。tray icon 跟主視窗計時器同色（橘）視覺連續、user mental model 一致
4. **Recording 中 tray icon 跟主視窗形成 ambient 提示** — user 漏看主視窗、瞄一眼右下角橘色 mic icon 立刻知「我還在錄」；綠點容易跟 Slack / Discord 「online」綠混淆

→ **本檔覆寫 SPEC-43 §8.1 line 637 Recording 配色為橘**；SPEC-43 §8.1 綠點仍適用於「cluster active task」非 Focus 情境。建議 SPEC-43 下版同步：把 §8.1 第 5 列「**focus-recording: phantom-tray-focus.ico（phantom-warning 橘）**」加進 state matrix，與 working 並列。
...
|---|---|---|
| `phantom-bg` | `#0f0f1a` | Start window 內容區背景（OS title bar 不染） |
| `phantom-card` | `#1a1a2e` | duration picker chip / button group bg |
| `phantom-warning` | `#ff9800` | **Tray icon Recording 配色（拍板）** + 計時器 + waveform |
| `phantom-danger` | `#dc3545` | Stop button bg + Tray icon Error state |
| `phantom-muted` | `#8888aa` | Tray icon Idle + secondary text + Paused state |
| `phantom-primary` | `#8ab4f8` | Start button bg + toast action button |

**Fluent design token 對映**：本檔**不採 Fluent native palette**（如 SystemAccentColor）— 維持 phantom brand consistency（同 Android 不採 Material You）。Windows 11 user 若想要系統色，v0.7+ 評估 Settings 加「Use system accent」toggle，預設關。

**Mica acrylic 半透明**：**v0.6.0 不做**（per wireframe §開放問題 #5 已決 + SPEC-43 §3.2 NG2 — Tauri 2 binding 未穩 + node-a black flash 踩坑）。Start window 採實心 `phantom-bg` 背景。
...
- **Toast**：**OS-rendered**，phantom 不控字體（XML 只控文字內容）
...
| Key | zh-TW | en |
|---|---|---|
| `focus.tray.header_idle` | Phantom Mesh · {peers} 節點 · 待機 | Phantom Mesh · {peers} peers · idle |
| `focus.tray.header_recording` | Phantom Mesh · Focus {elapsed} / {total} | Phantom Mesh · Focus {elapsed} / {total} |
...
│ Start Focus Session              [_][□][X]   │   bg: OS theme（不染 phantom-bg）
├──────────────────────────────────────────────┤   ← divider 1px Win11 stroke
│                                              │  bg: phantom-bg（內容區）
│                                              │
│              00:00 / 25:00                   │   display 48px/700, phantom-muted (idle)
│                                              │
│  Duration:                                   │   body-lg 16px/500, phantom-text, padding-left 24px
│  ┌────┐ ┌────┐ ┌────┐                       │   chip group: 3 × 64×32px, gap 8px
│  │ 15 │ │ 25 │ │ 50 │  min                   │   selected: bg phantom-primary, text phantom-bg, radius 6px
│  └────┘ └────┘ └────┘                       │   unselected: bg phantom-card, text phantom-text, border 1px phantom-border
│                                              │
│              ┌──────────────────┐            │   Start btn: 240×40px, radius 6px
│              │  ▶  開始計時錄音  │            │   bg phantom-primary, text phantom-bg, body-lg/600
│              └──────────────────┘            │   icon: Lucide `play` 16px phantom-bg
│                                              │
│         🔒 本地加密 · 麥克風 ASR                │   caption 12px/400, phantom-muted, centered
└──────────────────────────────────────────────┘  ← window border：Win 11 1px stroke + 8px corner radius
```

**Visual states**：
- duration chip：idle (card bg) / hovered (bg lighten 8%) / selected (primary bg) / focused (2px phantom-primary outline ring，鍵盤 navigation 必有)
- Start button：idle / hovered (primary @ 90%) / pressed (primary @ 80%) / focused (2px phantom-primary outline ring inset 2px) / disabled (`overlay-disabled-40`)
...
```
┌──────────────────────────────────────────────┐  覆蓋層: bg phantom-bg @ 92% on Start window
│                                              │
│         [mic-disabled-icon-64]               │  Lucide `mic-off` 64×64, phantom-danger
│                                              │
│   系統已停用麥克風，請至設定開啟              │  body-lg 16px/500, phantom-text, centered
...
- 「打開設定」button：bg phantom-primary, text phantom-bg, radius 6px, icon Lucide `settings` 16px → deep-link `ms-settings:privacy-microphone`
- 「重試」button：bg phantom-card, text phantom-text, border 1px phantom-border, radius 6px → re-invoke `WASAPI` init
...
|---|---|---|---|---|
| Idle | Lucide `mic` 16×16 | `phantom-muted` (#8888aa) | "Phantom Mesh · idle" | "Phantom Mesh microphone, idle" |
| **Recording**（**橘 — 本檔拍板**） | Lucide `mic` 16×16 | **`phantom-warning` (#ff9800)** | 取 `focus.tray.tooltip_recording` | "Phantom Mesh recording, {elapsed} elapsed" |
| Paused | Lucide `mic-off` 16×16 | `phantom-muted` | "Focus 已暫停 — 右鍵繼續" | "Phantom Mesh paused, right-click to resume" |
| Error | Lucide `mic` 16×16 + 紅點 overlay 6×6 右下角 | `phantom-danger` (#dc3545) | "麥克風錯誤 — 右鍵看選項" | "Phantom Mesh microphone error" |
...
- `phantom-tray-idle.ico` — Idle
- **`phantom-tray-focus.ico`**（新增） — Recording（橘）
- `phantom-tray-paused.ico`（新增） — Paused
- `phantom-tray-error.ico` — Error
...
│ Phantom Mesh · Focus 05:23 / 25:00          │  header 灰 disabled, italic, body 14px
...
├──────────────────────────────────────────────┤
│ Open Phantom Mesh                  Ctrl+O    │  row 32px
│ Settings...                                  │
└──────────────────────────────────────────────┘
```
...
```xml
<toast launch="phantom-mesh://focus/{session_id}" scenario="default">
  <visual>
    <binding template="ToastGeneric">
      <text id="1">Phantom Mesh</text>
      ...
      <image placement="appLogoOverride" hint-crop="circle"
             src="ms-appx:///assets/phantom-mono-icon-44.png"/>
    </binding>
  </visual>
  <actions>
...
```
┌─────────────────────────────────────────────┐  Win 11 toast，OS-rendered，phantom 只控 XML
│  ◯  Phantom Mesh                            │  AppLogo 44×44 circle crop, phantom mono icon
...
**XML 屬性決議**：
- `launch="phantom-mesh://focus/{session_id}"` — toast body click（非 button）的 fallback（per SPEC-43 §9.3 deep-link）
...
- `activationType="protocol"` — 走 `phantom-mesh://` deep-link 啟動主視窗（per SPEC-43 §9.3 同 coach review 機制）
- AUMID anchor：`com.phantom-mesh.app`（per SPEC-42 §8.5，MSI 安裝時 shortcut metadata 註冊；無 AUMID → `R.windows.toast_emit_fail` 退化）
...
```xml
<toast launch="phantom-mesh://focus/{session_id}/stop" scenario="urgent">
  <visual>
    <binding template="ToastGeneric">
      <text id="1">Phantom Mesh 焦點時段中斷</text>      <!-- focus.desktop.interrupt_notif_title -->
...
- **action button label** 取 `focus.desktop.interrupt_notif_action`「開啟並停止」 — 點下去 launch `phantom-mesh://focus/{id}/stop` deep-link、activate 主視窗 + invoke stop
...
- Interrupted toast（`scenario="urgent"`）正常穿透；若 user 在 Win 11 Settings 完全關掉 toast → 退化到主視窗頂部 banner（`focus.windows.focus_assist_fallback` 文案、icon `triangle-alert` 16px phantom-warning、bg `overlay-recording-16`、可點關閉）
...
- **F takeaway card 樣式**：bg `phantom-card`, radius 12px（不是 hero mockup §278 iOS 16pt — Win 11 標準 corner 12px），padding 20px, max-width 640px（主視窗 sidebar 220px + 內容區）
...
**Win 11 corner radius 慣例**：所有 phantom-controlled element 採 6px（小元件）或 12px（card / dialog）— 與 Win 11 Fluent 規範 corner radius 對齊（system menu / toast OS 自帶 8px）。
...
- **Tray icon Recording = `phantom-warning` 橘**（本檔拍板，覆寫 SPEC-43 §8.1）— 與 macOS NSStatusItem 紅點 / Linux StatusNotifierItem 橘維持「Recording = warning 系」桌面三平台對齊
...
| **空白（Empty — History）** | History list in main window left sidebar（無 session）| Lucide `inbox` SVG 64px phantom-muted + `focus.empty.history` 文案 + "前往 Focus" button (bg phantom-primary, 24×8px padding) |
...
| **錯誤（Error）** | B' Mic disabled / D' Interrupted toast / Focus Assist fallback banner | B' 覆蓋層 phantom-bg @ 92% + `mic-off` 64px phantom-danger / D' toast `scenario="urgent"` + Alarm2 audio / fallback banner `triangle-alert` 16px + `overlay-recording-16` bg |
| **局部（Partial）** | E Finalizing inline `focus.partial.chunk_failed` | 同 hero mockup §260 — body-sm phantom-warning inline 於 progress bar 下方 |
| **載入中（Loading）** | E Finalizing + tray icon 橘 + tray header 動態更新 | spinner 32px phantom-warning stroke + progress bar 240×4px + tray menu header 取 `focus.finalizing.asr` 即時更新 |

## 已決（per wireframe lock + 本檔拍板）

1. ~~Tray icon Recording 配色（綠 vs 橘）~~ → **已決**：**phantom-warning 橘**（本檔拍板，理由見 §「Tray icon Recording 配色 — 終值拍板」；覆寫 SPEC-43 §8.1 line 637，建議 SPEC-43 下版同步補「focus-recording」row）
...
7. ~~Corner radius~~ → **已決**：phantom-controlled element 6px（小） / 12px（card） — 對齊 Win 11 Fluent 慣例
...
3. **Tray icon DPI scaling Recording 配色一致性**：16/32/48 三 frame 預 render phantom-warning 橘是否在 200% scaling 機（node-a）與 100% scaling 機（node-b）肉眼一致？需 ico 檔產線測試。
