...
5. **GTK theme follow**（Adwaita / Yaru / Breeze）— phantom token 在 dark/light 兩模式都要不違和
...
Linux 不像 Android 有 Material 3 tonal palette 強制對映 — 直接用 phantom token，但要跟 GTK theme 在「dark/light 切換」與「accent color」上不打架：

| GTK 環境 | phantom 對應策略 |
|---|---|
| GTK dark（Adwaita-dark / Yaru-dark / Breeze-dark） | phantom dark token 直接用（見 hero mockup §design token 速查），window bg `phantom-bg` 對 GTK `@theme_bg_color` |
...
| GTK accent color（GNOME 47+ / KDE） | **不 follow** — 為保 phantom brand 一致（per hero invariant `focus.trust_badge` 跨平台一字不差），accent 永遠用 `phantom-primary` |
...
- **color**：icon 走 currentColor，由父元素 token 決定（`phantom-primary` / `phantom-warning` / `phantom-danger` / `phantom-muted`），不在 SVG 內 hardcode 色
- **tray icon mono**：SNI 規範 single-color，**用 phantom mono variant**（無漸層、無多色） — 讓 KDE 自動 tint 到 panel theme 色不打架
...
│ 開始焦點時段                   [_][□][✕] │  ← GTK CSD titlebar（系統繪），WM_CLASS=phantom-mesh
│ ────────────────────────────────────│  divider 1px phantom-border
│                                      │
│ 時長：                                │  title-sm phantom-text
│  ○ 25 分鐘 Pomodoro                  │  radio rows 32px, accent phantom-primary
│  ○ 50 分鐘                            │
│  ◉ 自訂： [ 30 ] 分鐘                 │  custom input 60×28px, radius 6px, bg phantom-bg
│                                      │
│ 目標標籤（選填）：                     │  title-sm phantom-text
│ ┌──────────────────────────────┐    │  text input full-width, height 32px,
│ │ deep_work, spec_writing       │    │   bg phantom-bg, radius 6px, padding 8px
│ └──────────────────────────────┘    │   placeholder "輸入標籤…" phantom-muted
│                                      │
│ 🔒 本地加密 · 麥克風 ASR              │  caption phantom-muted（取 `focus.trust_badge` — 一字不差）
│                                      │
│      [ 取消 ]      [ 開始 ]          │  Cancel 96×32px, bg transparent, text phantom-muted
│                                      │  Start 96×32px, bg phantom-primary, text phantom-bg
└──────────────────────────────────────┘
```

**Visual states**：
- radio row: idle / selected（`phantom-primary` outer ring + filled dot） / hover（bg `phantom-card` lighten 8%）
- Start button: idle / hover（`phantom-primary @ 90%`）/ pressed（`phantom-primary @ 80%`）/ disabled（`overlay-disabled-40`）
- Cancel button: idle / hover（text `phantom-text`）
- text input focus: border `phantom-primary` 2px
...
- WM_CLASS = `phantom-mesh`（tiling WM rule 認得）
- `.desktop` 內 `StartupWMClass=phantom-mesh` 對齊（per SPEC-45 §7.1 `LinuxScreenSpec`）
...
```
┌──────────────────────────────────────┐  bg: phantom-bg（覆蓋 Start window 或 main window Focus tab）
│                                      │
│           [mic-off-icon]             │  Lucide `mic-off` 64px, phantom-danger
│                                      │
│        找不到麥克風裝置               │  title (24px/600) phantom-text, centered
│                                      │
│      請檢查系統音訊設定               │  body phantom-muted, centered, line-height 1.5
│                                      │
│   ┌──────────────────────────┐      │  Open-settings btn: 240×40px centered, radius 8px
│   │  ⚙  打開音訊設定          │      │  bg phantom-primary, text phantom-bg, body-lg/600
│   └──────────────────────────┘      │  icon: Lucide `settings` 18px phantom-bg
│                                      │
└──────────────────────────────────────┘
```

**Visual states**：
- Open-settings btn: idle / hover（`phantom-primary @ 90%`） / pressed（`phantom-primary @ 80%`） / **disabled**（`overlay-disabled-40`，當測不到任何 audio control panel binary 時）
...
```
┌──────────────────────────────────────┐  bg: phantom-bg, Focus tab 內或獨立 GTK window
│             05:23 / 25:00            │  display (48px/700) phantom-warning
│                                      │
│  ▁▂▃▅▇▇▇▅▃▂▁                          │  waveform: 32 bars × 4px wide × 2px gap,
│                                      │   height 0-120px dynamic, color phantom-warning
│                                      │
│  ┌──────┐    ┌──────────┐            │  Pause: 120×56px, radius 12px, bg phantom-card,
│  │ ⏸暫停│    │ ⏹ 停止    │            │   icon Lucide `pause` 20px phantom-secondary
│  └──────┘    └──────────┘            │  Stop: 120×56px, radius 12px, bg phantom-danger,
│                                      │   icon Lucide `square` 20px phantom-text
│  📁 已落地 chunk: 3                   │  body phantom-muted, icon Lucide `folder` 14px
...
**Visual states**：
- Pause button: idle / hover（bg `phantom-card` lighten 8%） / pressed（lighten 12%）
- Stop button: idle / hover（`phantom-danger @ 90%`） / pressed（`phantom-danger @ 80%`）
- waveform：recording 期間 dynamic update（fps 由 Prototype 鎖）；interrupted 切 phantom-muted 凍結
...
|---|---|---|---|
| **Idle**（未錄音） | `mic` outlined | phantom mono（panel theme tint） | `Passive` |
| **Recording** | `mic` filled | phantom-warning（橘） | `Active` |
| **Paused** | `mic-off` | phantom-muted（灰） | `Active` |
| **Interrupted**（app 進背景時） | `mic` filled + dot overlay | phantom-warning + phantom-danger 小圓點 6px | `NeedsAttention` |

**SNI 限制**：
- single-color SVG —「橘 vs 灰 vs danger 點」靠 SVG 內 `currentColor` + 兩層 path（base mono + overlay color）達成
- 但 GNOME Shell 內建 indicator 模塊**強制 monochrome panel 色**，phantom 的 warning/danger 色可能被吃掉 — 此情境**仰賴 main window title** 補充狀態（title 加 `[Recording]` prefix）
...
│ 🔴 Focus 05:23/25:00          │  disabled row (italic), body phantom-warning
│ ──                            │  divider
│ ⏹ 停止並收工                  │  body, Lucide `square` 16px phantom-danger
│ ⏸ 暫停                        │  body, Lucide `pause` 16px phantom-secondary
│ ──                            │
│ Open Phantom Mesh             │  body
└──────────────────────────────┘
```
...
GTK window 內視覺等同 hero iOS C'（waveform 退色 phantom-muted、計時器退到 phantom-muted、Stop 仍可按）— 但 **desktop 強制系統通知**（per hero invariants + Linux wireframe §C'）：

```
[libnotify D-Bus org.freedesktop.Notifications — interrupted]
┌─────────────────────────────────────────┐  渲染由 dunst / mako / GNOME Shell / Plasma 處理
│ [phantom mono icon 32]                  │  bundled SVG mono variant
│  Phantom Mesh 焦點時段中斷               │  summary 取 `focus.desktop.interrupt_notif_title`
│                                         │
...
- `desktop-entry = phantom-mesh`（連結回 .desktop file）
...
- spinner: 32px CSS animated circle, stroke 3px phantom-warning（webview 內，**不用 GTK native spinner** — 為跟 mac/Win mockup 視覺一致）
- progress bar: 240×4px, fg phantom-warning, bg phantom-border
...
┌─────────────────────────────────────────┐
│ [phantom mono icon 32]  Focus complete  │  summary（en：`focus.done.title` 縮短版）
│                                         │
│  25 min · 5 chunks · takeaway ready     │  body（結構化 metadata，**不放 takeaway 內容**）
│                                         │
│  [ 開啟 ]                                │  action → `phantom://focus/done/<session_id>` deep link
└─────────────────────────────────────────┘
```
...
- 卡片視覺：success icon 改 `triangle-alert` 18px phantom-muted（不是 danger，是「值得注意但非錯誤」）+ 兩個 button「重錄這次」/「完成」
...
- **GTK CSD 真窗 + WM_CLASS=phantom-mesh** — tiling WM rule 認得
...
- **GTK accent color NOT follow** — phantom-primary 永遠 `#8ab4f8`（保 brand trust 一致性）

## 6 大資料狀態 — Linux Mockup 視覺對映

| 狀態 | 視覺 |
|---|---|
| **理想（Ideal）** | F Done card 完整三段 takeaway + Lucide `check-circle` 64px phantom-success + libnotify low 通知 |
| **空白（History）** | main window Focus tab 內 history 區：mono SVG illustration 192px phantom-muted + `focus.empty.history` + 「前往 Focus」按鈕 |
| **空白（ASR 無語音）** | F 卡片 Lucide `triangle-alert` phantom-muted + `focus.empty.no_speech` 安撫文 + 重錄/完成雙按鈕（**libnotify 不發**） |
| **極限（Limit）** | C chunk `99+` chip / F takeaway > 800 字截斷（hero invariant — fade-out gradient + 「看完整摘要」CTA） |
| **錯誤（Error）** | B' No-mic device-error（Lucide `mic-off` 64px phantom-danger + 「打開音訊設定」CTA）/ Interrupted libnotify critical / `LINUX-SCREEN-LIBNOTIFY-NO-SERVICE` headless server fallback main window inline 訊息 |
| **局部（Partial）** | E `focus.partial.chunk_failed` inline Lucide `triangle-alert` 14px phantom-warning |
| **載入中（Loading）** | E spinner-32 phantom-warning + progress bar 40% + tray icon attention 變體（best-effort） |
...
2. **GTK accent color 不 follow** — 保 phantom brand trust 一致
...
4. **Lucide SVG `currentColor` 在 GNOME Shell SNI mono 限制下失效**：interrupted attention 變體的 phantom-danger 紅點可能被吃掉變灰。提案：接受視覺降級（per Linux best-effort），main window 加同步提示補強。
