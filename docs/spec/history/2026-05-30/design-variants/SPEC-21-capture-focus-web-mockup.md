...
繼承 hero `phantom-*` 全部 token + 字級 type ramp。Web 額外使用 overlay tier：

| Overlay token | 數值 | Web 用途 |
|---|---|---|
| `overlay-web-warn-20` | `phantom-warning @ 20%` | A1 / A2 caveat banner（idle）背景 |
| `overlay-web-warn-30` | `phantom-warning @ 30%` | C Recording caveat banner（加深變體）背景 |
| `overlay-error-16` | `phantom-danger @ 16%` | C' upload-failed 卡片背景 |
...
| `focus.web.upload_failed` | C' | C' 卡片 title（body-lg, `phantom-danger`） |
...
- **PTT 大鈕**：圓 96×96px，bg `phantom-primary` 邊框 2px，icon `mic` 32px center；按住變 bg `overlay-ripple-24`（同 Android ripple opacity，但用 CSS `:active` 而非 Material ripple）
- **duration picker**：3 顆 chip（25 / 50 / custom）28px 高、radius 14px、bg `phantom-card`、selected 加邊框 1px `phantom-primary`
- **Timer 副按鈕**：full-width 48px 高、bg `phantom-card`、icon `play` + label `focus.btn.start_timer`
- **trust badge**：底部 caption `focus.trust_badge`，`phantom-muted`
- **bottom nav**：4 tabs（同 iOS bottom-nav 結構，per hero invariants History tab lock 表），56px 高，bg `phantom-card`
...
- **計時器顯示** `display` 48px / 700 / `phantom-muted`，centered
- **radio rows** 3 行：每行 32px 高、left padding 16px，radio circle 16px + label `body-lg`；selected 圓填 `phantom-primary`
- **Start 按鈕** 320×48px centered，bg `phantom-primary`、text `phantom-bg`、icon `play` 20px + label `body-lg`；hover bg 加 10% 亮度、active 加 `overlay-ripple-24`
- **無 PTT 按鈕**：桌機鍵盤情境不適合 press-and-hold（hero R6 已決）→ **PTT × Timer 互斥 invariant 在 A2 不適用**（per hero mockup §551，僅 A1 適用）
- **trust badge**：caption `focus.trust_badge`，centered，`phantom-muted`
...
│                                               │  border-bottom 1px phantom-warning
└───────────────────────────────────────────────┘  text body-sm, phantom-warning, padding 8px 16px
...
**Browser native dialog — 不可自訂**（per Wireframe §103-106）。視覺由 Chrome / Safari / Firefox 各自決定，phantom 完全無 control。
...
- **icon** `mic-off` Lucide 48px `phantom-danger`
- **主文** `focus.perm.denied` body-lg `phantom-text`
- **安撫文** `focus.perm.denied_reassure` body `phantom-muted`
- **「打開設定」deep link Web 不可實現** → 改顯示 in-card 步驟提示，文案取 `focus.web.perm_settings_hint`：
  ```
  ┌──────────────────────────────────┐
  │  [mic-off icon 48px phantom-danger] │
  │                                  │
  │  需要麥克風才能 focus 錄音         │  body-lg
  │  我們不會上傳音訊到雲端...           │  body, phantom-muted
  │                                  │
  │  ┌──────────────────────────┐    │  bg phantom-card, padding 12px
  │  │ 請從瀏覽器設定恢復麥克風權限 │    │  body-sm, phantom-muted
...
- **waveform** 32 bars × 4px wide × gap 2px，高 0-100px dynamic，color `phantom-warning`
- **計時器** `display` 48px / 700 / `phantom-warning`（recording 中色）
- **Pause / Stop** 兩鈕並列：Pause 96×40px `phantom-card` + `pause` icon / Stop 96×40px `phantom-danger` + `square` icon
- **chunk count chip** 右下角，bg `phantom-card`、radius 12px、icon `folder` + 數字
...
- **卡片背景** `overlay-error-16`（`phantom-danger @ 16%`）— 全寬覆蓋 C Recording 容器
- **title** `focus.web.upload_failed`，body-lg `phantom-danger`，centered，icon `wifi-off` 20px gap 8px
- **waveform 凍結** — 灰色 `phantom-muted`，停止繪製（視覺上明顯區別於 C 的 `phantom-warning` 動態）
- **Retry button**（96×40px）：bg `phantom-danger`、text `phantom-text`、**主要救濟**（user 期望「先試重連」） — 放左、視覺權重高
- **Save-offline button**（144×40px）：bg `phantom-card`、text `phantom-text`、**次要救濟**（fallback path） — 放右、視覺權重低
...
- **ASR / LLM 全跑 host**（phantom-serve 那台），browser 只 render — 視覺上跟 iOS 完全一樣（spinner + progress bar + 文案）
- **E spinner**：用 CSS animation 或 Lucide `loader-2` + `animate-spin` Tailwind class，32px `phantom-primary`
- **F success icon**：Lucide `check-circle` 64px `phantom-success`
...
- **error toast** 全寬 48px 高，bg `phantom-danger @ 95%`，text `phantom-text`，body-lg
...
| **空白（History）** | History tab in-tab，mono SVG illustration 192px `phantom-muted` + `focus.empty.history` + 「前往 Focus」`phantom-primary` 按鈕 |
...
| **錯誤** | B' Denied 卡（Lucide `mic-off` `phantom-danger` icon + in-card `focus.web.perm_settings_hint`，**無 deep-link button**）/ **C' upload-failed**（Web 獨有，`overlay-error-16` 卡片 + retry × save-offline 雙救濟）|
...
1. **C' Retry vs Save-offline 視覺權重**：目前提案 Retry 主色（`phantom-danger` bg）/ Save-offline 次要（`phantom-card` bg），但 user 真正常用路徑可能是 save-offline（host 真不可達時 retry 浪費時間）→ v0.7+ 看遙測數據再考慮對調
