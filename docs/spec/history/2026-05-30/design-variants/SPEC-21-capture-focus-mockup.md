...
| Token | Hex | 用途 |
|---|---|---|
| `spectyn-bg` | `#0f0f1a` | screen background |
| `spectyn-card` | `#1a1a2e` | card / sheet / modal surface |
| `spectyn-border` | `#2a2a3e` | divider / outline |
| `spectyn-primary` | `#8ab4f8` | primary action (Start / Stop) |
| `spectyn-secondary` | `#bb86fc` | secondary action (Pause) |
| `spectyn-success` | `#4caf50` | done state / trust badge accent |
| `spectyn-warning` | `#ff9800` | recording state (icon, waveform) |
| `spectyn-danger` | `#dc3545` | stop / destructive |
| `spectyn-text` | `#e0e0e0` | primary text |
| `spectyn-muted` | `#8888aa` | secondary text / placeholder |

### Overlay opacity（從散落值收成 token）

| Token | 數值 | 用途 |
|---|---|---|
| `overlay-recording-16` | `spectyn-warning @ 16%` | iOS recording 時 nav bar tint |
| `overlay-web-warn-20` | `spectyn-warning @ 20%` | Web caveat banner（idle） |
| `overlay-ripple-24` | `spectyn-primary @ 24%` | Android Material ripple |
| `overlay-web-warn-30` | `spectyn-warning @ 30%` | Web caveat banner（recording 強化版） |
| `overlay-error-16` | `spectyn-danger @ 16%` | Web C' upload-failed 背景 |
| `overlay-disabled-40` | element opacity 40% | PTT/Timer disabled 全用此（不換色，只壓透明度） |
| `overlay-denied-72` | `spectyn-bg @ 72%` | iOS B'（Denied）遮罩 |
...
| `focus.desktop.interrupt_notif_title` | Spectyn Mesh 焦點時段中斷 | Spectyn Mesh focus interrupted |
...
│                            │
│      ◯  00:00 / 25:00      │  clock-face: 240×240pt dim circle stroke 2pt spectyn-border
│                            │  text: display, spectyn-muted (idle)
│                            │
│  ┌─────────┐ ┌──┐ ┌──────┐│  pill buttons 32pt height, radius 16pt
│  │ 25 分鐘 │ │50│ │ 自訂 ││  selected: bg spectyn-primary, text spectyn-bg
│  └─────────┘ └──┘ └──────┘│  unselected: bg spectyn-card, text spectyn-text
│                            │
│ ┌────────────────────────┐ │  PTT btn: full-width minus 16pt margin,
│ │   ⏺  按住說話           │ │  height 96pt, radius 24pt
│ └────────────────────────┘ │  bg spectyn-card, border 2pt spectyn-primary
│  (large primary)           │  icon: SF "mic.fill" 32pt
│                            │  text: body-lg, spectyn-primary
│ ─── 或 ───                  │  divider with caption text, spectyn-muted
│                            │
...
│                            │
│ 🔒 本地加密 · 麥克風 ASR    │  trust badge: caption, spectyn-muted, centered
└────────────────────────────┘
```

**Visual states**：
- 25/50/custom pill: idle (card bg) / selected (primary bg, bg text) / pressed (primary @ 80% opacity)
- PTT button: idle / pressed (border thickens to 3pt, bg spectyn-card lighten 8%) / disabled (40% opacity, when perm denied)
- Timer button: idle / pressed (primary @ 80% opacity) / disabled

**Icons**：
- back chevron — SF `chevron.left` 18pt spectyn-text
- settings — SF `gearshape` 18pt spectyn-text
- PTT — SF `mic.fill` 32pt spectyn-primary
- Timer start — SF `play.fill` 20pt spectyn-bg

### 螢幕 C — Recording (Timer mode)

```
┌────────────────────────────┐  bg: spectyn-bg
│             05:23 / 25:00  │  display/600, spectyn-warning (recording accent)
│                            │
│  ▁▂▃▅▇▇▇▅▃▂▁              │  waveform: 120pt height, 32 bars, spectyn-warning
│                            │
│ ┌──────┐    ┌──────────┐  │  Pause: 120×56pt, radius 12pt, bg spectyn-card,
│ │ ⏸暫停│    │ ⏹ 停止    │  │   icon spectyn-secondary, text spectyn-text
│ └──────┘    └──────────┘  │  Stop: 120×56pt, radius 12pt, bg spectyn-danger,
│                            │   text spectyn-text, icon SF "stop.fill" 20pt
│ 📁 已落地 chunk: 3          │  body, spectyn-muted, icon: SF "folder.fill" 14pt
...
**Recording 中 nav bar 改色**：頂部 nav bar tint 從 `spectyn-bg` → `overlay-recording-16`（`spectyn-warning @ 16%`），作為「我在錄」的全螢幕提醒。

### 螢幕 B' — Denied（覆蓋在 Idle 上的遮罩卡）

```
┌────────────────────────────┐  bg: overlay-denied-72（spectyn-bg @ 72% on Idle 之上）
│                            │
│      [mic-denied-icon]     │  SF "mic.slash.circle.fill" 64pt, spectyn-danger
│                            │
│  需要麥克風才能 focus 錄音   │  title, spectyn-text, centered
│                            │
│  我們不會上傳音訊到雲端，    │  body, spectyn-muted, centered, line-height 1.5
│  ASR 也跑在本機。            │
│                            │
│ ┌────────────────────────┐ │  Open-settings btn: 240×48pt centered, radius 12pt
│ │     打開設定            │ │  bg spectyn-primary, text spectyn-bg, body-lg/600
│ └────────────────────────┘ │  icon: SF "gearshape" 18pt spectyn-bg
│                            │
└────────────────────────────┘
```

**Visual states**: Idle 螢幕仍可見背景但 PTT 跟 Timer button 都套 `overlay-disabled-40`（40% opacity，無 hover/pressed）。Open-settings 按鈕 pressed 狀態 `spectyn-primary @ 80%`。

### 螢幕 C' — Interrupted（Recording 變體，waveform 凍結）

```
┌────────────────────────────┐  bg: spectyn-bg
│             05:23 / 25:00  │  display, spectyn-muted (從 warning 退色到 muted)
│                            │
│  ▁▂▃▅▇▇▇▅▃▂▁              │  waveform: spectyn-muted bars（凍結，無 update）
│                            │
│  電話中已暫停                │  body-lg, spectyn-warning, centered
│  30 秒內回復將自動繼續        │  body-sm, spectyn-muted, centered
│                            │
...
┌────────────────────────────┐  bg: spectyn-bg
│                            │
│        [spinner-32]        │  centered, 32pt spinner, spectyn-warning stroke
│                            │
│   整理逐字稿 (2/5)          │  body-lg, spectyn-text, centered
│   ████████░░ 40%           │  progress bar 240×4pt, fg spectyn-warning, bg spectyn-border
│   ⚠ 轉文字失敗 (chunk 3/5)，│  body-sm, spectyn-warning, centered（partial — 僅在某 chunk ASR fail 時顯示，inline 於 progress 下方）
│      已跳過                  │
│                            │
│   產生 takeaway 中…         │  body, spectyn-muted (pending — 灰，等 ASR 完才亮色)
│                            │
│   取消並先看逐字稿           │  caption, spectyn-muted, underline, tap target
└────────────────────────────┘
```

**Visual states**: 兩個 phase 並列 — phase 1 (ASR) 跑時 progress 走、訊息一在 spectyn-text、訊息二在 spectyn-muted；phase 2 (LLM) 接手時訊息二切到 spectyn-text。Partial 訊息（`focus.partial.chunk_failed`）僅在 chunk ASR fail 時 inline 出現。「取消並先看逐字稿」action 行為由 [Prototype](./SPEC-21-capture-focus-prototype.md) 層鎖定，本檔僅定視覺。

### 螢幕 F — Done (Takeaway card)

```
┌────────────────────────────┐  bg: spectyn-bg
│  ✓ 完成 · 25 分鐘 · 5 chunks│  title-sm, spectyn-success, icon SF "checkmark.circle.fill"
│  ────────────────────       │  divider 1pt spectyn-border
│                            │
│  ┌──────────────────────┐  │  takeaway card: bg spectyn-card, radius 16pt, padding 20pt
│  │ 主要 ideas:           │  │   title-sm, spectyn-text
│  │  • [line 1]          │  │   body, spectyn-text, line-height 1.5
│  │  • [line 2]          │  │
│  │                      │  │
│  │ Action items:        │  │
│  │  • [...]             │  │
...
│  │ [看完整摘要] CTA      │  │   gradient overlay spectyn-card → transparent；
│  └──────────────────────┘  │   底部 inline "看完整摘要" tap target（body-sm, spectyn-primary, underline）
│                            │
│ ┌─────────────┐ ┌────────┐ │  view-full: 56pt height, bg spectyn-card
│ │ 看完整逐字稿 │ │ 新 session│ │  new: 56pt, bg spectyn-primary, text spectyn-bg
│ └─────────────┘ └────────┘ │
└────────────────────────────┘
```

**Limit state — Done card 截斷**：takeaway 字數 > 800 時，卡片 max-height 480pt + 底部 64pt fade-out gradient（`spectyn-card → transparent`）+ inline hint「（摘要過長已截斷）」（取 `focus.limit.takeaway_truncated_hint`）+ CTA「看完整摘要」（取 `focus.limit.view_full_takeaway`）。Tap CTA 等同點 `[看完整逐字稿]` 主按鈕。
...
- B2 (POST_NOTIFICATIONS) skip 後的 Idle 頂部多一條提示 bar：
  ...
  ```
  bg `spectyn-card`（subtle info），body-sm `spectyn-muted`，high 32dp，可滑掉
...
│ 開始焦點時段                  [✕]    │  title 24px/600 spectyn-text
│ ────────────────────────────────    │  divider spectyn-border
│                                      │
│ 時長：                                │  title-sm spectyn-text
│  ○ 25 分鐘 Pomodoro                  │  radio rows: 32pt height
│  ○ 50 分鐘                            │  selected radio: spectyn-primary
│  ◉ 自訂： [ 30 ] 分鐘                 │  custom input: 60×28pt, radius 6pt, bg spectyn-bg
│                                      │
│ 目標標籤（選填）：                     │
...
│                                      │  Start: 96×32pt, bg spectyn-primary, text spectyn-bg
└──────────────────────────────────────┘
```
...
│ 🔴 Focus 05:23/25:00      │  body-lg spectyn-warning + display-sm time
...
Recording 中 OS interrupt 觸發、主視窗非 active focus 時必發。spectyn 設：
...
- Idle: Lucide `mic` 16×16, spectyn-muted single-color
- Recording: Lucide `mic` 16×16 spectyn-warning（飽和橘）
- Paused: Lucide `mic-off` 16×16 spectyn-muted

### Tray context menu

Windows 標準 menu 樣式（system theme），spectyn 只控 label：
...
├──────────────────────────────────────────────┤
│                                              │
│              00:00 / 25:00                   │  display, spectyn-muted, centered
│                                              │
...
│        │  ▶  開始計時錄音    │                │  bg spectyn-primary, text spectyn-bg, body-lg
│        └────────────────────┘                │  label 取 `focus.btn.start_timer`
│                                              │
│        🔒 本地加密 · 麥克風 ASR                 │  caption, spectyn-muted, centered
└──────────────────────────────────────────────┘
```
...
│ ⚠ 瀏覽器模式：請保持本頁開啟  │  bg overlay-web-warn-20, border-bottom 1px spectyn-warning
│   切走會中斷錄音              │  text body-sm, spectyn-warning
└──────────────────────────────┘
```
...
```
┌──────────────────────────────┐  bg: overlay-error-16
│ ⚠ 上傳到 host 失敗            │  body-lg, spectyn-danger, centered
│  ▁▂▃▅▇▅▃▂                     │  waveform: spectyn-muted（凍結）
│                              │
│ ┌────────┐  ┌──────────────┐│  Retry: 96×40px, bg spectyn-danger, text spectyn-text
│ │ 重試    │  │ 暫存到瀏覽器 ││  Save-offline: 144×40px, bg spectyn-card, text spectyn-text
│ └────────┘  └──────────────┘│
│ 🔒 本地加密                  │
└──────────────────────────────┘
```
...
- **計時器顏色**：idle = spectyn-muted，recording = spectyn-warning，interrupted = spectyn-muted，done = spectyn-success
- **Stop 按鈕**：永遠 spectyn-danger 背景；mobile 螢幕內 ≥ 96pt 寬置於最右；desktop tray/menu 內列第一項（横向 toolbar 取最右、直向選單取最上）
- **PTT button**（mobile only）：永遠 ≥ 96pt 高，永遠 spectyn-primary 邊框
...
- **Takeaway card 尺寸**：永遠 spectyn-card 背景 + radius 16pt + padding 20pt；mobile max-width 480pt；desktop main-window 640pt；可滾、可截斷（見 F Limit state）
- **Error toast**：spectyn-danger 背景 95% opacity，48pt 高，全寬；toast 自動消失行為由 Prototype 鎖（mockup 只定靜態樣式）
- **Error background overlay**（非 toast 的 error 區塊背景，例：Web C' upload-failed）：`overlay-error-16`（`spectyn-danger @ 16%`）
...
| **空白（Empty）** | History tab（無 session） | **本檔僅列規格**：mono SVG illustration 192pt spectyn-muted + `focus.empty.history` 文案 + "前往 Focus" 按鈕。ASCII frame **defer 到平台 catalog**（SPEC-31 iOS / SPEC-34 Android / SPEC-41 macOS / SPEC-43 Windows / SPEC-45 Linux）— 因 wireframe §OoS3 已標 History tab 出本 spec 範圍 |
...
| **錯誤（Error）** | B' / Web C' | B' Denied 遮罩（spectyn-danger icon + open-settings CTA）/ Web upload-failed（overlay-error-16 + retry / save-offline 雙按鈕）/ 全平台 error toast 規格 |
| **局部（Partial）** | E Finalizing | 「轉文字失敗 (chunk 3/5)，已跳過」inline 訊息 spectyn-warning，繼續 stitch 其他 chunks |
| **載入中（Loading）** | E Finalizing | spinner-32 + progress bar 40%（ASR）+ pending 訊息（spectyn-muted → 切到 spectyn-text 表示進度推進）|

## 開放問題（mockup 層面）

1. **計時器數字顏色**：recording 期間應該全程 `spectyn-warning`（高警示）還是只在「過半」後變色？目前提案：全程 warning。
