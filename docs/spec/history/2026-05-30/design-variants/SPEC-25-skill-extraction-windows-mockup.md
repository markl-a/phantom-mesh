...
| Token | Hex | skill 用途 |
|---|---|---|
| `phantom-bg` | `#0f0f1a` | Skills tab 背景 |
| `phantom-card` | `#1a1a2e` | 左列表 row / 右詳情卡 |
| `phantom-card-warm` | `#221a2e` | **apply banner 背景**（暖塊 — 同 SPEC-23 coach action「邀請」色溫，學習是正向） |
| `phantom-primary` | `#8ab4f8` | 「跨機分享」鈕、apply「好」鈕 |
| `phantom-success` | `#81c995` | quality 高分點、套用次數正向 |
| `phantom-warning` | `#ff9800` | quality 低分點（建議檢視，非錯誤） |
| `phantom-muted` | `#6b6b80` | Archival/低分 skill 灰階、provenance 文字、quality 空格點 |
...
|---|---|---|---|
| `>= 0.8` | ▮▮▮▮▮▮▮▮▯▯ | `phantom-success` 綠 | 命中表現好，優先 recall |
| `0.5 - 0.8` | ▮▮▮▮▮▮▯▯▯▯ | `phantom-primary` 藍 | 中等，照常 recall |
| `< 0.5` | ▮▮▮▯▯▯▯▯▯▯ | `phantom-warning` 橘 | 表現差，列表沉底 / 建議 Decline 或降到 Archival（**不染紅** — 低分不是 error） |
...
| 來源 | `git-commit` | provenance 行首，12px phantom-muted（Work Track 技能；Life Track 食物/習慣 skill 改用 `sparkle`） |
...
|  Rust 錯誤... cargo [Core]| [Core] 品質 ▮▮▮▮▮▮▮▮▯▯ 套用12 |   quality 10 格進度條 phantom-success
| Recall (23)               | 觸發：任務含「Rust 編譯錯」    |   trigger phantom-muted
|  git rebase 衝突   [Rec]  | last applied：今天 09:14     |
|  壓圖到 720p       [Rec]  | (git-commit) 來源：5/20-5/27  |   provenance phantom-muted 12px
...
- 詳情行動鈕（§10.1 Screen B）：**Edit**（pencil）/ **Decline**（thumbs-down，降 score）/ **Delete**（trash，phantom-danger，需 confirm）/ **Promote to Core**（arrow-up-circle）+ **跨機分享**（share-2 → `/rpc/skill/sync`）
- **只 Delete 用紅**（phantom-danger，破壞性）；Decline / 低分 / tier 都不染紅
...
```
+----------------------------------------------------------+
| (sparkle) 套用了你上次的做法：Rust 錯誤先看 cargo check  |   phantom-card-warm 暖塊
|   會先跑 cargo check 讀第一個 error                       |   說明 phantom-muted 13px
|                          [ 好 ]  [ 這次不要 ]  [ 詳情 ]  |   好=phantom-primary 主鈕
+----------------------------------------------------------+
```
- **暖塊（phantom-card-warm）+ sparkle** — 語氣「我學會了、幫你套上」，**不是 alert / warning**（無橘無紅無驚嘆號）
- 「好」phantom-primary（預設、不點也採用、不阻塞）；「這次不要」phantom-muted 文字鈕（decline → 本次 skip + measure 負回饋）；「詳情」→ 跳 Skills tab
...
| 多用幾天，phantom 會從你的重複做法裡學會自動套用          |   empty.body 14px phantom-muted
| （要 ≥ 5 次重複才會抽成技能，避免雜訊）                   |
```
