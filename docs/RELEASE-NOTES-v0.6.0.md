...
- **E003 教練節點 SHIPPED**：`phantom coach review --date` CLI、`daily_review.rs` 聚合器（經 E004 解密）、shame-free prompt lint、`--save` 加密寫入 `reviews/<date>.md`、Gemini→Groq→Ollama 降級鏈，外加 spec 之外的每日排程器（`coach_scheduler.rs` + `phantom serve` 內嵌 21:00 timer）。
...
- `PHANTOM_KEYSTORE=file` 環境覆寫，讓無 Secret Service 的環境可控退回檔案儲存（#314 / `c38ade2a`）。
...
- **`phantom exec --json` AgentEvent JSONL 契約**：hermetic 跨平台 schema 測試（#317 / `b545909a`）——遠端控制器消費的機器可讀介面從此有測試釘住。
...
2. **E005 技能庫 feature-gated 且未接線**（`experimental-memory`，預設 off）：即使旗標編譯，`phantom serve` 也不會把 `state.skill_memory` 接成 `Some`，三個端點回 503。`phantom skill extract --commit` CLI、F402 recall-trace 端點、與 serve 端正式接線**延後至 v0.7**（位於 lineage 管制檔案內）。FTS5 p99 < 200ms 從未實測；FTS5 用 `unicode61` tokenizer，**CJK 斷詞未測試**。萃取器本體（`from_daily_review.rs`）預設編譯且有測試。
...
7. **切版前雜項**（不擋出貨但要做）：~~版本號飄移需收斂（core=`0.6.0-rc.1`、pm-types=`0.6.0`、app/src-tauri=`0.6.1`）~~ **[已解 — drift 回填 2026-06-12]** 四個 manifest 已全部收斂為 `0.6.0`：`core/Cargo.toml`、`app/package.json`、`app/src-tauri/Cargo.toml`、`app/src-tauri/tauri.conf.json` 皆 `version = "0.6.0"`；`phantom --version` / `--version --short` 的字串唯一來源為 `env!("CARGO_PKG_VERSION")`（裁決依據：`core/src/bin/phantom.rs:3718`/`:3722`），故實機輸出即 `0.6.0`（無 `-rc.1`、無 `0.6.1`）。舊的 `v0.6.0-rc1` tag（05-25 @ `f4a5c7e0`）仍過時，需操作者決定重切或補 rc2；epic spec 驗收勾選與 F600 計分板的中文標題漂移需在 06-13 凍結檢查前修。
...
與 rc1 §5/§8 相同（事件前向讀取、deep-link 重註冊、OAuth 惰性重加密；`git pull` → rebuild → `phantom selftest` → 30 秒 demo），無新增遷移步驟。Windows 使用者首次啟動會把身分種子遷入 DPAPI/Credential Manager（自動，無需動作）。
...
- **2026-06-12（P1-5 版本收斂 drift 回填）** — §7 條目 7 的「版本號飄移需收斂」標為已解：四個 manifest 已全部為 `0.6.0`（讀碟核實 `core/Cargo.toml`、`app/package.json`、`app/src-tauri/Cargo.toml`、`app/src-tauri/tauri.conf.json`）；以 `core/src/bin/phantom.rs:3718`/`:3722`（`env!("CARGO_PKG_VERSION")`）為 as-built 裁決 `--version` 輸出 = `0.6.0`。core/tests 版本斷言（`cli_win.rs`/`cli_linux.rs`/`v5_smoke_linux.rs`/`v4_e2e_desktop_*.rs`）本已為結構性/容忍式（`X.Y.Z[-pre]`、semver-ish ≥2 dots），切版至 `0.6.0` 不會 false-FAIL，無需修改。
