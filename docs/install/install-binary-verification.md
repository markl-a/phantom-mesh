...
每個 `spectyn` / `spectyn-mesh` 安裝腳本都會在**標記為可執行或移入 PATH（執行路徑）之前**，
...
| 攻擊者 | 未驗證 | 有驗證 |
|---|---|---|
| **線路上的 MITM（中間人攻擊）** — 任何能劫持 `http://coord/dist/spectyn-…` 請求的一方 | 替換掉二進位檔；使用者會以自身身分執行它（並透過 `spectyn service install`，在每次登入時執行）。 | 透過 HTTPS 取得的 sidecar 不會與被掉包的二進位檔吻合 → 安裝中止；二進位檔被刪除。 |
...
```
https://github.com/markl-a/spectyn-mesh/releases/download/v0.1.0/spectyn-mesh-x86_64-linux
https://github.com/markl-a/spectyn-mesh/releases/download/v0.1.0/spectyn-mesh-x86_64-linux.sha256   ← 由安裝程式取用
https://github.com/markl-a/spectyn-mesh/releases/download/v0.1.0/SHA256SUMS                         ← 參考清單
```

每個 sidecar 的格式正是 `sha256sum <name>` 的輸出：

```
26d39c…b71e  spectyn-mesh-x86_64-linux
```
...
1. **不要為了「讓它跑起來」而以 `SPECTYN_SKIP_VERIFY=1` 重新執行。**
   檢查失敗的整個重點，就在於有東西出錯了。
...
3. 把 `expected`（預期）那一行與 GitHub 上最新的 release notes（發行說明）
   （`https://github.com/markl-a/spectyn-mesh/releases`）比對。release 頁面
   內嵌了 SHA256SUMS 的內容。
...
- `.github/workflows/publish-spectyn-binary.yml` — 在每次 R2 上傳時發出每個資產的
  `.sha256` sidecar
