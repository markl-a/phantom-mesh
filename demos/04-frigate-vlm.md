...
> "Your security camera saw 'person'. But was it a delivery guy? Your kid? An intruder? phantom-mesh asks a vision LLM — locally — and only pings you when it matters."

## Cast / setup
- **門口攝影機**(IP cam / 環門口或入口的視角)
- **Frigate NVR**(Linux box / Synology / Pi):跑著,偵測到 person
- **node-a / GPU 機**:跑 phantom + vision provider(Pixtral / GPT-4V / Claude Vision)
...
| 0:05-0:12 | Frigate 截圖 → 送到 phantom-mesh(箭頭動畫) | "It asks phantom-mesh: 'who is this, and why?'" |
| 0:12-0:25 | node-a TUI:`[ToolCall: vlm_describe(image=...)]` → `[ToolResult: a delivery person carrying a brown box, walking toward the door]` | "Phantom routes the image to a vision LLM, running locally on the gaming PC." |
...
| 0:48-0:55 | 反例對比:同一鏡頭路過鄰居 → 沒通知(Phantom 不認識為 delivery) | "Your kid walking past? No alert." |
| 0:55-0:60 | End card + 「contrast with: dumb motion alerts spam」 | — |

## Pre-record checklist
- [ ] Frigate 跑著,接 cam stream,person detection 開
- [ ] phantom 加 vision provider(等 v0.7.0)
- [ ] phantom 收 Frigate webhook(自寫 small Python 接 Frigate MQTT → call phantom)
