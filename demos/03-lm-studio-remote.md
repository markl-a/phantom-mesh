...
> "Your MacBook at a café, your home 4090 sitting idle. With phantom-mesh, LM Studio sees the 70B model running at home — zero port forwarding."

## Cast / setup
- **MacBook**(咖啡店):LM Studio 視窗,model picker
- **node-a / GPU rig**(家裡):4090 跑 Llama 70B via Ollama / vLLM behind phantom
...
| 0:10-0:18 | 顯示「+ Add remote provider」→ 貼一行 URL `https://node-a.tailnet.ts.net:7878/v1` | "Add phantom-mesh as a provider." |
...
| 0:57-0:60 | End card:phantom-mesh + "your hardware, anywhere" | — |

## Pre-record checklist
- [ ] node-a 跑 `phantom serve --port 7878` + 後面接 Ollama 或 vLLM 跑 70B
...
- 強調:**no port forwarding 是 Tailscale 的 magic**,phantom 是 model 路由
