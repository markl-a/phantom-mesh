...
- **Phone（手機）**：iPhone，Telegram 開著，與 `@my_spectyn_mesh_bot` 對話
...
| 時間 | 螢幕上的畫面 | 旁白 / 字幕 |
|---|---|---|
| 0:00-0:03 | 標題:"spectyn-mesh — one message, three machines" | — |
...
| 0:53-0:60 | 結尾卡片（end card）:spectyn-mesh logo + URL + GitHub stars badge（星星數徽章） | "spectyn-mesh.io" |

## 錄製前檢查清單（Pre-record checklist）

- [ ] 3 台機器都在 Tailscale 上,用 `spectyn peer list` 確認 peer list（節點清單）
- [ ] Telegram bot allowlist（允許清單）包含示範用 Telegram account
- [ ] node-c MLX 跑著（`spectyn mlx serve`）+ node-b spectyn 跑著
