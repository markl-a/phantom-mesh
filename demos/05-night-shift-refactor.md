...
**Status（狀態）**: 🟡 v0.6.0(`spectyn autoevolve schedule` 已 ship,但要拍 demo 等夜間 footage（影像素材）)
...
- **GitHub 介面**: PR list,8 個 PR 都有 spectyn 簽名 + 通過 CI
...
| 0:05-0:12 | Time-lapse:spectyn autoevolve TUI 滾動,終端不斷有任務跑出 | "Your gaming PC keeps working." |
| 0:12-0:22 | 鏡頭拉近 terminal:`spectyn autoevolve --watch --target check --max-rounds 5` 跑著,每 hour 一輪 | "Every hour: cargo check → if red, try to fix → if green, refactor or test." |
...
| 0:42-0:53 | 點 GitHub PR — 看到 spectyn 寫的 PR 描述 + diff + 全部 CI 綠 | "Each PR has a clear description, a clean diff, and CI passing." |
| 0:53-0:60 | End card:"sleep is when you ship the most" + spectyn-mesh logo | — |

## 預錄製檢查清單
- [ ] node-a / gaming PC 上 `spectyn autoevolve schedule install --interval 3600`
- [ ] `EVOLVE-GOALS.md` 預先寫 5-10 個小目標(refactor X / add test for Y)
- [ ] git 設好 push 權限 + PR 模板(spectyn 用得到)
...
## 字幕中的誠實聲明（caveat）
- "PRs still need human review before merge. spectyn drafts; you decide."
