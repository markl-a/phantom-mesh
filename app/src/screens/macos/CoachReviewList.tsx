    ...
    <div className="max-w-2xl mx-auto space-y-5" data-testid="coach-review-list">
      <header className="flex items-center gap-3">
        <div className="w-10 h-10 rounded-lg bg-phantom-primary/15 flex items-center justify-center">
          <ClipboardList size={20} className="text-phantom-primary" />
        </div>
        <div className="flex-1">
          <h1 className="text-xl font-bold text-phantom-text">教練回顧</h1>
          <p className="text-xs text-phantom-muted">Coach reviews · 最近 {SCAN_DAYS} 天有紀錄的日子</p>
        </div>
        <button
      ...
      {error && (
        <div
          className="bg-phantom-warning/10 border border-phantom-warning/40 rounded-lg p-3 text-sm text-phantom-warning flex items-center justify-between gap-3"
          role="alert"
        >
          <span className="min-w-0 break-words">{error}</span>
          <button
            onClick={() => void refresh()}
            className="flex-shrink-0 text-xs px-2 py-1 rounded border border-phantom-warning/40 hover:bg-phantom-warning/15"
          >
            重試
          </button>
        </div>
      ...
      {loading && entries.length === 0 && !error && (
        <div className="space-y-2" data-testid="review-list-skeleton" aria-busy="true">
          {[0, 1, 2, 3].map((i) => (
            <div key={i} className="h-14 rounded-lg bg-phantom-card border border-phantom-border animate-pulse" />
          ))}
        </div>
      )}

      {!loading && !error && entries.length === 0 && (
        <div className="bg-phantom-card border border-phantom-border rounded-lg p-6 text-center" data-testid="review-list-empty">
          <p className="text-sm text-phantom-text">最近 {SCAN_DAYS} 天還沒有可回顧的紀錄。</p>
          <p className="text-xs text-phantom-muted mt-1">
            用專注 / 習慣 / 飲食頁記錄一筆後，當天就會出現在這裡。
          </p>
          <p className="text-[11px] text-phantom-muted/70 mt-2">空白的日子沒關係 — 這是紀錄，不是評分表。</p>
        </div>
      )}

      {anyLocked && (
        <div className="bg-phantom-card border border-phantom-border rounded-lg p-3 text-xs text-phantom-muted flex items-center gap-2">
        ...
        <div className="space-y-1.5">
          {entries.map((e) => (
            <Link
              ...
              className="flex items-center gap-3 px-4 py-3 rounded-lg bg-phantom-card border border-phantom-border hover:border-phantom-primary/40 transition group"
              data-testid={`review-row-${e.date}`}
            >
              <div className="flex-1 min-w-0">
                <p className="text-sm text-phantom-text font-medium">{e.date}</p>
                <p className="text-xs text-phantom-muted">{dayLabel(e.date)}</p>
              </div>
              {e.locked ? (
                <span className="text-[11px] text-phantom-muted flex items-center gap-1">
                  <Lock size={11} /> 已加密
                </span>
              ) : (
                <span className="text-[11px] px-2 py-0.5 rounded-full bg-phantom-primary/10 text-phantom-primary">
                  {e.eventCount} events
                </span>
              )}
              <ChevronRight size={16} className="text-phantom-muted group-hover:text-phantom-primary transition flex-shrink-0" />
            </Link>
          ))}
        </div>
      )}
