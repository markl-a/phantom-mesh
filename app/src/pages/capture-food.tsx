      ...
      <div className="flex min-h-screen flex-col">
        <main className="flex-1 overflow-y-auto px-4 pb-6 pt-5">
          ...
          <section
            ...
            className="mb-5 flex items-start gap-3 rounded-lg border border-spectyn-border bg-spectyn-card px-3 py-3"
          >
            <Camera aria-hidden="true" className="mt-0.5 size-5 shrink-0 text-spectyn-warning" />
          ...
          <section className="space-y-3">
            <label className="block">
              ...
              <textarea
                ...
                className="min-h-[120px] w-full resize-none rounded-lg border border-spectyn-border bg-spectyn-card px-3 py-2 text-base text-spectyn-text placeholder:text-spectyn-muted"
              />
            </label>
            ...
            {result && (
              <div role="status" className="space-y-3 rounded-lg border border-spectyn-border bg-spectyn-card px-3 py-3">
                <p className="flex items-center gap-2 text-base font-medium text-spectyn-success">
                  <Check aria-hidden="true" className="size-5" />
                  分析完成 / Analysis ready
                </p>

                <p className="text-base text-spectyn-text">{result.summary}</p>

                {result.macroEstimate ? (
                  <dl className="grid grid-cols-2 gap-x-4 gap-y-1 text-base">
                    <dt className="text-spectyn-muted">熱量 / Calories</dt>
                    <dd className="text-right text-spectyn-text">
                      {Math.round(result.macroEstimate.calories)} kcal
                    </dd>
                    <dt className="text-spectyn-muted">蛋白質 / Protein</dt>
                    <dd className="text-right text-spectyn-text">
                      {Math.round(result.macroEstimate.proteinG)} g
                    </dd>
                    <dt className="text-spectyn-muted">碳水 / Carbs</dt>
                    <dd className="text-right text-spectyn-text">
                      {Math.round(result.macroEstimate.carbsG)} g
                    </dd>
                    <dt className="text-spectyn-muted">脂肪 / Fat</dt>
                    <dd className="text-right text-spectyn-text">
