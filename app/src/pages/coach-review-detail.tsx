...
// offline daily review (reuses the `spectyn coach review` backend via
...
export default function CoachReviewDetail({ date: dateProp, onBack }: CoachReviewDetailProps) {
  ...
  return (
    <div data-testid="coach-review-detail" className="min-h-screen flex flex-col bg-spectyn-bg text-spectyn-text pt-[env(safe-area-inset-top)] pb-[env(safe-area-inset-bottom)] pl-[env(safe-area-inset-left)] pr-[env(safe-area-inset-right)]">
      {/* Header — DOM order matches visual order. */}
      <header className="flex items-center gap-3 px-4 py-3 border-b border-spectyn-border">
        {onBack && (
          <button
            onClick={() => { impact("light"); onBack(); }}
            className="min-h-[44px] min-w-[44px] -ml-2 flex items-center justify-center rounded-lg text-spectyn-muted hover:text-spectyn-text transition-colors motion-reduce:transition-none"
            aria-label="返回 / Back"
          >
        ...
        <div className="w-10 h-10 rounded-lg bg-spectyn-primary/15 flex items-center justify-center flex-shrink-0">
          <CalendarDays size={20} className="text-spectyn-primary" aria-hidden="true" />
        </div>
        <div className="flex-1 min-w-0">
          <h1 className="text-lg font-bold text-spectyn-text truncate">每日回顧</h1>
          <p className="text-sm text-spectyn-muted truncate">
      ...
      <main className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
        ...
        {error && (
          <div
            className="bg-spectyn-danger/10 border border-spectyn-danger/40 rounded-lg p-4 text-base text-spectyn-danger flex items-start gap-3"
            role="alert"
          >
        ...
        {loading && !view && !error && (
          <div className="space-y-3" role="status" aria-busy="true" aria-label="載入中 / Loading">
            <div className="flex items-center gap-2 text-spectyn-muted text-base">
            ...
            {[0, 1, 2].map((i) => (
              <div
                key={i}
                className="min-h-[48px] rounded-lg bg-spectyn-card border border-spectyn-border animate-pulse motion-reduce:animate-none"
                aria-hidden="true"
              />
            ))}
          </div>
        ...
        {tomorrow && !tomorrow.skipped && (
          <section
            className="bg-spectyn-primary/10 border border-spectyn-primary/40 rounded-lg p-4"
            aria-label="明日的一個行動 / Tomorrow's one action"
          >
            <p className="text-sm font-semibold text-spectyn-primary flex items-center gap-1.5 mb-1.5">
              <Sparkles size={14} aria-hidden="true" /> 明天行動 / Tomorrow
            </p>
            <p className="text-lg text-spectyn-text whitespace-pre-wrap break-words leading-relaxed">
              {tomorrow.text}
            </p>
          </section>
        )}
        {tomorrow?.skipped && (
          <div className="bg-spectyn-card border border-spectyn-border rounded-lg p-4 text-sm text-spectyn-muted">
        ...
        {hasEvents && view?.flagged && (
          <div className="bg-spectyn-warning/10 border border-spectyn-warning/40 rounded-lg p-3 text-sm text-spectyn-muted">
        ...
        {!error && view?.locked && (
          <div className="bg-spectyn-card border border-spectyn-border rounded-lg p-6 text-center">
            <Lock size={24} className="text-spectyn-muted mx-auto mb-2" aria-hidden="true" />
            <p className="text-base text-spectyn-text">事件已加密(age v1) / Events encrypted</p>
            <p className="text-sm text-spectyn-muted mt-1">
              尚未載入身分金鑰 — 執行 <code className="text-spectyn-primary">spectyn init</code> 後重試。
            </p>
          </div>
        )}
        ...
        {!error && !loading && view === null && (
          <div className="bg-spectyn-card border border-spectyn-border rounded-lg p-6 text-center">
            <p className="text-base text-spectyn-text">{date} 尚無回顧。</p>
            <p className="text-sm text-spectyn-muted mt-1">No review for this date yet.</p>
          </div>
        )}
        ...
        {!error && view && !view.locked && view.eventCount === 0 && (
          <div className="bg-spectyn-card border border-spectyn-border rounded-lg p-6 text-center">
            <p className="text-base text-spectyn-text">{date} 沒有 Life Node 事件。</p>
            <p className="text-sm text-spectyn-muted mt-1">No Life Node events for this date.</p>
            <p className="text-sm text-spectyn-muted/70 mt-2">
                ...
                if (r.kind === "group") {
                  return (
                    <div key={`g-${i}`} className="flex items-baseline gap-2 pt-2 first:pt-0">
                      <span className="text-base font-semibold text-spectyn-primary">{r.tag}</span>
                      <span className="text-sm text-spectyn-muted">({r.n})</span>
                    </div>
                  );
                }
                return (
                  <div
                    key={`b-${i}`}
                    className="flex items-start gap-3 px-3 py-2.5 min-h-[44px] rounded-lg bg-spectyn-card border border-spectyn-border ml-2"
                  >
                    <span className="text-lg w-6 text-center flex-shrink-0" aria-hidden="true">
                      {KIND_EMOJI[r.eventKind] ?? "•"}
                    </span>
                    <div className="flex-1 min-w-0">
                      <span className="text-sm text-spectyn-muted mr-2">{r.time}</span>
                      <span className="text-base text-spectyn-text break-words">{r.summary}</span>
                    </div>
                  </div>
                );
              })}
      ...
      <footer className="sticky bottom-0 border-t border-spectyn-border bg-spectyn-bg px-4 py-3 pb-[max(0.75rem,env(safe-area-inset-bottom))]">
        <button
          onClick={() => { impact("light"); void refresh(date); }}
          disabled={loading}
          className="w-full min-h-[48px] flex items-center justify-center gap-2 rounded-lg bg-spectyn-primary text-spectyn-bg font-semibold text-base disabled:opacity-50 transition-opacity motion-reduce:transition-none"
