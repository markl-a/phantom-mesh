      ...
      <div className="flex min-h-screen flex-col">
        <main className="flex-1 overflow-y-auto px-5 pb-5 pt-6">
          <header className="mb-6">
            <p className="text-base font-medium text-phantom-muted">
              專注 / Focus
            </p>
            <h1 className="mt-2 text-2xl font-semibold tracking-normal text-phantom-text">
          ...
          <section className="rounded-lg border border-phantom-border bg-phantom-card p-4">
            <div className="mb-4 flex items-center gap-3">
              ...
              <div>
                <p className="text-lg font-semibold text-phantom-text">
                  {plannedLabel}
                </p>
            ...
            <fieldset>
              <legend className="mb-3 text-base font-medium text-phantom-text">
                選擇時長 / Duration
              </legend>
                    ...
                    <label
                      ...
                      className={`flex min-h-[44px] items-center gap-3 rounded-lg border p-3 transition motion-reduce:transition-none ${
                        selected
                          ...
                          : "border-phantom-border bg-phantom-bg"
                      }`}
                    >
                      <input
                        type="radio"
                        ...
                        className="min-h-[20px] min-w-[20px] accent-phantom-primary"
                      />
                      <span className="flex flex-1 items-center justify-between gap-3">
                        <span>
                          <span className="block text-base font-medium text-phantom-text">
                            {choice.label}
                          </span>
                        ...
                        {choice.mode === "custom" ? (
                          <span className="flex items-center gap-2">
                            <input
                              ...
                              className="min-h-[44px] w-20 rounded-lg border border-phantom-border bg-phantom-card px-3 text-base text-phantom-text disabled:opacity-50"
                            />
                            <span className="text-base text-phantom-muted">
                              min
                            </span>
                          </span>
                        ) : null}
          ...
          <section className="mt-4 rounded-lg border border-phantom-border bg-phantom-card p-4">
            <label className="block text-base font-medium text-phantom-text">
              備註 / Note
              <textarea
                value={note}
                onChange={(event) => {
                ...
                aria-label="備註 / Note"
                rows={4}
                className="mt-3 min-h-[96px] w-full resize-y rounded-lg border border-phantom-border bg-phantom-bg px-3 py-3 text-base text-phantom-text placeholder:text-phantom-muted"
          ...
          {error ? (
            <p role="alert" className="mt-4 text-base text-phantom-danger">
              {error}
            </p>
          ) : null}

          {done ? (
            <p role="status" className="mt-4 text-base text-phantom-muted">
              {done}
            </p>
          ) : null}
        </main>

        <footer className="sticky bottom-0 border-t border-phantom-border bg-phantom-bg/95 px-5 py-4 backdrop-blur">
          <button
            type="button"
            onClick={start}
            disabled={starting}
