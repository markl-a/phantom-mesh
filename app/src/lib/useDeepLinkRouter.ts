// SPEC-31 §6 + G1 — deep-link router: spectyn://<route> → allowlisted React Router navigate.
//
...
// OS-level `spectyn://...` URL and re-emits it as a `deep-link://<kind>` Tauri
...
// navigation-routing channel for allowlisted spectyn:// navigation URLs.
...
//   so only safe URLs reach JS (the OS routes EVERY `spectyn://…` here, scheme
...
//   justification as the existing `spectyn://demo-mode` case), NOT for "any URL it
...
//
// Note: the broker OAuth callback (`spectyn://oauth/callback?p=...`) is handled
...
/**
 * Allowlist of routes reachable from an external `spectyn://` URL, per
...
/** Result of parsing a `spectyn://<route>[?query]` URL. */
...
/**
 * Parse a `spectyn://<route>[?query]` URL into a React Router path + search.
 *
 ...
 * string directly. Returns null when the URL isn't a `spectyn://` deep-link.
 */
function parseDeepLink(url: string): ParsedDeepLink | null {
  const SCHEME = "spectyn://";
  ...
  // Normalise to a leading-slash React Router path. "spectyn://coach/review"
  // → "/coach/review"; "spectyn://" (empty route) → "/".
 ...
 * deep-link event convention) that routes allowlisted `spectyn://` deep-links
...
export function useDeepLinkRouter(): void {
  ...
  useEffect(() => {
    ...
    // navigation spectyn:// URLs as `deep-link://route` (Rust-side allowlist-gated
