...
export default function ChannelsPanel() {
  ...
  const daemonBadge = () => {
    switch (daemonStatus) {
      case "checking":
        return (
          <span className="inline-flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full bg-spectyn-border/50 text-spectyn-muted">
            <span className="w-2 h-2 rounded-full bg-spectyn-muted animate-pulse" />
            檢查中...
          </span>
        );
      case "online":
        return (
          <span className="inline-flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full bg-spectyn-success/20 text-spectyn-success">
            <span className="w-2 h-2 rounded-full bg-spectyn-success" />
            連線中
          </span>
        );
      case "offline":
        return (
          <span className="inline-flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full bg-spectyn-warning/20 text-spectyn-warning">
            <span className="w-2 h-2 rounded-full bg-spectyn-warning" />
            離線模式
          </span>
        );
    }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
          <div className="text-sm text-spectyn-muted">
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <div
                className={`w-12 h-12 rounded-lg flex items-center justify-center font-bold text-sm ${
                  channel.status === "connected"
                    ...
                    : "bg-spectyn-border/50 text-spectyn-muted"
                }`}
              >
                {channel.icon}
              </div>
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-spectyn-muted">{channel.description}</p>
              </div>
              ...
              <div className="flex items-center gap-4 shrink-0">
                {channel.status === "connected" && (
                  <div className="text-right">
                    ...
                    <p className="text-xs text-spectyn-muted">訊息數</p>
                  </div>
                )}
                <button
                  ...
                  className={`px-3 py-1.5 rounded text-xs font-medium border ${
                    channel.status === "connected"
                      ? "border-spectyn-primary text-spectyn-primary hover:bg-spectyn-primary/10"
                      : "border-spectyn-border text-spectyn-muted hover:text-spectyn-text hover:border-spectyn-text"
                  }`}
                >
                  設定
                </button>
            ...
            {showConfig === channel.id && (
              <div className="mt-4 pt-4 border-t border-spectyn-border">
                {channel.status === "connected" ? (
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                    <div>
                      <label className="block text-spectyn-muted text-xs mb-1">Bot Token</label>
                      <div className="bg-spectyn-bg border border-spectyn-border rounded px-3 py-2 font-mono text-spectyn-muted">
                        ••••••••••••:•••••••••••
                      </div>
                    </div>
                    <div>
                      <label className="block text-spectyn-muted text-xs mb-1">Chat ID</label>
                      <div className="bg-spectyn-bg border border-spectyn-border rounded px-3 py-2 font-mono text-spectyn-muted">
                        -100**********
                      </div>
                    </div>
                    <div>
                      <label className="block text-spectyn-muted text-xs mb-1">狀態</label>
                      <div className="bg-spectyn-bg border border-spectyn-border rounded px-3 py-2">
                        <span className="text-spectyn-success">Webhook 活躍</span>
                      </div>
                    </div>
                  </div>
                ) : (
                  <div className="text-center py-4">
                    <p className="text-sm text-spectyn-muted mb-3">尚未設定此頻道</p>
