  ...
  if (pages.length === 0) {
    return (
      <div className="text-center py-8">
        <p className="text-sm text-spectyn-muted">尚無頁面</p>
        <p className="text-xs text-spectyn-muted mt-1">在對話頁請 Agent 生成</p>
      </div>
    );
  }
      ...
      {pages.map((page) => (
        ...
        >
          <div className="min-w-0 flex-1">
            ...
            {page.description && (
              <p className="text-xs text-spectyn-muted truncate">{page.description}</p>
            )}
          </div>
          <button
            onClick={(e) => { e.stopPropagation(); onDelete(page.name); }}
            className="text-spectyn-muted hover:text-spectyn-danger p-1 flex-shrink-0"
            title="刪除"
          >
            <Trash2 size={12} />
          </button>
