    ...
    <div className="max-w-2xl mx-auto space-y-6" data-testid="food-capture-panel">
      <header className="flex items-center gap-3">
        <div className="w-10 h-10 rounded-lg bg-spectyn-primary/15 flex items-center justify-center">
          <Utensils size={20} className="text-spectyn-primary" />
        </div>
        <div>
          <h1 className="text-xl font-bold text-spectyn-text">飲食記錄</h1>
          <p className="text-xs text-spectyn-muted">Food log · SPEC-20 capture-food</p>
        </div>
      </header>

      <div className="bg-spectyn-card border border-spectyn-border rounded-lg p-5 space-y-3">
        <textarea
          value={text}
          onChange={(e) => setText(e.target.value)}
          placeholder="今天吃了什麼？（例：中午的鮭魚便當 + 一杯無糖豆漿）"
          rows={3}
          className="w-full bg-spectyn-bg border border-spectyn-border rounded px-3 py-2 text-sm text-spectyn-text placeholder-spectyn-muted focus:outline-none focus:border-spectyn-primary resize-none"
        />
        <div className="flex items-center justify-between">
          <span className="text-[11px] text-spectyn-muted">標籤：fat_loss（減脂）</span>
          <button
      ...
      {result && (
        <div className="bg-spectyn-card border border-spectyn-border rounded-lg p-5 space-y-4">
          {result.summary && (
          ...
          {macro ? (
            <div className="grid grid-cols-5 gap-2 text-center">
              {[
                ...
                <div key={m.l}>
                  <p className="text-base font-bold text-spectyn-text tabular-nums">{m.v}<span className="text-[10px] text-spectyn-muted">{m.u}</span></p>
                  <p className="text-[10px] text-spectyn-muted">{m.l}</p>
                </div>
              ))}
            </div>
          ) : (
            <p className="text-xs text-spectyn-muted">無法估算巨量營養素（信心不足或純飲料）</p>
          )}

          <div>
            <div className="flex items-center justify-between mb-1">
              <span className="text-xs text-spectyn-muted">減脂對齊分數</span>
              <span className="text-xs text-spectyn-primary">{Math.round((result.fatLossScore ?? 0) * 100)}%</span>
            </div>
            <div className="w-full h-1.5 bg-spectyn-bg rounded-full overflow-hidden">
              <div className="h-full bg-spectyn-primary rounded-full transition-all" style={{ width: `${Math.min(100, Math.max(0, (result.fatLossScore ?? 0) * 100))}%` }} />
            </div>
          </div>
