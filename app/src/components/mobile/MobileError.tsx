...
export function MobileErrorView({ code }: { code: string }) {
  ...
  const reportUrl =
    "https://github.com/markl-a/phantom-mesh/issues/new?title=" +
  ...
  return (
    <div className="flex flex-col items-center justify-center h-[100dvh] bg-phantom-bg px-6 text-center">
      <AlertTriangle
        size={56}
        className="text-phantom-danger mb-4"
        aria-label="錯誤圖示"
      />
      <h1 className="text-lg font-semibold text-phantom-text">發生未預期錯誤</h1>
      <p className="text-sm text-phantom-muted mt-2 max-w-xs">
        Phantom 遇到無法自動復原的問題。你可以重試、回報，或重設後重新設定。
      </p>
      <code className="text-xs text-phantom-muted font-mono mt-3">code: {errorCode}</code>

      <div className="flex flex-col gap-2 w-full max-w-xs mt-6">
        <button
          onClick={retry}
          className="flex items-center justify-center gap-2 bg-phantom-primary/15 text-phantom-primary py-2.5 rounded-lg text-sm font-medium hover:bg-phantom-primary/25 transition"
        >
        ...
        <a
          ...
          className="flex items-center justify-center gap-2 bg-phantom-card border border-phantom-border text-phantom-text py-2.5 rounded-lg text-sm hover:border-phantom-primary transition"
        >
          <Flag size={16} /> 回報問題
        </a>
        <button
          onClick={reset}
          className="flex items-center justify-center gap-2 bg-phantom-card border border-phantom-border text-phantom-danger py-2.5 rounded-lg text-sm hover:border-phantom-danger transition"
        >
