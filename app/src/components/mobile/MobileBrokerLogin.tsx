...
export default function MobileBrokerLogin() {
  ...
  const pickCoordinator = (peerUrl: string) => {
    try {
      ...
      localStorage.setItem("SPECTYN_HOST", host);
      localStorage.setItem("SPECTYN_PORT", port);
      localStorage.setItem("SPECTYN_SCHEME", scheme);
      localStorage.setItem("SPECTYN_THIN_SHELL", "1");
  ...
  return (
    <div className="space-y-3">
      ...
      {phase.kind === "loading" && (
        <div className="text-sm text-spectyn-muted">讀取登入狀態 …</div>
      )}

      {phase.kind === "logged-out" && (
        <button
          onClick={handleSignIn}
          className="w-full bg-spectyn-primary text-spectyn-bg font-medium px-4 py-3 rounded-lg active:opacity-80"
        >
          以 phantommesh.io 登入
        </button>
      )}
      ...
      {phase.kind === "logged-in" && (
        <div className="space-y-3">
          <div className="bg-spectyn-card border border-spectyn-border rounded-lg p-3 text-sm">
            <div className="text-spectyn-muted text-xs">已登入</div>
            <div className="text-spectyn-text font-medium mt-0.5">
              {phase.identity.email}
            </div>
            <div className="text-spectyn-muted text-xs mt-1">
              provider: {phase.identity.provider}
              {phase.identity.display_name ? ` · ${phase.identity.display_name}` : ""}
            </div>
            <div className="text-spectyn-muted text-[11px] mt-1">
              token 有效期至{" "}
          ...
          <div className="bg-spectyn-card border border-spectyn-border rounded-lg p-3 text-sm">
            <div className="text-spectyn-muted text-xs flex items-center justify-between">
              <span>已同步</span>
              <button
                onClick={handleResync}
                className="text-spectyn-primary text-[11px] underline"
              >
                重新同步
              </button>
            </div>
            {phase.sync ? (
              <div className="mt-2 space-y-1">
                <div className="text-spectyn-text text-xs">
                  LLM keys: {phase.sync.keys_written.length} 個
                </div>
                {phase.sync.keys_written.length > 0 && (
                  <div className="text-spectyn-muted text-[11px]">
                ...
                <div className="text-spectyn-muted text-[10px] break-all">
                  {phase.sync.env_path}
                </div>
              </div>
            ) : (
          ...
          {/* Coordinator picker — Android variant A pattern: each peer is
              a spectyn serve node; pick one and the WebView reloads to
          ...
          {phase.sync && Array.isArray(phase.sync.peers) && phase.sync.peers.length > 0 && (
            <div className="bg-spectyn-card border border-spectyn-border rounded-lg p-3 text-sm">
              <div className="text-spectyn-muted text-xs">選協調者（Coordinator）</div>
              <div className="text-spectyn-muted text-[11px] mt-0.5 mb-2">
              ...
              <div className="flex flex-col gap-2">
                {phase.sync.peers.map((p) => (
                  <button
                    ...
                    className="text-left bg-spectyn-bg border border-spectyn-border rounded-md px-3 py-2 hover:border-spectyn-primary active:opacity-80"
                  >
                    <div className="text-spectyn-text text-xs font-medium">{p.name}</div>
                    <div className="text-spectyn-muted text-[11px] break-all mt-0.5">{p.url}</div>
                    {p.label && (
                      <div className="text-spectyn-muted text-[10px] mt-0.5">{p.label}</div>
                    )}
                  </button>
                ))}
              </div>
              <div className="text-spectyn-muted text-[10px] mt-2">
                不在列表？到桌面端跑 <code>spectyn cluster join</code> 把那台註冊進你的帳號。
              </div>
            </div>
          )}
