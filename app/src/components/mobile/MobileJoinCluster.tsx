// MobileJoinCluster — 3-step form to join an existing phantom-mesh cluster.
...
async function testCoordinator(url: string, secret: string): Promise<{ ok: boolean; error?: string; name?: string }> {
  ...
  try {
    ...
    let respJson: { name?: string; phantom_version?: string } | null = null;
    if (isIOS) {
      ...
      respJson = await r.json() as { name?: string; phantom_version?: string };
    }
...
export default function MobileJoinCluster({ discovered, onDone, onCancel }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      localStorage.setItem("phantom_mesh_v2_onboarded_mode", "join");
      localStorage.setItem("phantom_mesh_device_name", deviceName);
    } catch (_e) { /* ignore */ }
  ...
  return (
    <div className="h-[100dvh] bg-phantom-bg flex flex-col">
      {/* Header */}
      <header
        className="flex items-center px-4 py-3 border-b border-phantom-border flex-shrink-0"
        style={{ paddingTop: "calc(env(safe-area-inset-top) + 0.75rem)" }}
      >
        <button onClick={handleBackOrCancel}
                aria-label="返回"
                className="p-2 -ml-2 text-phantom-muted">
          <ChevronLeft size={22} />
        </button>
        <h2 className="flex-1 text-base font-semibold text-phantom-text text-center">
      ...
      <main className="flex-1 overflow-y-auto px-4 py-6">
        {step === 1 && (
          <>
            <h3 className="text-lg font-semibold text-phantom-text mb-1">Coordinator URL</h3>
            <p className="text-sm text-phantom-muted mb-4">
              你 mac 上 phantom serve 監聽的位址 (mac 預設 :7878)
            </p>
            {discovered && (
              <button
                onClick={() => setUrl(discovered.url)}
                className="w-full mb-3 p-3 border border-phantom-primary/50 bg-phantom-primary/10 rounded-lg text-left"
              >
                <div className="text-xs text-phantom-muted">本機網路發現</div>
                <div className="text-phantom-text font-mono text-sm truncate">{discovered.host}:{discovered.port}</div>
              </button>
            )}
            <input
              type="url"
              ...
              className="w-full p-3 bg-phantom-bg-elevated border border-phantom-border rounded-lg text-phantom-text font-mono text-sm"
            />
        ...
        {step === 2 && (
          <>
            <h3 className="text-lg font-semibold text-phantom-text mb-1">Cluster Secret</h3>
            <p className="text-sm text-phantom-muted mb-4">
              從 mac 上 <code className="bg-phantom-bg-elevated px-1 rounded">~/.phantom-mesh/agents.toml</code> 找
              <code className="bg-phantom-bg-elevated px-1 rounded">cluster_secret = "..."</code>
            </p>
            <input
              type="password"
              value={secret}
        ...
        {step === 3 && (
          <>
            <h3 className="text-lg font-semibold text-phantom-text mb-1">這個裝置叫什麼？</h3>
            <p className="text-sm text-phantom-muted mb-4">
            ...
            <input
              ...
              className="w-full p-3 bg-phantom-bg-elevated border border-phantom-border rounded-lg text-phantom-text"
            />
            <div className="mt-6 p-3 bg-phantom-bg-elevated border border-phantom-border rounded-lg text-xs text-phantom-muted space-y-1">
              <div><span className="text-phantom-muted">Coordinator:</span> <code className="text-phantom-text">{url}</code></div>
              <div><span className="text-phantom-muted">Secret:</span> <code className="text-phantom-text">{"●".repeat(Math.min(secret.length, 16))}</code></div>
              <div><span className="text-phantom-muted">Capabilities:</span> <code className="text-phantom-text">camera, microphone, mobile-ios</code></div>
            </div>
            {testError && (
      ...
      {/* Footer button */}
      <footer className="border-t border-phantom-border p-4 flex-shrink-0" style={{ paddingBottom: "calc(env(safe-area-inset-bottom) + 1rem)" }}>
        {step < 3 ? (
          <button
            onClick={() => setStep(s => s + 1)}
            disabled={step === 1 ? !url.trim() : !secret.trim()}
            className="w-full py-3 bg-phantom-primary text-white rounded-lg font-semibold disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2"
          >
          ...
          <button
            ...
            disabled={testing}
            className="w-full py-3 bg-phantom-primary text-white rounded-lg font-semibold disabled:opacity-40 flex items-center justify-center gap-2"
          >
