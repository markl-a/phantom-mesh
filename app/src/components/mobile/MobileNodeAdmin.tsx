    ...
    <div className="space-y-6 max-w-md mx-auto">
      ...
      <section data-testid="section-rotate" className="space-y-2">
        <div className="flex items-center gap-2">
          <Key size={16} className="text-phantom-primary" />
          <h2 className="text-sm font-medium text-phantom-text">Broker token</h2>
        </div>
        <p className="text-xs text-phantom-muted">
          phantommesh.io 簽發給這台裝置的 token。按 Rotate 會在本機產生新 token；
          下次跟 broker 通訊時會用新值（可能需要重新登入）。
        </p>
        <div className="bg-phantom-card border border-phantom-border rounded-lg px-3 py-2.5">
          <div className="text-[10px] text-phantom-muted">目前 token</div>
          <code
            data-testid="rotate-current-preview"
            className="text-sm text-phantom-text font-mono"
          >
            {tokenPreview?.token_preview || '(尚未登入 broker)'}
          </code>
          {tokenPreview?.broker_url && (
            <div className="text-[10px] text-phantom-muted mt-1 break-all">
              {tokenPreview.broker_url}
            </div>
          )}
        </div>
      ...
      <section data-testid="section-peer" className="space-y-2">
        <div className="flex items-center gap-2">
          <Plus size={16} className="text-phantom-primary" />
          <h2 className="text-sm font-medium text-phantom-text">手動加 peer</h2>
        </div>
        <p className="text-xs text-phantom-muted">
          把另一個 phantom-mesh 節點寫進 <code className="bg-phantom-card px-1 rounded">~/.phantom-mesh/agents.toml</code>{' '}
          的 <code className="bg-phantom-card px-1 rounded">[cluster] peers</code>。
        </p>
        <input
      ...
      <section data-testid="section-heartbeat" className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-medium text-phantom-text">Heartbeat 間隔</h2>
        </div>
        <p className="text-xs text-phantom-muted">
          多久 ping 一次 cluster peers — 範圍 {MIN_HEARTBEAT}–{MAX_HEARTBEAT} 秒。
          數字越小越快發現掉線，越大越省電。
        </p>
        <div className="bg-phantom-card border border-phantom-border rounded-lg px-3 py-3 space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-xs text-phantom-muted">目前</span>
            <span
              data-testid="heartbeat-value"
              className="text-sm font-mono text-phantom-text"
            >
              {heartbeat}s
            </span>
          </div>
          <input
            ...
            className="w-full accent-phantom-primary"
          />
          <div className="flex justify-between text-[10px] text-phantom-muted">
...
function PillView({
  ...
  if (pill.kind === 'loading') {
    return (
      <div {...rest} className={`${base} bg-phantom-card border border-phantom-border text-phantom-muted`}>
  ...
  if (pill.kind === 'ok') {
    return (
      <div
        {...rest}
        className={`${base} bg-phantom-success/15 border border-phantom-success/40 text-phantom-success`}
      >
  ...
  return (
    <div
      {...rest}
      className={`${base} bg-phantom-danger/15 border border-phantom-danger/40 text-phantom-danger`}
    >
